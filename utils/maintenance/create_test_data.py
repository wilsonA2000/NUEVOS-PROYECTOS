...
def create_properties(landlord, amenities):
    ...
    for prop_data in properties_data:
        ...
        property_obj = Property.objects.create(landlord=landlord, **prop_data)
        ...
        for name in amenity_names:
            ...
            if amenity:
                PropertyAmenityRelation.objects.create(
                    property=property_obj, amenity=amenity
                )
...
def create_messages(users):
    ...
    thread = MessageThread.objects.create(
        subject="Consulta sobre apartamento", created_by=tenant
    )
...
def main():
    ...
    try:
        ...
        if User.objects.filter(
            email__in=["landlord@test.com", "tenant@test.com"]
        ).exists():
            print("⚠️  Los usuarios de prueba ya existen. Limpiando datos anteriores...")
            # Limpiar datos de prueba existentes
            User.objects.filter(
                email__in=["landlord@test.com", "tenant@test.com", "service@test.com"]
            ).delete()
