import { api } from './api';
import { User, LoginDto, RegisterDto, UpdateProfileDto } from '../types/user';
...
const processError = (error: any): Error => {
  ...
  if (!error.response) {
    if (
      ...
      return new Error(
        'No se puede conectar con el servidor VeriHome.\n\nVerifica que el servidor este ejecutandose.',
      );
    }
    if (
    ...
    if (error.code === 'TIMEOUT' || error.message?.includes('timeout')) {
      return new Error(
        'La peticion tardo demasiado tiempo.\n\nIntenta nuevamente en unos segundos.',
      );
    }
  ...
  if (status === 400 && data?.error_type) {
    switch (data.error_type) {
      ...
      default:
        return new Error(
          data.detail || 'Error de validacion. Verifica los datos ingresados.',
        );
    }
  }
  ...
  // Error genérico
  return new Error(
    data?.detail || data?.message || 'Ha ocurrido un error inesperado.',
  );
};
...
export const authService = {
  async login(data: LoginDto): Promise<User> {
    try {
      ...
      if (access && refresh) {
        ...
        throw new Error(
          'No se recibieron tokens validos del servidor.\n\nIntenta nuevamente o contacta soporte.',
        );
      }
    } catch (error: any) {
  ...
  async register(data: RegisterDto): Promise<User> {
    try {
      ...
      if (
        !registrationData.interview_code ||
        registrationData.interview_code.trim() === ''
      ) {
  ...
  async changePassword(
    ...
    try {
      await api.put('/users/auth/change-password/', {
        oldPassword,
        newPassword,
      });
    } catch (error: any) {
