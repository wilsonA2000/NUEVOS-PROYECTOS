...
const BiometricAuthenticationPage: React.FC = () => {
  ...
  const validateTurn = async () => {
    ...
    try {
      ...
      const response = await api.post(
        `/contracts/${id}/start-biometric-authentication/`,
      );
      ...
      if (error.response?.status === 423) {
        ...
        setError(
          'La autenticación biométrica ya fue completada por ambas partes.',
        );
      } else {
        // Otros errores
  ...
  if (!validStatesForAuth.includes(contract.status)) {
    ...
    const suggestedActionByState: Record<string, string> = {
      ...
      PUBLISHED:
        'Este contrato ya nació a la vida jurídica (firmas completas).',
    ...
    try {
      ...
      if (userType === 'tenant') {
        ...
        navigate('/app/contracts/tenant', {
          state: {
            message:
              'Autenticación completada. Esperando autenticación del arrendador.',
            type: 'success',
          },
        });
        ...
        navigate('/app/contracts', {
          state: {
            message:
              '¡Contrato firmado y activo! El contrato ha nacido a la vida jurídica.',
            type: 'success',
          },
        });
      } else {
      ...
      setError(
        `Error: ${error.response?.data?.error || 'No se pudo completar la autenticación'}`,
      );
      setLoading(false);
    }
  };
          ...
          <Box mt={2} p={2} bgcolor='rgba(25, 118, 210, 0.05)' borderRadius={1}>
            ...
            <Box component='ol' pl={2}>
              <Typography
                ...
                variant='body2'
                color={
                  turnValidation.currentTurn === 'tenant'
                    ? 'primary'
                    : 'text.secondary'
                }
              >
                <strong>1. Arrendatario</strong>{' '}
                {turnValidation.currentTurn === 'tenant'
                  ? '(En proceso...)'
                  : contract.guarantor
                    ? '(Pendiente)'
              ...
              {contract.guarantor && (
                <Typography
                  ...
                  variant='body2'
                  color={
                    (turnValidation.currentTurn as string) === 'guarantor'
                      ? 'primary'
                      : 'text.secondary'
                  }
                >
                  <strong>2. Garante/Codeudor</strong>{' '}
                  {(turnValidation.currentTurn as string) === 'guarantor'
                    ? '(En proceso...)'
                    : turnValidation.currentTurn === 'tenant'
                      ? '(Esperando...)'
              ...
              <Typography
                ...
                variant='body2'
                color={
                  turnValidation.currentTurn === 'landlord'
                    ? 'primary'
                    : 'text.secondary'
                }
              >
                <strong>{contract.guarantor ? '3' : '2'}. Arrendador</strong>{' '}
                {turnValidation.currentTurn === 'landlord'
                  ? '(En proceso...)'
                  : '(Esperando...)'}
              </Typography>
            ...
            {user?.user_type === 'landlord' &&
              turnValidation.currentTurn === 'tenant' && (
                <Typography variant='body2'>
                  El arrendatario debe completar su verificación biométrica
                  primero. Una vez que termine, recibirás una notificación y
                  podrás continuar con tu autenticación.
                </Typography>
              )}
            {user?.user_type === 'landlord' &&
              (turnValidation.currentTurn as string) === 'guarantor' && (
                <Typography variant='body2'>
                  El garante/codeudor debe completar su verificación biométrica.
                  Una vez que termine, recibirás una notificación y podrás
                  continuar con tu autenticación.
                </Typography>
              )}
            {user?.user_type === 'tenant' &&
              (turnValidation.currentTurn as string) === 'guarantor' && (
                <Typography variant='body2'>
                  Has completado tu autenticación exitosamente. Ahora el
                  garante/codeudor debe completar su verificación biométrica
                  antes de que el arrendador pueda proceder.
                </Typography>
              )}
            {user?.user_type === 'tenant' &&
              turnValidation.currentTurn === 'landlord' && (
                <Typography variant='body2'>
                  ¡Felicitaciones! Has completado tu autenticación biométrica
                  exitosamente. Ahora el arrendador debe completar su
                  verificación biométrica para activar el contrato. Te
  ...
  if (isTenant) {
    userInfo = {
      fullName:
        contract.tenant?.full_name || contract.secondary_party?.full_name || '',
      documentNumber:
        contract.tenant?.document_number ||
        contract.secondary_party?.document_number ||
        '',
      documentIssueDate:
        contract.tenant?.document_issue_date ||
        contract.secondary_party?.document_issue_date ||
        '',
    };
  } else if (isLandlord) {
    userInfo = {
      fullName:
        contract.landlord?.full_name || contract.primary_party?.full_name || '',
      documentNumber:
        contract.landlord?.document_number ||
        contract.primary_party?.document_number ||
        '',
      documentIssueDate:
        contract.landlord?.document_issue_date ||
        contract.primary_party?.document_issue_date ||
        '',
    };
  } else {
