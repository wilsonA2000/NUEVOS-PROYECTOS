...
const PaymentDashboardPage: React.FC = () => {
  ...
  const buildFilter = useCallback((tabIdx: number) => {
    if (isAdmin) {
      ...
      switch (tabIdx) {
        case 1: return { status: 'pending' as const };
        case 2: return { overdue: true };
        case 3: return { status: 'paid' as const };
        default: return {};
      }
    }
    // Tenant/Landlord/Provider: 0=pendientes, 1=pagadas, 2=mora
    switch (tabIdx) {
      case 0: return { status: 'pending' as const };
      case 1: return { status: 'paid' as const };
      case 2: return { overdue: true };
      default: return {};
    }
  }, [isAdmin]);

  const loadOrders = useCallback(async (tabIdx: number) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const tabsForRole = (): string[] => {
    ...
    if (userType === 'service_provider') return ['Por cobrar', 'Cobrado', 'En mora'];
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      {summary && (
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard icon={<ReceiptIcon fontSize="inherit" />}
              label="Total órdenes" value={summary.total} />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard icon={<WalletIcon fontSize="inherit" />}
              label="Pendientes" value={summary.pending} color="warning.main" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard icon={<WarningIcon fontSize="inherit" />}
              label="En mora" value={summary.overdue} color="error.main" />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard icon={<CheckIcon fontSize="inherit" />}
              label="Pagadas este mes" value={summary.paid_this_month} color="success.main" />
          </Grid>
          {summary.total_amount_pending > 0 && (
            <Grid item xs={12}>
              <Alert severity="info" sx={{ mt: 1 }}>
                Monto total pendiente: <strong>{formatCOP(summary.total_amount_pending)}</strong>
              </Alert>
            </Grid>
          )}
        </Grid>
      ...
      <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ mb: 2 }}>
        {labels.map((label, idx) => <Tab key={idx} label={label} />)}
      </Tabs>
