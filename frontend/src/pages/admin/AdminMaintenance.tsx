  ...
  const runAction = useCallback(
    async (
      ...
      try {
        ...
        setResult(null);
        const response =
          method === 'get' ? await api.get(endpoint) : await api.post(endpoint);
        setResult({
          success: true,
          message: response.data?.message || 'Operacion completada',
          ...
          <Card elevation={2}>
            <CardContent>
              ...
              {healthStatus && (
                <List dense>
                  ...
                  <ListItem>
                    <ListItemIcon>
                      {getStatusIcon(healthStatus.redis?.status)}
                    </ListItemIcon>
                    <ListItemText
                      primary='Redis / Cache'
                    ...
                    />
                    <Chip
                      label={healthStatus.redis?.status || 'unknown'}
                      size='small'
                      color={
                  ...
                  <ListItem>
                    ...
                    />
                    <Chip
                      label={healthStatus.storage?.status || 'unknown'}
                      size='small'
                      color={
                        healthStatus.storage?.status === 'healthy'
                          ? 'success'
                          : 'warning'
                      }
                    />
                  ...
                  <ListItem>
                    <ListItemIcon>
                      {getStatusIcon(healthStatus.celery?.status)}
                    </ListItemIcon>
                    <ListItemText
                    ...
                    />
                    <Chip
                      label={healthStatus.celery?.status || 'unknown'}
                      size='small'
                      color={
                        healthStatus.celery?.status === 'healthy'
                          ? 'success'
                          : 'warning'
                      }
                    />
            ...
            <CardActions>
              <Button
                ...
                onClick={() =>
                  runAction('health', '/core/maintenance/health/', 'get')
                }
              >
        ...
        <Grid item xs={12} md={6}>
          <Card elevation={2}>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Eliminar logs antiguos del sistema para liberar espacio. Se
                conservan los ultimos 30 dias.
              </Typography>
            </CardContent>
            <CardActions>
              <Button
                ...
                onClick={() =>
                  confirmAction(
                    'clear-logs',
                    'Limpiar Logs',
                    'Se eliminaran los logs con mas de 30 dias de antiguedad. Esta accion no se puede deshacer.',
                  )
                }
              >
        ...
        <Grid item xs={12} md={6}>
          <Card elevation={2}>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Limpiar cache del sistema para forzar la actualizacion de datos.
              ...
              <Button
                ...
                onClick={() =>
                  confirmAction(
                    'clear-cache',
                    'Limpiar Cache',
                    'Se limpiara toda la cache del sistema. Esto puede causar lentitud temporal mientras se regenera.',
                  )
                }
              >
                Limpiar Cache
        ...
        <Grid item xs={12} md={6}>
          <Card elevation={2}>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Cerrar sesiones expiradas o inactivas. Las sesiones activas no
                se veran afectadas.
              </Typography>
            </CardContent>
            <CardActions>
              <Button
                ...
                onClick={() =>
                  confirmAction(
                    'clear-sessions',
                    'Limpiar Sesiones',
                    'Se cerraran todas las sesiones expiradas e inactivas.',
                  )
                }
              >
        ...
        <Grid item xs={12} md={6}>
          <Card elevation={2}>
            <CardContent>
              ...
              <Typography variant='body2' color='text.secondary' sx={{ mb: 2 }}>
                Ejecutar optimizacion de indices y estadisticas de la base de
                datos.
              </Typography>
            </CardContent>
            <CardActions>
              <Button
                ...
                onClick={() =>
                  confirmAction(
                    'optimize-db',
                    'Optimizar BD',
                    'Se ejecutara VACUUM ANALYZE en la base de datos. Esto puede tomar varios minutos.',
                  )
                }
              >
