    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity='error'>
          ...
          {(error as Error)?.message || 'No encontrado'}
        </Alert>
        <Button
          startIcon={<BackIcon />}
    ...
    <Box sx={{ p: 3 }}>
      ...
      {contract.is_urgent && (
        <Alert severity='error' icon={<WarningIcon />} sx={{ mb: 3 }}>
          Este contrato lleva más de 7 días pendiente de revisión. Se recomienda
          atención inmediata.
        </Alert>
      )}
              ...
              const FLOW_STAGES = [
                {
                  ...
                  desc: 'Admin revisa y aprueba',
                },
                {
                  key: 'DRAFT',
                  label: 'Borrador Aprobado',
                  desc: 'Listo para el arrendatario',
                },
                {
                  key: 'BOTH_REVIEWING',
                  label: 'Arrendatario Revisando',
                  desc: 'Tenant acepta contrato',
                },
                {
                  key: 'TENANT_AUTHENTICATION',
                  label: 'Firma Biométrica',
                  desc: 'Tenant → Garante → Arrendador',
                },
                {
                  key: 'ACTIVE',
                  label: 'Contrato Activo',
                  desc: 'En plena ejecución',
                },
              ];
              const currentIdx = FLOW_STAGES.findIndex(
                s => s.key === contract.current_state,
              );
                      ...
                      <Step key={stage.key} completed={done}>
                        <StepLabel
                          StepIconComponent={() =>
                              ...
                              <DoneStepIcon
                                sx={{ color: vhColors.success, fontSize: 20 }}
                              />
                            ) : active ? (
                              <ActiveStepIcon
                                sx={{
          ...
          <Paper sx={{ p: 2 }}>
            ...
            <List dense disablePadding>
              ...
              {(!contract.history_entries ||
                contract.history_entries.length === 0) && (
                <Typography
                  variant='body2'
                  color='text.secondary'
                  sx={{ py: 2 }}
