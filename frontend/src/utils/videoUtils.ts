...
export class VideoProcessor {
  ...
  static async generateThumbnails(
    ...
    return new Promise(resolve => {
      ...
      const generateNext = () => {
        ...
        const timePoint =
          (metadata.duration / (count + 1)) * (currentThumbnail + 1);
  ...
  static async compressVideo(
    ...
    try {
      ...
      if (
        ...
        originalMetadata.duration <= config.maxDuration
      ) {
        return {
          file,
      ...
      const compressedSize = Math.floor(
        file.size * compressionRatio * config.quality,
      );
   ...
   */
  static getOptimalCompressionSettings(
    metadata: VideoMetadata,
  ): CompressionOptions {
  ...
  static validateYouTubeUrl(url: string): {
    ...
    if (!videoId) {
      return {
        valid: false,
        error:
          'URL de YouTube inválida. Formatos soportados: youtube.com/watch?v=..., youtu.be/..., o solo el ID del video',
      };
    }
