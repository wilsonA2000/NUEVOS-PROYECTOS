...
export const useBusinessNotifications = (): NotificationTrigger => {
  ...
  const onPropertyInterest = useCallback(
    async (property: any, message?: string) => {
      if (!user || !property.landlord) return;

      try {
        await businessNotificationService.notifyPropertyInterest(
          user,
          property,
          property.landlord,
        );
  ...
  const onServiceRequest = useCallback(
    async (serviceProvider: any, serviceDetails: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyServiceRequest(
          user,
          serviceProvider,
          serviceDetails,
        );
  ...
  const onRequestAccepted = useCallback(
    async (requester: any, requestDetails: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyRequestAccepted(
          user,
          requester,
          requestDetails,
        );
  ...
  const onRequestRejected = useCallback(
    async (requester: any, requestDetails: any) => {
      ...
      try {
        await businessNotificationService.notifyRequestRejected(
          user,
          requester,
          requestDetails,
        );
  ...
  const onMessageSent = useCallback(
    async (recipient: any, message: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyMessageReceived(
          user,
          recipient,
          message,
        );
  ...
  const onContractCreated = useCallback(
    async (signatory: any, contract: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyContractCreated(
          user,
          signatory,
          contract,
        );
  ...
  const onContractSigned = useCallback(
    async (otherParty: any, contract: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyContractSigned(
          user,
          otherParty,
          contract,
        );
  ...
  const onRatingGiven = useCallback(
    async (rated: any, rating: any) => {
      if (!user) return;

      try {
        await businessNotificationService.notifyRatingReceived(
          user,
          rated,
          rating,
        );
  ...
  const onVerificationCompleted = useCallback(
    async (verificationType: string) => {
      if (!user) return;
  ...
  const onSystemAlert = useCallback(
    async (message: string, targetUsers?: any[]) => {
      if (!user) return;
