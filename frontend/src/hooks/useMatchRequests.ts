...
export const useMatchRequests = (): UseMatchRequestsReturn => {
  ...
  const getStatusColor = (status: string) =>
    matchingService.getMatchStatusColor(status);
  const getStatusText = (status: string) =>
    matchingService.getMatchStatusText(status);
  const getPriorityColor = (priority: string) =>
    matchingService.getPriorityColor(priority);
  const getPriorityText = (priority: string) =>
    matchingService.getPriorityText(priority);
  const formatCurrency = (amount: number | null) =>
    matchingService.formatCurrency(amount);

  const isExpired = (expiresAt: string): boolean => {
    return (
      matchingService.calculateDaysUntilExpiry(expiresAt) !== null &&
