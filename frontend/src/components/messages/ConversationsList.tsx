...
export const ConversationsList: React.FC<ConversationsListProps> = ({
  ...
  const filteredConversations = conversations.filter(conv => {
    ...
    return (
      ...
      (conv.context?.property?.title &&
        conv.context.property.title
          .toLowerCase()
          .includes(searchTerm.toLowerCase())) ||
      (conv.context?.service?.title &&
        conv.context.service.title
          .toLowerCase()
          .includes(searchTerm.toLowerCase()))
    );
  ...
  if (filteredConversations.length === 0) {
    return (
      <Box sx={{ p: 4, textAlign: 'center' }}>
        ...
        >
          {!searchTerm &&
            'Las conversaciones aparecerán aquí cuando aceptes solicitudes'}
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <List sx={{ p: 0 }}>
      {filteredConversations.map(conversation => {
        ...
        const isOnline =
          onlineUsers.get(participant.id)?.isOnline || participant.is_online;
        const isSelected = selectedConversation?.id === conversation.id;
        const isTyping = typingUsers.some(
          user => user.userId === participant.id,
        );
                ...
                <Box>
                  ...
                  >
                    ...
                    >
                      ...
                      {conversation.context?.property?.title ||
                        conversation.context?.service?.title}
                    </Typography>
