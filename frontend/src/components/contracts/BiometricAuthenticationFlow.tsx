...
const BiometricAuthenticationFlow: React.FC<
  BiometricAuthenticationFlowProps
> = ({ open, onClose, contractId, onSuccess, onError }) => {
  ...
  const [faceCaptureData, setFaceCaptureData] = useState<{
    front: string;
    side: string;
  } | null>(null);
  ...
  const [combinedImageData, setCombinedImageData] = useState<string | null>(
    null,
  );
  ...
  const steps: AuthenticationStep[] = [
    {
      ...
      icon: <CameraAlt />,
      completed:
        (biometricData.completedSteps?.face_front &&
          biometricData.completedSteps?.face_side) ||
        false,
    },
    {
      id: 1,
  ...
  useEffect(() => {
    if (biometricData.expiresAt) {
      ...
      const updateTimer = () => {
        ...
        if (remaining === 0) {
          setError(
            'La sesión de autenticación ha expirado. Inicie el proceso nuevamente.',
          );
        }
      };
  ...
  const initializeBiometricAuth = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err.response?.data?.error || 'Error iniciando autenticación biométrica';
  ...
  const handleFaceCapture = useCallback(
    async (frontImage: string, sideImage: string) => {
      try {
        setLoading(true);
        setError(null);

        const response = await contractService.processFaceCapture(
          contractId,
          frontImage,
          sideImage,
        );
        ...
        const errorMessage =
          err.response?.data?.error || 'Error procesando captura facial';
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const handleDocumentVerification = useCallback(
    async (
      ...
      try {
        ...
        const response = await contractService.processDocumentVerification(
          contractId,
          documentImage,
          documentType,
          documentNumber,
        ...
        const errorMessage =
          err.response?.data?.error || 'Error procesando documento';
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const handleCombinedVerification = useCallback(
    async (combinedImage: string) => {
      try {
        setLoading(true);
        setError(null);

        const response = await contractService.processCombinedVerification(
          contractId,
          combinedImage,
        );
        ...
        const errorMessage =
          ...
          'Error procesando verificación combinada';
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const handleVoiceRecording = useCallback(
    async (voiceRecording: string) => {
      try {
        setLoading(true);
        setError(null);

        const response = await contractService.processVoiceVerification(
          contractId,
          voiceRecording,
        ...
        const errorMessage =
          err.response?.data?.error || 'Error procesando grabación de voz';
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
  ...
  const handleCompleteAuthentication = useCallback(async () => {
    try {
      ...
      const errorMessage =
        err.response?.data?.error || 'Error completando autenticación';
      setError(errorMessage);
    } finally {
      setLoading(false);
    }
  ...
  const handleDigitalSignature = useCallback(
    async (signatureImage: string) => {
      try {
        ...
        const errorMessage =
          err.response?.data?.error || 'Error procesando firma digital';
        setError(errorMessage);
      } finally {
        setLoading(false);
      }
    ...
    >
      ...
      >
        ...
        {biometricData.confidenceScores &&
          Object.keys(biometricData.confidenceScores).length > 0 && (
            <Box sx={{ px: 2, pb: 2 }}>
                ...
                >
                  {(biometricData.confidenceScores.face_confidence ?? 0) >
                    ...
                    <Box textAlign='center'>
                      ...
                      <Typography variant='h6' color='success.main'>
                        {Math.round(
                          (biometricData.confidenceScores.face_confidence ??
                            0) * 100,
                        )}
                        %
                      </Typography>
                  ...
                  {(biometricData.confidenceScores.voice_confidence ?? 0) >
                    ...
                    <Box textAlign='center'>
                      ...
                      <Typography variant='h6' color='success.main'>
                        {Math.round(
                          (biometricData.confidenceScores.voice_confidence ??
                            0) * 100,
                        )}
                        %
                      </Typography>
                  ...
                  {(biometricData.confidenceScores.overall_confidence ?? 0) >
                    ...
                    <Box textAlign='center'>
                      ...
                      <Typography variant='h6' color='success.main'>
                        {Math.round(
                          (biometricData.confidenceScores.overall_confidence ??
                            0) * 100,
                        )}
                        %
                      </Typography>
