...
const DropzoneBlock: React.FC<DropzoneBlockProps> = ({
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop: (acceptedFiles, rejectedFiles) =>
      onDrop(acceptedFiles, rejectedFiles, docType),
    accept: docType.acceptedTypes.reduce(
      (acc, type) => ({ ...acc, [type]: [] }),
      {},
    ),
    maxFiles: 1,
  ...
  codeudor_salario: [
    {
      ...
      name: 'Carta Laboral',
      description:
        'Certificación laboral actualizada con menos de 30 días de expedición',
      icon: <Work />,
      category: 'required',
    ...
    },
    {
      ...
      description:
        'Autorización para consulta en centrales de riesgo crediticio',
      icon: <Security />,
      category: 'required',
      acceptedTypes: ['application/pdf', 'image/jpeg', 'image/png'],
      maxSize: 3,
...
const GuaranteeDocumentUpload: React.FC<GuaranteeDocumentUploadProps> = ({
  ...
  const [documents, setDocuments] =
    useState<GuaranteeDocument[]>(existingDocuments);
  const [previewDocument, setPreviewDocument] =
    useState<GuaranteeDocument | null>(null);
  ...
  const completionPercentage =
    requiredDocs.length > 0
      ? (requiredUploaded / requiredDocs.length) * 100
      : 0;

  const onDrop = useCallback(
    (acceptedFiles: File[], rejectedFiles: any[], docType: DocumentType) => {
      setUploadError('');
      ...
      acceptedFiles.forEach(file => {
        ...
        if (file.size > docType.maxSize * 1024 * 1024) {
          setUploadError(
            `El archivo ${file.name} excede el tamaño máximo de ${docType.maxSize}MB`,
          );
          return;
        }
  ...
  const updateDocumentStatus = (
    documentId: string,
    status: GuaranteeDocument['status'],
    progress?: number,
    error?: string,
  ) => {
    setDocuments(prev => {
      const updated = prev.map(doc =>
        doc.id === documentId ? { ...doc, status, progress, error } : doc,
      );
  ...
  return (
    <Card sx={{ mt: 3 }}>
      <CardHeader
        title={
          <Box display='flex' alignItems='center' gap={1}>
            ...
            <Typography variant='h6'>Documentos de Garantía</Typography>
          </Box>
        }
        subheader={
          <Box>
          ...
          >
            <Typography variant='body2'>
              {completionPercentage === 100
                ...
                : ` Faltan ${requiredDocs.length - requiredUploaded} documentos requeridos por subir`}
            </Typography>
          </Alert>
        )}
      </CardContent>
      ...
      >
        <DialogTitle>
          Vista Previa:{' '}
          {documentTypes.find(dt => dt.id === previewDocument?.type)?.name}
        </DialogTitle>
        <DialogContent>
          {previewDocument &&
