    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if email_address.verified:
                self.stdout.write(
                    self.style.WARNING("[ADVERTENCIA] Este email ya está verificado")
                )
                return
            ...
            try:
                ...
                confirmation = (
                    EmailConfirmation.objects.filter(email_address__user=user)
                    .order_by("-created")
                    .first()
                )
                ...
                if confirmation:
                    ...
                    self.stdout.write(
                        f"- URL: http://localhost:3000/confirm-email/{confirmation.key}"
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                ...
                self.stdout.write(
                    self.style.ERROR(f"[ERROR] Error enviando email: {str(e)}")
                )
                import traceback
            ...
            self.stdout.write(
                self.style.ERROR(f"[ERROR] Usuario no encontrado: {email}")
            )
