...
class UserSettings(models.Model):
    ...
    sms_notifications = models.BooleanField("Notificaciones SMS", default=False)
    whatsapp_notifications = models.BooleanField(
        "Notificaciones WhatsApp", default=False
    )

    # Tipos de notificaciones específicas (usando campos existentes en DB)
    message_notifications = models.BooleanField(
        "Notificar nuevos mensajes", default=True
    )
    property_alerts = models.BooleanField(
        "Notificar actualizaciones de propiedades", default=True
    )
    payment_reminders = models.BooleanField(
        "Notificar recordatorios de pago", default=True
    )
    newsletter = models.BooleanField("Notificaciones de marketing", default=False)

    # Preferencias de idioma y tema (usando campos existentes)
    language = models.CharField("Idioma", max_length=2, choices=LANGUAGES, default="es")
    theme = models.CharField("Tema", max_length=10, choices=THEMES, default="light")
    ...
    show_contact_info = models.BooleanField(
        "Mostrar información de contacto", default=False
    )
    show_property_history = models.BooleanField(
        "Mostrar historial de propiedades", default=True
    )
    ...
    date_format = models.CharField(
        "Formato de fecha", max_length=20, default="DD/MM/YYYY"
    )
    two_factor_enabled = models.BooleanField(
        "Autenticación de dos factores", default=False
    )
    login_notifications = models.BooleanField(
        "Notificaciones de inicio de sesión", default=True
    )
    session_timeout = models.PositiveIntegerField(
        "Tiempo de sesión (minutos)", default=30
    )
