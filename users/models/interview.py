...
class InterviewCode(models.Model):
    ...
    code = models.CharField("Código", max_length=10, unique=True, db_index=True)
    user_type = models.CharField(
        "Tipo de usuario", max_length=20, choices=User.USER_TYPES
    )
    ...
    def validate_code(cls, code, email=None):
        ...
        if email and interview_code.email:
            if interview_code.email.lower() != email.lower():
                raise ValidationError("Este código está asociado a un email diferente")
...
class ContactRequest(models.Model):
    ...
    request_type = models.CharField(
        "Tipo de solicitud", max_length=20, choices=REQUEST_TYPES
    )
    ...
    interview_code_sent = models.ForeignKey(
        InterviewCode, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class InterviewSession(models.Model):
    ...
    interview_code = models.ForeignKey(
        InterviewCode, on_delete=models.CASCADE, related_name="sessions"
    )
