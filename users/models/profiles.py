...
class BaseProfile(models.Model):
    ...
    profile_image = models.ImageField(
        "Imagen de perfil", upload_to="profiles/", null=True, blank=True
    )
...
class TenantProfile(BaseProfile):
    ...
    employment_status = models.CharField(
        "Estado laboral", max_length=20, choices=EMPLOYMENT_STATUS, blank=True
    )
    ...
    years_employed = models.PositiveIntegerField(
        "Años de empleo", null=True, blank=True
    )

    # Documentos específicos
    income_proof = models.FileField(
        "Comprobante de ingresos", upload_to="documents/income/", null=True, blank=True
    )
    employment_letter = models.FileField(
        "Carta laboral", upload_to="documents/employment/", null=True, blank=True
    )
    bank_statements = models.FileField(
        "Extractos bancarios", upload_to="documents/bank/", null=True, blank=True
    )

    # Información adicional
    has_pets = models.BooleanField("Tiene mascotas", default=False)
    pets_description = models.TextField(
        "Descripción de mascotas", max_length=500, blank=True
    )
    ...
    emergency_contact_name = models.CharField(
        "Nombre contacto de emergencia", max_length=200, blank=True
    )
    emergency_contact_phone = models.CharField(
        "Teléfono contacto de emergencia", max_length=15, blank=True
    )

    # Preferencias
    preferred_property_types = models.JSONField(
        "Tipos de propiedad preferidos", default=list
    )
    preferred_locations = models.JSONField("Ubicaciones preferidas", default=list)
...
class ServiceProviderProfile(BaseProfile):
    ...
    service_types = models.JSONField(
        "Tipos de servicios", default=list, help_text="Tipos de servicios que ofrece"
    )
    years_experience = models.PositiveIntegerField("Años de experiencia", default=0)
    hourly_rate = models.DecimalField(
        "Tarifa por hora", max_digits=8, decimal_places=2, null=True, blank=True
    )
    ...
    professional_license = models.FileField(
        "Licencia profesional", upload_to="documents/licenses/", null=True, blank=True
    )
    insurance_certificate = models.FileField(
        "Certificado de seguro", upload_to="documents/insurance/", null=True, blank=True
    )
    ...
    accepts_emergency_calls = models.BooleanField(
        "Acepta llamadas de emergencia", default=False
    )
