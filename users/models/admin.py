...
class AdminActionLog(models.Model):
    ...
    object_repr = models.CharField(
        "Representación del objeto", max_length=255, blank=True
    )
    ...
    def __str__(self):
        return (
            f"{self.admin_user} - {self.get_action_type_display()} - {self.timestamp}"
        )
...
class UserActionNotification(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="action_notifications"
    )
    notification_type = models.CharField(
        "Tipo de notificación", max_length=20, choices=NOTIFICATION_TYPES
    )
...
class AdminSessionSummary(models.Model):
    ...
    admin_user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="session_summaries"
    )
    ...
    impersonation_time = models.DurationField(
        "Tiempo de suplantación", default=timezone.timedelta
    )
    unique_users_affected = models.PositiveIntegerField(
        "Usuarios únicos afectados", default=0
    )
