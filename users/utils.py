...
def create_interview_code(
    ...
    if created_by is None:
        ...
        if created_by is None:
            raise ValueError(
                "No superuser found to assign as created_by. Please provide a created_by user."
            )
...
def send_interview_code_email(interview_code, admin_name="Administrador"):
    ...
    context = {
        ...
        "site_name": "VeriHome",
        "registration_url": f"{settings.SITE_URL}/register"
        if hasattr(settings, "SITE_URL")
        else "/register",
    }

    # Renderizar templates
    html_message = render_to_string("users/email/interview_code.html", context)
    plain_message = render_to_string("users/email/interview_code.txt", context)
...
def send_interview_notification_to_admin(candidate_email, candidate_name, admin_email):
    ...
    html_message = render_to_string(
        "users/email/interview_notification_admin.html", context
    )
    plain_message = render_to_string(
        "users/email/interview_notification_admin.txt", context
    )
...
def validate_interview_code(code, email):
    ...
    try:
        interview_code = InterviewCode.objects.get(
            interview_code=code, candidate_email=email, status="active"
        )
...
def get_interview_statistics():
    ...
    avg_rating = (
        InterviewCode.objects.filter(status="used", initial_rating__gt=0).aggregate(
            avg_rating=models.Avg("initial_rating")
        )["avg_rating"]
        or 0
    )
    ...
    return {
        ...
        "usage_rate": round(
            (used_codes / total_codes * 100) if total_codes > 0 else 0, 1
        ),
    }
...
def get_user_verification_status(user):
    ...
    status = {
        ...
        "interview_rating": user.interview_code.initial_rating
        if user.interview_code
        else 0,
