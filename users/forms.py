    ...
    def clean_interview_code(self):
        ...
        try:
            ...
            raise ValidationError(
                "Código de entrevista inválido o no coincide con el correo electrónico."
            )
...
class InterviewCodeVerificationForm(forms.Form):
    ...
    interview_code = forms.CharField(
        label="Código de entrevista", max_length=8, required=True
    )
    ...
    def clean(self):
        ...
        if code and email:
            try:
                ...
                if interview_code.status == "used":
                    raise ValidationError(
                        "Este código de entrevista ya ha sido utilizado."
                    )
    ...
    def clean(self):
        ...
        if graduation_year:
            ...
            if graduation_year < 1950:
                raise forms.ValidationError(
                    "El año de graduación debe ser posterior a 1950."
                )
        ...
        if credit_score and (credit_score < 300 or credit_score > 850):
            raise forms.ValidationError(
                "La puntuación crediticia debe estar entre 300 y 850."
            )
