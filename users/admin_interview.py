...
class InterviewCodeAdmin(admin.ModelAdmin):
    list_display = ["code", "user_type", "email", "is_active", "is_used", "created_at"]
    list_filter = ["user_type", "is_active", "is_used", "created_at", "valid_until"]
    search_fields = ["code", "email", "created_by__username"]
    readonly_fields = ["code", "created_at", "used_at", "current_uses"]
    list_per_page = 25
    ordering = ["-created_at"]
    ...
    fieldsets = (
        (
            ...
            {"fields": ("code", "user_type", "is_active", "current_uses", "max_uses")},
        ),
        ("Información del Usuario", {"fields": ("email", "notes")}),
        (
            "Fechas Importantes",
            {"fields": ("valid_from", "valid_until", "created_at", "used_at")},
        ),
        ("Gestión", {"fields": ("created_by", "is_used")}),
    )
    ...
    def rating_display(self, obj):
        if obj.interview_rating:
            ...
            color = (
                "green"
                if obj.interview_rating >= 7
                else "orange"
                if obj.interview_rating >= 5
                else "red"
            )
            return format_html(
        ...
        if "apply" in request.POST:
            ...
            if user_type and rating:
                try:
                    ...
                    self.message_user(
                        request, f"{queryset.count()} códigos aprobados exitosamente."
                    )
                    return
                except ValueError:
                    self.message_user(
                        request,
                        "Error: Rating debe ser un número válido.",
                        level="ERROR",
                    )
        ...
        for code in queryset:
            writer.writerow(
                [
                    ...
                    code.get_approved_user_type_display()
                    if code.approved_user_type
                    else "",
    ...
    def extend_expiry(self, request, queryset):
        ...
        self.message_user(
            request, f"Expiración extendida para {queryset.count()} códigos."
        )
