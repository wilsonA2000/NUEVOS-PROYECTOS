    ...
    def get_admin_actions_on_user(user, days=30):
        ...
        return (
            AdminActionLog.objects.filter(
                impersonation_session__impersonated_user=user, timestamp__gte=start_date
            )
            .select_related("impersonation_session__admin_user")
            .order_by("-timestamp")
        )
...
class AdminSessionService:
    ...
    def get_admin_session_stats(admin_user, days=30):
        ...
        sessions = AdminImpersonationSession.objects.filter(
            admin_user=admin_user, started_at__gte=start_date
        )
        ...
        # Acciones por sesión
        total_actions = sum(s.actions.count() for s in sessions)
    ...
    def get_impersonation_history(user):
        ...
        return (
            AdminImpersonationSession.objects.filter(impersonated_user=user)
