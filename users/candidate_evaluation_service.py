...
class CandidateEvaluationService:
    ...
    THRESHOLDS = {
        ...
        "credit_score": {"excellent": 750, "good": 700, "acceptable": 650, "poor": 600},
        "employment_months": {
            "excellent": 24,  # 2+ años
            "good": 12,  # 1-2 años
    ...
    def __init__(
        ...
        logger.info(
            f"CandidateEvaluationService iniciado - match_request: {match_request}, property_interest_request: {property_interest_request}"
        )
    ...
    def get_green_flags(self) -> List[Dict]:
        ...
        if not self.resume.eviction_history and not getattr(
            self.resume, "criminal_records", False
        ):
            green_flags.append(
                {
                    "type": "good",
    ...
    def get_full_evaluation(self) -> Dict:
        ...
        return {
            ...
            "verihome_rating": {
                ...
                "verification_score": self.resume.verification_score
                if self.resume
                else None,
                "verified_by": self.resume.verified_by.get_full_name()
                if self.resume and self.resume.verified_by
                else None,
                "verified_at": self.resume.verified_at.isoformat()
                if self.resume and self.resume.verified_at
                else None,
                "is_verified": getattr(self.candidate, "is_verified", False),
            },
            "financial_metrics": {
                "monthly_income": float(self.resume.monthly_salary)
                if self.resume and self.resume.monthly_salary
                else None,
                "income_ratio": round(income_ratio, 2) if income_ratio else None,
                "credit_score_verified": self.resume.verification_score
                if self.resume
                else None,  # Score oficial de VeriHome
                "credit_score_reported": self.resume.credit_score
                if self.resume
                else None,  # Score auto-reportado
    ...
    def _get_match_request_info(self) -> Dict:
        ...
        if self.match_request:
            return {
                ...
                "preferred_move_in_date": self.match_request.preferred_move_in_date.isoformat()
                if self.match_request.preferred_move_in_date
                else None,
                "lease_duration_months": self.match_request.lease_duration_months,
                "monthly_income": float(self.match_request.monthly_income)
                if self.match_request.monthly_income
                else None,
                ...
                "created_at": self.match_request.created_at.isoformat()
                if hasattr(self.match_request, "created_at")
                else None,
                "updated_at": self.match_request.updated_at.isoformat()
                if hasattr(self.match_request, "updated_at")
                else None,
            }
        ...
        # Usar PropertyInterestRequest como alternativa
            ...
            try:
                return {
                    ...
                    "status": getattr(
                        self.property_interest_request, "status", "pending"
                    ),
                    "status_display": self.property_interest_request.get_status_display()
                    if hasattr(self.property_interest_request, "get_status_display")
                    else "Pendiente",
                    "priority": "medium",  # Default priority
                    "priority_display": "Medium",
                    "tenant_message": getattr(
                        self.property_interest_request, "description", ""
                    ),
                    "tenant_phone": getattr(self.candidate, "phone", ""),
                    "tenant_email": self.candidate.email,
                    "preferred_move_in_date": self.property_interest_request.preferred_move_in_date.isoformat()
                    if hasattr(self.property_interest_request, "preferred_move_in_date")
                    and self.property_interest_request.preferred_move_in_date
                    else None,
                    "lease_duration_months": getattr(
                        self.property_interest_request, "lease_duration_months", 12
                    ),
                    "monthly_income": float(
                        self.property_interest_request.monthly_income
                    )
                    if hasattr(self.property_interest_request, "monthly_income")
                    and self.property_interest_request.monthly_income
                    else None,
                    "employment_type": getattr(
                        self.property_interest_request, "employment_type", ""
                    ),
                    "number_of_occupants": getattr(
                        self.property_interest_request, "number_of_occupants", 1
                    ),
                    "has_pets": getattr(
                        self.property_interest_request, "has_pets", False
                    ),
                    "pet_details": getattr(
                        self.property_interest_request, "pet_details", ""
                    ),
                    "smoking_allowed": getattr(
                        self.property_interest_request, "smoking_allowed", False
                    ),
                    "has_rental_references": getattr(
                        self.property_interest_request, "has_rental_references", False
                    ),
                    "has_employment_proof": getattr(
                        self.property_interest_request, "has_employment_proof", False
                    ),
                    "has_credit_check": getattr(
                        self.property_interest_request, "has_credit_check", False
                    ),
                    "created_at": self.property_interest_request.created_at.isoformat()
                    if hasattr(self.property_interest_request, "created_at")
                    else None,
                    "updated_at": self.property_interest_request.updated_at.isoformat()
                    if hasattr(self.property_interest_request, "updated_at")
                    else None,
                }
