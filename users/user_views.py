...
def user_activity_history(request):
    ...
    activity_types = (
        UserActivityLog.objects.filter(user=request.user)
...
def activity_detail(request, activity_id):
    ...
    try:
        activity = UserActivityLog.objects.get(id=activity_id, user=request.user)
...
def admin_action_detail(request, action_id):
    ...
    try:
        admin_action = AdminActionLog.objects.get(
            id=action_id, impersonation_session__impersonated_user=request.user
        )
...
def api_mark_notification_read(request, notification_id):
    ...
    try:
        ...
        return Response(
            {"success": False, "error": "Notificación no encontrada"}, status=404
        )
