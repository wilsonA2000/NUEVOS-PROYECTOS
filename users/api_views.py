...
from .serializers import (
    ...
    InterviewCodeVerificationSerializer,
...
class RequestVerificationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if user.is_verified:
            return Response(
                {"error": "Usuario ya verificado"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ServiceProviderSearchAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        query = getattr(self.request, "query_params", self.request.GET).get("q", "")
        return (
            User.objects.filter(user_type="service_provider", email__icontains=query)
            | User.objects.filter(
                user_type="service_provider", first_name__icontains=query
            )
            | User.objects.filter(
                user_type="service_provider", last_name__icontains=query
            )
        )
        ...
        try:
            ...
            if user.user_type == "tenant":
                    ...
                    profile_data.update(
                        {
                            ...
                            "rental_history": getattr(
                                tenant_profile, "rental_history", []
                            ),
                            "emergency_contact": getattr(
                                tenant_profile, "emergency_contact", None
                            ),
                        }
                    )
                    ...
                    profile_data.update(
                        {
                            ...
                            "properties_count": landlord_profile.properties_count
                            if hasattr(landlord_profile, "properties_count")
                            else 0,
                            "years_experience": landlord_profile.years_experience
                            if hasattr(landlord_profile, "years_experience")
                            else 0,
                            "rating": landlord_profile.rating
                            if hasattr(landlord_profile, "rating")
                            else 0,
                        }
                    )
                ...
                try:
                    ...
                    profile_data.update(
                        {
                            "services": service_profile.services
                            if hasattr(service_profile, "services")
                            else [],
                            "experience_years": service_profile.experience_years
                            if hasattr(service_profile, "experience_years")
                            else 0,
                            "rating": service_profile.rating
                            if hasattr(service_profile, "rating")
                            else 0,
                            "hourly_rate": service_profile.hourly_rate
                            if hasattr(service_profile, "hourly_rate")
                            else None,
                        }
                    )
            ...
            try:
                ...
                profile_data["resume"] = {
                    ...
                    "monthly_salary": float(user_resume.monthly_salary)
                    if user_resume.monthly_salary
                    else None,
                    "credit_score": user_resume.credit_score,
                    "eviction_history": user_resume.eviction_history,
                    "criminal_records": user_resume.criminal_records
                    if hasattr(user_resume, "criminal_records")
                    else False,
        ...
        try:
            ...
            try:
                ...
                resume_data["personal_info"] = {
                    "date_of_birth": user_resume.date_of_birth.isoformat()
                    if user_resume.date_of_birth
                    else None,
                ...
                resume_data["employment"] = {
                    ...
                    "start_date": user_resume.start_date.isoformat()
                    if user_resume.start_date
                    else None,
                    "end_date": user_resume.end_date.isoformat()
                    if user_resume.end_date
                    else None,
                    "monthly_salary": float(user_resume.monthly_salary)
                    if user_resume.monthly_salary
                    else None,
                ...
                resume_data["financial"] = {
                    "monthly_expenses": float(user_resume.monthly_expenses)
                    if user_resume.monthly_expenses
                    else None,
                    "credit_score": user_resume.credit_score,
                ...
                resume_data["housing_history"] = {
                    ...
                    "eviction_history": user_resume.eviction_history,
                    "eviction_details": user_resume.eviction_details
                    if user_resume.eviction_history
                    else None,
                }
                ...
                resume_data["additional_info"] = {
                    ...
                    "criminal_record_details": user_resume.criminal_record_details
                    if user_resume.criminal_record
                    else None,
                }
                ...
                resume_data["verification"] = {
                    ...
                    "verified_at": user_resume.verified_at.isoformat()
                    if user_resume.verified_at
                    else None,
                    "verified_by": user_resume.verified_by.get_full_name()
                    if user_resume.verified_by
                    else None,
                }
        ...
        if serializer.is_valid():
            ...
            try:
                interview_code = InterviewCode.objects.get(
                    interview_code=code, candidate_email=email
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            if interview_code_str and interview_code_str.strip():
                try:
                    interview_code_obj = InterviewCode.objects.get(
                        code=interview_code_str
                    )
            ...
            with transaction.atomic():
                ...
                if interview_code_obj:
                    ...
                    user.initial_rating = (
                        interview_code_obj.interview_rating
                        if interview_code_obj.interview_rating is not None
                        else 0
                    )
                    user.save()
...
def create_interview_code(request):
    ...
    try:
        ...
        interview_code = InterviewCode.objects.create(
            email=email, initial_rating=initial_rating, notes=notes
        )
        ...
        return Response(
            {"error": "Datos JSON inválidos"}, status=status.HTTP_400_BAD_REQUEST
        )
    except Exception as e:
        return Response(
...
class EmailConfirmationView(APIView):
    ...
    def post(self, request, key):
        ...
        try:
            ...
            return Response(
                {"detail": "Email confirmado exitosamente.", "email": user.email},
...
class ActivityTypesAPIView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"activity_types": activity_types, "total_types": len(activity_types)},
            status=status.HTTP_200_OK,
        )
...
class CandidateEvaluationView(APIView):
    ...
    def get(self, request, user_id):
        ...
        try:
            ...
            if match_request_id:
                try:
                    ...
                    match_request = MatchRequest.objects.get(
                        id=match_request_id, landlord=request.user, tenant=candidate
                    )
                    ...
                    try:
                        ...
                        logger = logging.getLogger(__name__)
                        logger.info(
                            f"Buscando PropertyInterestRequest con ID: {match_request_id}"
                        )
                        ...
                        logger.info(
                            f"PropertyInterestRequest encontrado: {property_request}"
                        )
                        ...
                        logger.error(
                            f"Error inesperado buscando PropertyInterestRequest: {e}"
                        )
                        return Response(
                ...
                try:
                    ...
                    property_obj = Property.objects.get(
                        id=property_id, landlord=request.user
                    )
            ...
            evaluation_service = CandidateEvaluationService(
                candidate, property_obj, match_request, property_interest_request
            )
            ...
            if property_obj:
                comparison = evaluation_service.compare_with_other_candidates(
                    property_id
                )
    ...
    def _get_public_profile_data(self, candidate):
        ...
        profile_data = {
            "basic_info": {
                ...
                "date_joined": candidate.date_joined.isoformat()
                if candidate.date_joined
                else None,
                "is_verified": EmailAddress.objects.filter(
                    user=candidate, verified=True
                ).exists(),
            }
        }
    ...
    def post(self, request, user_id):
        ...
        try:
            ...
            try:
                match_request = MatchRequest.objects.get(
                    id=match_request_id, landlord=request.user, tenant_id=user_id
                )
            except MatchRequest.DoesNotExist:
                # Si no se encuentra MatchRequest, buscar en PropertyInterestRequest
                try:
                    property_request = PropertyInterestRequest.objects.get(
                        id=match_request_id, assignee=request.user, requester_id=user_id
                    )
                except PropertyInterestRequest.DoesNotExist:
                    return Response(
...
class ImpersonationSessionsAPIView(APIView):
    ...
    def get(self, request):
        ...
        qs = AdminImpersonationSession.objects.select_related(
        ...
        data = [
            {
                ...
                "impersonating": s.impersonated_user.email
...
class StartImpersonationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not target_id:
            return Response(
                {"error": "user_id requerido"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            target = User.objects.get(id=target_id)
        except User.DoesNotExist:
            return Response(
                {"error": "Usuario objetivo no existe"},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        return Response(
            {"session_id": str(session.id), "impersonating": target.email},
            status=status.HTTP_201_CREATED,
        )
...
class StopImpersonationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if not session:
            return Response(
                {"detail": "No hay sesión activa"}, status=status.HTTP_404_NOT_FOUND
            )
