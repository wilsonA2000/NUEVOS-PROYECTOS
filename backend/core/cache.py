...
class CacheManager:
    ...
    def set(
        self, key: str, value: Any, timeout: int = None, cache_type: str = "default"
    ) -> bool:
...
class SmartCache:
    ...
    @staticmethod
    def cache_properties_list(
        properties_data: List[Dict], filters: Dict = None, user_id: int = None
    ):
        ...
        for prop in properties_data:
            ...
            cache_manager.set(
                prop_key, prop, CACHE_TIMEOUTS.get("property_detail", 600)
            )
    ...
    def get_cached_user_data(
        user_id: int, data_type: str = "profile"
    ) -> Optional[Dict]:
        ...
        for cache_name in ["default", "sessions", "query_cache"]:
            try:
                ...
                backend_info = {
                    ...
                    "location": getattr(cache_backend, "_cache", {}).get(
                        "location", "N/A"
                    ),
                }
...
def warm_up_cache():
    ...
    try:
        ...
        for prop in featured_properties:
            ...
            cache_manager.set(
                cache_key, prop_data, CACHE_TIMEOUTS.get("property_detail", 600)
            )
        ...
        logger.info(
            f"Cache precalentado con {len(properties_data)} propiedades destacadas"
        )
