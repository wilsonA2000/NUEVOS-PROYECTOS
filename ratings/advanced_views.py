...
class AdvancedRatingViewSet(viewsets.ModelViewSet):
    ...
    def bulk_invite(self, request):
        ...
        try:
            ...
            ).exclude(ratings__reviewer=request.user)
            ...
            for contract in completed_contracts:
                ...
                try:
                    success = notification_manager.send_rating_invitation(
                        contract=contract, inviter=request.user, invitee=other_party
                    )
                    if success:
                        invitations_sent += 1
                except Exception as e:
                    errors.append(
                        f"Error inviting {other_party.get_full_name()}: {str(e)}"
                    )
...
class RatingAnalyticsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            analytics_type = getattr(request, "query_params", request.GET).get(
                "type", "global"
            )
            date_range = int(
                getattr(request, "query_params", request.GET).get("days", 30)
            )
            user_id = getattr(request, "query_params", request.GET).get("user_id")
            property_id = getattr(request, "query_params", request.GET).get(
                "property_id"
            )
...
class RatingModerationAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            flagged_ratings = Rating.objects.filter(is_flagged=True).order_by(
                "-created_at"
            )

            # Reportes pendientes
            pending_reports = RatingReport.objects.filter(status="pending").order_by(
                "-created_at"
            )
            ...
            data = {
                ...
                "pending_reports": RatingReportSerializer(
                    pending_reports, many=True
                ).data,
    ...
    def post(self, request):
        ...
        try:
            ...
            if action == "approve_rating" and rating_id:
                ...
                profile, created = UserRatingProfile.objects.get_or_create(
    ...
    def get(self, request):
        ...
        try:
            ...
            received_invitations = RatingInvitation.objects.filter(
                invitee=request.user, status__in=["pending", "sent"]
            ).order_by("-created_at")
                ...
                "received": [
                    {
                        ...
                        "property_title": inv.contract.property.title
                        if inv.contract.property
                        else "Servicio",
                ...
                "sent": [
                    {
                        ...
                        "expires_at": inv.expires_at,
                        "status": inv.status,
                        "resulting_rating_id": inv.resulting_rating.id
                        if inv.resulting_rating
                        else None,
                    }
    ...
    def post(self, request):
        ...
        try:
            ...
            existing_rating = Rating.objects.filter(
                reviewer=request.user, reviewee=invitee, contract=contract
            ).first()
            ...
            notification_manager = RatingNotificationManager()
            success = notification_manager.send_rating_invitation(
                contract=contract, inviter=request.user, invitee=invitee
            )
    ...
    def get(self, request, token):
        ...
        try:
            invitation = get_object_or_404(
                RatingInvitation, invitation_token=token, invitee=request.user
            )
            ...
            data = {
                ...
                "contract": {
                    ...
                    "property_title": invitation.contract.property.title
                    if invitation.contract.property
                    else "Servicio",
        ...
        try:
            ...
            if serializer.is_valid():
                rating = serializer.save(
                    ...
                    property=invitation.contract.property
                    if invitation.contract.property
                    else None,
                ...
                return Response(
                    RatingDetailSerializer(rating).data, status=status.HTTP_201_CREATED
                )
            else:
...
class RatingStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            total_given = Rating.objects.filter(reviewer=user, is_active=True).count()
            ...
            pending_invitations = RatingInvitation.objects.filter(
                invitee=user, status__in=["pending", "sent"]
            ).count()
