...
def rating_dashboard(request):
    ...
    ratings_received = Rating.objects.filter(
        reviewee=request.user, is_active=True, moderation_status="approved"
    ).order_by("-created_at")

    # Obtener calificaciones dadas
    ratings_given = Rating.objects.filter(
        reviewer=request.user, is_active=True
    ).order_by("-created_at")
    ...
    context = {
        ...
        "average_rating": ratings_received.aggregate(Avg("overall_rating"))[
            "overall_rating__avg"
        ],
    }
...
def user_ratings(request, user_id):
    ...
    ratings = Rating.objects.filter(
        reviewee=user, is_active=True, is_public=True, moderation_status="approved"
    ).order_by("-created_at")
...
def create_rating(request, contract_id):
    ...
    existing_rating = Rating.objects.filter(
        reviewer=request.user, reviewee=reviewee, contract=contract
    ).first()
            ...
            for category, score in form.cleaned_data.items():
                if category.startswith("category_") and score:
                    ...
                    RatingCategory.objects.create(
                        rating=rating, category=category_name, score=score
                    )

            # Actualizar perfil de calificaciones del usuario calificado
            rating_profile, created = UserRatingProfile.objects.get_or_create(
                user=reviewee
            )
...
def respond_to_rating(request, rating_id):
    ...
    if request.user != rating.reviewee:
        messages.error(
            request, "No puedes responder a una calificación que no es tuya."
        )
        return redirect("ratings:dashboard")
...
def report_rating(request, rating_id):
    ...
    existing_report = RatingReport.objects.filter(
        rating=rating, reporter=request.user
    ).first()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Reporte enviado correctamente. Un moderador revisará la calificación.",
            )
            return redirect("ratings:rating_detail", rating_id=rating.id)
    else:
        ...
        if contract_id:
            ...
            if self.request.user not in [
                ...
                raise permissions.PermissionDenied(
                    "No tienes permiso para calificar este contrato."
                )
            ...
            rating = serializer.save(
                reviewer=self.request.user, reviewee=reviewee, rating_type="general"
            )
        else:
        ...
        if hasattr(request, "impersonation_session"):
            ...
            log_activity(
                ...
                details={
                    ...
                    "service_order_id": str(rating.service_order_id)
                    if rating.service_order_id
                    else None,
...
class RatingDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        ...
        if (
            user.is_staff
            or user == obj.reviewer
            or (user == obj.reviewee and obj.is_public)
        ):
            return obj

        raise permissions.PermissionDenied(
            "No tienes permiso para acceder a esta calificación."
        )
    ...
    def perform_update(self, serializer):
        ...
        if self.request.user != self.get_object().reviewer:
            raise permissions.PermissionDenied(
                "No puedes modificar una calificación que no es tuya."
            )
    ...
    def perform_destroy(self, instance):
        ...
        if self.request.user != instance.reviewer and not self.request.user.is_staff:
            raise permissions.PermissionDenied(
                "No puedes eliminar una calificación que no es tuya."
            )
...
class RatingResponseCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user != rating.reviewee:
            raise permissions.PermissionDenied(
                "No puedes responder a una calificación que no es tuya."
            )
...
class RatingReportCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        existing_report = RatingReport.objects.filter(
            rating=rating, reporter=self.request.user
        ).first()
...
class ContractRatingsView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user not in [contract.primary_party, contract.secondary_party]:
            raise permissions.PermissionDenied(
                "No tienes permiso para ver estas calificaciones."
            )
...
class RatingCategoryListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        rating_id = getattr(self.request, "query_params", self.request.GET).get(
            "rating_id"
        )
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user != rating.reviewer:
            raise permissions.PermissionDenied(
                "No puedes añadir categorías a una calificación que no es tuya."
            )
