...
def update_rating_profile_on_rating_save(sender, instance, created, **kwargs):
    ...
    try:
        ...
        profile, profile_created = UserRatingProfile.objects.get_or_create(
        ...
        if instance.moderation_status == "approved" and instance.is_active:
            ...
            if created:
                ...
                _check_and_notify_milestones(
                    instance.reviewee, profile, old_avg_rating, old_total
                )

                # Enviar alerta si es una calificación baja
                if instance.overall_rating <= 4:
                    notification_manager.send_low_rating_alert(
                        instance.reviewee, instance
                    )
...
def _check_and_notify_milestones(user, profile, old_avg_rating, old_total):
    ...
    try:
        ...
        if old_total == 0 and profile.total_ratings_received == 1:
            notification_manager.send_milestone_achievement_notification(
                user, "first_rating"
            )
        ...
        if (
            old_total < 50
            and profile.total_ratings_received >= 50
        ...
        def handle_contract_completion(sender, instance, created, **kwargs):
            ...
            if not created and instance.status == "completed":
                try:
                    ...
                    existing_invitations = RatingInvitation.objects.filter(
                        contract=instance
                    )
                    ...
                    logger.error(
                        f"Error creating rating invitations for contract {instance.id}: {str(e)}"
                    )
