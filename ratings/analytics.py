...
class RatingAnalytics:
    ...
    def __init__(self, user: Optional[User] = None, date_range: Optional[int] = 30):
        ...
        self.start_date = (
            self.end_date - timedelta(days=date_range) if date_range else None
        )

    def get_global_statistics(self) -> Dict[str, Any]:
        """Obtiene estadísticas globales de calificaciones."""
        queryset = Rating.objects.filter(is_active=True, moderation_status="approved")
    ...
    def get_user_analytics(self, target_user: User) -> Dict[str, Any]:
        ...
        received_ratings = Rating.objects.filter(
            reviewee=target_user, is_active=True, moderation_status="approved"
        )

        given_ratings = Rating.objects.filter(reviewer=target_user, is_active=True)
    ...
    def get_property_analytics(self, property_id: str) -> Dict[str, Any]:
        ...
        ratings = Rating.objects.filter(
            property_id=property_id, is_active=True, moderation_status="approved"
        )
        ...
        property_condition_ratings = RatingCategory.objects.filter(
            rating__in=ratings, category="property_condition"
        )

        cleanliness_ratings = RatingCategory.objects.filter(
            rating__in=ratings, category="cleanliness"
        )
        ...
        return {
            ...
            "landlord_ratings": {
                ...
                "average": landlord_ratings.aggregate(avg=Avg("overall_rating"))["avg"]
                or 0,
            },
            "tenant_ratings": {
                "count": tenant_ratings.count(),
                "average": tenant_ratings.aggregate(avg=Avg("overall_rating"))["avg"]
                or 0,
            },
            ...
            or 0,
            "cleanliness_avg": cleanliness_ratings.aggregate(avg=Avg("score"))["avg"]
            or 0,
            ...
            trends.append(
                {
                    ...
                    "average": week_ratings.aggregate(avg=Avg("overall_rating"))["avg"]
                    or 0,
                }
            )
    ...
    def _analyze_given_ratings(self, ratings) -> Dict[str, Any]:
        ...
        return {
            ...
            "generosity_score": round(high_ratings / total * 100, 2)
            if total > 0
            else 0,
        }
    ...
    def _get_user_trends(self, user: User) -> Dict[str, List]:
        ...
        ratings = Rating.objects.filter(
            reviewee=user, is_active=True, moderation_status="approved"
        ).order_by("created_at")
            ...
            monthly_data.append(
                {
                    ...
                    "average": month_ratings.aggregate(avg=Avg("overall_rating"))["avg"]
                    or 0,
                }
            )
    ...
    def _get_peer_comparison(self, user: User) -> Dict[str, Any]:
        ...
        peer_profiles = UserRatingProfile.objects.filter(
            user__user_type=user.user_type, total_ratings_received__gte=3
        ).exclude(user=user)
    ...
    def _analyze_user_categories(self, user: User) -> Dict[str, Any]:
        ...
        if category_stats:
            ...
            weaknesses = sorted(category_stats.items(), key=lambda x: x[1]["average"])[
                :3
            ]
        else:
            strengths = []
        ...
        return {
            ...
            "strengths": [
                {"category": s[0], "name": s[1]["name"], "score": s[1]["average"]}
                for s in strengths
            ],
            "weaknesses": [
                {"category": w[0], "name": w[1]["name"], "score": w[1]["average"]}
                for w in weaknesses
            ],
        }
    ...
    def _calculate_reputation_score(self, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            total_responded = (
                Message.objects.filter(recipient=user, thread__messages__sender=user)
                .distinct()
                .count()
            )
    ...
    def _detect_rapid_raters(self) -> List[Dict]:
        ...
        recent_ratings = (
            Rating.objects.filter(created_at__gte=timezone.now() - timedelta(days=1))
            .values("reviewer")
            .annotate(count=Count("id"))
            .filter(count__gte=10)
        )  # 10+ calificaciones en un día
    ...
    def _get_recent_trend(self, ratings) -> str:
        ...
        recent = ratings.filter(created_at__gte=timezone.now() - timedelta(days=30))
...
class RatingRecommendationEngine:
    ...
    def get_users_to_rate(self, user: User, limit: int = 10) -> List[Dict]:
        ...
        completed_contracts = Contract.objects.filter(
            Q(primary_party=user) | Q(secondary_party=user), status="completed"
        ).exclude(ratings__reviewer=user)

        recommendations = []
        for contract in completed_contracts[:limit]:
            other_party = (
                contract.secondary_party
                if contract.primary_party == user
            ...
            recommendations.append(
                {
                    ...
                    "suggested_rating_type": self._suggest_rating_type(
                        user, other_party, contract
                    ),
                }
            )
    ...
    def _suggest_rating_type(self, rater: User, ratee: User, contract) -> str:
        ...
        if rater.user_type == "landlord" and ratee.user_type == "tenant":
            ...
            rater.user_type in ["tenant", "landlord"]
            and ratee.user_type == "service_provider"
        ):
            return "client_to_service_provider"
        elif rater.user_type == "service_provider" and ratee.user_type in [
            "tenant",
            "landlord",
        ]:
