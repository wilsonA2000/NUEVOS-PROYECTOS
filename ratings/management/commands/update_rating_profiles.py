...
class Command(BaseCommand):
    ...
    def _update_all_users(self, create_missing, batch_size):
        ...
        if self.verbosity > 1:
            self.stdout.write(
                f"Actualizando {existing_profiles.count()} perfiles existentes..."
            )
        ...
        for i in range(0, existing_profiles.count(), batch_size):
            ...
            for profile in batch:
                try:
                    ...
                    if self.verbosity > 2:
                        self.stdout.write(
                            f"Actualizado: {profile.user.get_full_name()}"
                        )

                except Exception as e:
                    logger.error(
                        f"Error updating profile for user {profile.user.id}: {str(e)}"
                    )

            if self.verbosity > 1:
                self.stdout.write(
                    f"Procesados {min(i + batch_size, existing_profiles.count())} perfiles..."
                )
        ...
        if create_missing:
            ...
            users_with_ratings = (
                User.objects.filter(ratings_received__isnull=False)
                .exclude(rating_profile__isnull=False)
                .distinct()
            )
            ...
            for user in users_with_ratings:
                try:
                    ...
                    if created:
                        ...
                        if self.verbosity > 2:
                            self.stdout.write(
                                f"Creado perfil para: {user.get_full_name()}"
                            )
