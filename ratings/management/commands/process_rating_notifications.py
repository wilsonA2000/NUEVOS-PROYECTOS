...
class Command(BaseCommand):
    ...
    def _process_daily_notifications(self, scheduler):
        ...
        if not self.dry_run:
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Modo de prueba: Se habrían procesado notificaciones diarias"
                )
            )
    ...
    def _process_weekly_notifications(self, scheduler):
        ...
        if not self.dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Resúmenes semanales enviados: {summaries_sent}")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    "Modo de prueba: Se habrían enviado resúmenes semanales"
                )
            )
