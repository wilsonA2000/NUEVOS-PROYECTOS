...
class Rating(models.Model):
    ...
    rating_type = models.CharField(
        "Tipo de calificación", max_length=30, choices=RATING_TYPES
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.contract and not self.can_rate():
            raise ValueError(
                "No hay relación contractual válida para esta calificación"
            )
...
class RatingCategory(models.Model):
    ...
    rating = models.ForeignKey(
        Rating, on_delete=models.CASCADE, related_name="category_ratings"
    )
    category = models.CharField(
        "Categoría", max_length=30, choices=Rating.RATING_CATEGORIES
    )
    score = models.PositiveIntegerField(
        "Puntuación", validators=[MinValueValidator(1), MaxValueValidator(10)]
    )
...
class RatingResponse(models.Model):
    ...
    rating = models.OneToOneField(
        Rating, on_delete=models.CASCADE, related_name="response"
    )
    responder = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="rating_responses"
    )
...
class RatingReport(models.Model):
    ...
    reporter = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="rating_reports_made"
    )

    # Información del reporte
    reason = models.CharField(
        "Razón del reporte", max_length=20, choices=REPORT_REASONS
    )
    description = models.TextField("Descripción detallada", max_length=1000)

    # Estado del reporte
    status = models.CharField(
        "Estado", max_length=20, choices=REPORT_STATUS, default="pending"
    )
    ...
    resolution_notes = models.TextField(
        "Notas de resolución", max_length=1000, blank=True
    )
    ...
    def __str__(self):
        return (
            f"Reporte de {self.reporter.get_full_name()} - {self.get_reason_display()}"
        )
...
class UserRatingProfile(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="rating_profile"
    )

    # Estadísticas generales
    total_ratings_received = models.PositiveIntegerField(
        "Total de calificaciones recibidas", default=0
    )
    average_rating = models.DecimalField(
        "Calificación promedio", max_digits=3, decimal_places=2, default=0.00
    )
    ...
    landlord_rating = models.DecimalField(
        "Calificación como arrendador",
        max_digits=3,
        decimal_places=2,
        null=True,
        blank=True,
    )
    tenant_rating = models.DecimalField(
    ...
    service_provider_rating = models.DecimalField(
        "Calificación como prestador de servicios",
        max_digits=3,
        decimal_places=2,
        null=True,
        blank=True,
    ...
    def update_statistics(self):
        ...
        received_ratings = Rating.objects.filter(
            reviewee=self.user, is_active=True, moderation_status="approved"
        )
        ...
        if self.total_ratings_received > 0:
            ...
            for category_code, category_name in Rating.RATING_CATEGORIES:
                ...
                if category_ratings.exists():
                    avg_score = (
                        sum(cr.score for cr in category_ratings)
                        / category_ratings.count()
                    )
...
class RatingInvitation(models.Model):
    ...
    inviter = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="rating_invitations_sent"
    )
    invitee = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="rating_invitations_received"
    )

    # Estado de la invitación
    status = models.CharField(
        "Estado", max_length=20, choices=INVITATION_STATUS, default="pending"
    )
    ...
    invitation_token = models.CharField(
        "Token de invitación", max_length=100, unique=True
    )
