...
class MatchRequestModelTests(TestCase):
    ...
    def test_can_create_contract_accepted(self):
        ...
        mr = _make_match_request(
            self.tenant, self.landlord, self.prop,
            status='accepted',
...
class MatchRequestAPITests(APITestCase):
    ...
    def test_create_match_request_success(self):
        ...
        created = MatchRequest.objects.filter(tenant=self.tenant, property=self.prop).first()
...
class MatchNotificationAPITests(APITestCase):
    ...
    def test_mark_all_notifications_read(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        unread = MatchNotification.objects.filter(user=self.landlord, is_read=False).count()
...
class MatchSpecialEndpointsAPITests(APITestCase):
    ...
    def test_preferences_post(self):
        ...
        response = self.client.post('/api/v1/matching/preferences/', data, format='json')
        self.assertIn(response.status_code, [
            status.HTTP_200_OK,
            status.HTTP_201_CREATED,
        ])
    ...
    def test_smart_matching_endpoint(self):
        ...
        response = self.client.post('/api/v1/matching/smart-matching/', data, format='json')
