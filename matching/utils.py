...
def create_match_notification(match_request, notification_type, custom_message=None):
    ...
    notification = MatchNotification.objects.create(
        ...
        metadata={
            ...
            "created_by": str(match_request.tenant.id)
            if notification_type == "match_request_received"
            else str(match_request.landlord.id),
        },
    )
...
def find_potential_matches(tenant, limit=10):
    ...
    try:
        ...
        return Property.objects.filter(is_active=True, status="available")[:limit]
    ...
    if match_request.preferred_move_in_date:
        if hasattr(property, "available_from"):
            if (
                ...
                and match_request.preferred_move_in_date >= property.available_from
            ):
                preferences_score += 7
        else:
...
def get_landlord_match_recommendations(landlord, limit=5):
    ...
    landlord_properties = landlord.properties.filter(is_active=True, status="available")
...
def find_potential_tenants_for_property(property, limit=5):
    ...
    matching_criteria = (
        MatchCriteria.objects.filter(
            Q(preferred_cities__contains=property.city)
            | Q(preferred_cities__isnull=True)
            | Q(preferred_cities=[])
        )
        .filter(Q(max_price__gte=property.rent_price) | Q(max_price__isnull=True))
        .filter(
    ...
    for criteria in matching_criteria:
        ...
        if score >= 50:  # Solo incluir matches con score decente
            ...
            if criteria.required_amenities:
                property_amenities = set(
                    property.amenities.values_list("name", flat=True)
                )
                matching_amenities = (
                    set(criteria.required_amenities) & property_amenities
                )
                if matching_amenities:
                    reasons.append(
                        f"Tiene {len(matching_amenities)} amenidades deseadas"
                    )
...
def process_expired_matches():
    ...
    expired_matches = MatchRequest.objects.filter(
        status__in=["pending", "viewed"], expires_at__lt=timezone.now()
    )
    ...
    if user.user_type == "tenant":
        ...
        if stats["total_sent"] > 0:
            stats["response_rate"] = round(
                (total_responded / stats["total_sent"]) * 100, 1
            )

        if total_responded > 0:
            stats["acceptance_rate"] = round(
                (stats["accepted"] / total_responded) * 100, 1
            )
        ...
        if responded_requests.exists():
            ...
            stats["avg_response_time"] = round(
                total_time / responded_requests.count(), 1
            )
        ...
        if stats["total_received"] > 0:
            stats["response_rate"] = round(
                (total_responded / stats["total_received"]) * 100, 1
            )
