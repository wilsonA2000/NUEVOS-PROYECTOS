...
class MatchRequestAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ["id", "match_code", "created_at", "viewed_at", "responded_at"]
    ordering = ["-created_at"]
...
class MatchCriteriaAdmin(admin.ModelAdmin):
    ...
    list_filter = ["pets_required", "smoking_required", "created_at"]
    search_fields = ["tenant__email", "tenant__first_name", "tenant__last_name"]
    readonly_fields = ["id", "created_at", "updated_at"]

    fieldsets = (
        ("Usuario", {"fields": ("id", "tenant")}),
        ("Criterios de Precio", {"fields": ("min_price", "max_price")}),
        (
            "Criterios de Propiedad",
            {"fields": ("property_types", "min_bedrooms", "min_bathrooms", "min_area")},
        ),
        (
        ...
        ("Configuración", {"fields": ("auto_apply_enabled", "notification_frequency")}),
        ("Fechas", {"fields": ("created_at", "updated_at", "last_search")}),
    )
...
class MatchNotificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Contenido", {"fields": ("title", "message", "metadata")}),
        (
            "Estado",
    ...
    def match_request_display(self, obj):
        if obj.match_request:
            return format_html(
                '<a href="{}">{}</a>',
                reverse(
                    "admin:matching_matchrequest_change", args=[obj.match_request.pk]
                ),
