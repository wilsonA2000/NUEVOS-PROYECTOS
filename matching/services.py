...
class MatchingMessagingService:
    ...
    def create_match_thread(match_request):
        ...
        ThreadParticipant.objects.create(
            thread=thread, user=match_request.landlord, is_active=True
        )

        ThreadParticipant.objects.create(
            thread=thread, user=match_request.tenant, is_active=True
        )
...
Pueden proceder a coordinar los siguientes pasos en esta conversación.""",
    ...
    def get_user_match_conversations(user):
        ...
        return MessageThread.objects.filter(
            participants=user, thread_type="inquiry", property__isnull=False
        ).distinct()
...
class MatchNotificationService:
    ...
    def create_notification(
        user, notification_type, title, message, match_request=None, metadata=None
    ):
        """
    ...
    def get_unread_notifications(user):
        ...
        """
        return MatchNotification.objects.filter(user=user, is_read=False).order_by(
            "-created_at"
        )
    ...
    def mark_all_as_read(user):
        ...
        """
        return MatchNotification.objects.filter(user=user, is_read=False).update(
    ...
    def get_notification_count(user):
        ...
        """
        return MatchNotification.objects.filter(user=user, is_read=False).count()
...
class MatchRecommendationService:
    ...
    def find_potential_matches_for_tenant(tenant, limit=10):
        ...
        properties = Property.objects.filter(availability_status="available")
        ...
        existing_requests = MatchRequest.objects.filter(
            tenant=tenant, status__in=["pending", "viewed", "accepted"]
    ...
    def find_qualified_tenants_for_property(property, limit=10):
        ...
        qualified_tenants = User.objects.filter(user_type="tenant", is_verified=True)
...
class MatchWorkflowService:
    ...
    def process_match_request_response(match_request, action, landlord_message=""):
        ...
        if action == "accept":
            ...
            return {"status": "success", "message": "Match rechazado"}
    ...
    def create_match_request(tenant, property, request_data):
        ...
        match_request = MatchRequest.objects.create(
            property=property, tenant=tenant, landlord=property.landlord, **request_data
        )
...
class MatchingAutomationService:
    ...
    def process_daily_matches():
        ...
        for criteria in active_criteria:
            user_results = MatchingAutomationService._process_user_daily_matches(
                criteria
            )
    ...
    def _process_user_daily_matches(criteria):
                    ...
                    if compatibility_score >= 70:
                        match_request = MatchRequest.objects.create(
                            ...
                            monthly_income=getattr(
                                criteria.tenant, "monthly_income", None
                            ),
        ...
        MatchingAutomationService._send_daily_matches_notification(
            criteria.tenant, matches, user_results["applications_sent"]
        )
    ...
    def expire_old_matches():
        ...
        expired_matches = MatchRequest.objects.filter(
            status__in=["pending", "viewed"], created_at__lte=expiry_date
        )

        expired_count = expired_matches.count()

        # Marcar como expirados
        expired_matches.update(status="expired", expires_at=timezone.now())
...
class MatchingRecommendationService:
    ...
    def get_property_recommendations_for_tenant(tenant, limit=10):
        ...
        try:
            ...
            properties = Property.objects.filter(is_active=True, status="available")
        ...
        for property in properties[: limit * 2]:  # Buffer para filtrado
            try:
                score = (
                    criteria.get_match_score(property)
                    if hasattr(tenant, "match_criteria")
                    else 50
                )
            except Exception:
                ...
                recommendations.append(
                    {
                        "property": property,
                        "match_score": score,
                        "reasons": MatchingRecommendationService._get_recommendation_reasons(
                            property, tenant
                        ),
                    }
                )
                    ...
                    recommendations.append(
                        {
                            ...
                            "criteria": criteria,
                            "reasons": MatchingRecommendationService._get_tenant_recommendation_reasons(
                                tenant, property
                            ),
                        }
                    )
    ...
    def _get_tenant_recommendation_reasons(tenant, property):
        ...
        recent_activity = MatchRequest.objects.filter(
            tenant=tenant, created_at__gte=timezone.now() - timezone.timedelta(days=30)
        ).count()
