...
class MatchRequest(models.Model):
    ...
    match_code = models.CharField(
        "Código de Match", max_length=12, unique=True, editable=False
    )
    ...
    status = models.CharField(
        "Estado", max_length=20, choices=MATCH_STATUS, default="pending"
    )
    priority = models.CharField(
        "Prioridad", max_length=10, choices=PRIORITY_LEVELS, default="medium"
    )
    ...
    monthly_income = models.DecimalField(
        "Ingresos mensuales", max_digits=12, decimal_places=2, null=True, blank=True
    )
    ...
    preferred_move_in_date = models.DateField(
        "Fecha preferida de mudanza", null=True, blank=True
    )
    ...
    has_rental_references = models.BooleanField(
        "Tiene referencias de alquiler", default=False
    )
    has_employment_proof = models.BooleanField(
        "Tiene comprobante de ingresos", default=False
    )
    has_credit_check = models.BooleanField(
        "Autoriza verificación crediticia", default=False
    )
    ...
    landlord_response = models.TextField(
        "Respuesta del arrendador", max_length=1000, blank=True
    )
    landlord_notes = models.TextField(
        "Notas privadas del arrendador", max_length=500, blank=True
    )
    ...
    contract_generated_at = models.DateTimeField(
        "Fecha de generación del contrato", null=True, blank=True
    )

    # Datos del workflow (sincronización con PropertyInterestRequest)
    workflow_stage = models.IntegerField(
        "Etapa del workflow", default=1, help_text="1=Visita, 2=Documentos, 3=Contrato"
    )
    workflow_status = models.CharField(
        "Estado del workflow",
        max_length=30,
        default="pending",
        help_text="Estado específico dentro de la etapa",
    )
    ...
    def reject_match(self, landlord_message=""):
        ...
        with transaction.atomic():
            ...
            print(
                f"🧹 Match {self.match_code} rechazado y datos limpiados completamente"
            )
    ...
    def _cleanup_associated_data(self):
        ...
        property_requests = PropertyInterestRequest.objects.filter(
            requester=self.tenant, property=self.property
        )
        doc_count = 0
        ...
        for prop_request in property_requests:
            ...
            if tenant_docs.exists():
                print(
                    f"   📋 Eliminando {tenant_docs.count()} documento(s) del inquilino"
                )
                tenant_docs.delete()
        ...
        try:
            ...
            thread = (
                Thread.objects.filter(participants__in=[self.landlord, self.tenant])
                .filter(participants__in=[self.landlord, self.tenant])
                .distinct()
                .first()
            )
        ...
        if self.property:
            print(
                f"   🏠 Liberando propiedad {self.property.title} para nuevas solicitudes"
            )
                ...
                LandlordControlledContract.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "economic_terms": {
                            ...
                            "security_deposit": float(monthly_rent)
                            if monthly_rent
                            else 0,
                        },
                        "contract_terms": {
...
class MatchCriteria(models.Model):
    ...
    min_price = models.DecimalField(
        "Precio mínimo", max_digits=10, decimal_places=2, null=True, blank=True
    )
    max_price = models.DecimalField(
        "Precio máximo", max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    def get_match_score(self, property):
        ...
        if self.required_amenities:
            ...
            matching_amenities = len(
                set(self.required_amenities) & set(property_amenities)
            )
...
class MatchNotification(models.Model):
    ...
    notification_type = models.CharField(
        "Tipo", max_length=30, choices=NOTIFICATION_TYPES
    )
...
class MatchAnalytics(models.Model):
    ...
    total_requests_created = models.PositiveIntegerField(
        "Solicitudes creadas", default=0
    )
    ...
    total_requests_accepted = models.PositiveIntegerField(
        "Solicitudes aceptadas", default=0
    )
    total_requests_rejected = models.PositiveIntegerField(
        "Solicitudes rechazadas", default=0
    )
    ...
    avg_response_time_hours = models.FloatField(
        "Tiempo promedio de respuesta (horas)", default=0.0
    )
    ...
    def calculate_daily_analytics(cls, date=None):
        ...
        total_viewed = requests.filter(
            status__in=["viewed", "accepted", "rejected"]
        ).count()
        ...
        acceptance_rate = (
            (total_accepted / total_viewed * 100) if total_viewed > 0 else 0
        )
        response_rate = (
            ((total_accepted + total_rejected) / total_created * 100)
            if total_created > 0
            else 0
        )
