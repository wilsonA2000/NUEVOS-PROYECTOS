...
from .models import MatchRequest, MatchCriteria, MatchNotification
from .serializers import (
    MatchRequestSerializer,
    MatchCriteriaSerializer,
    MatchNotificationSerializer,
    CreateMatchRequestSerializer,
    MatchRequestDetailSerializer,
)
from .utils import (
    find_potential_matches,
    calculate_match_compatibility,
    get_landlord_match_recommendations,
...
class MatchRequestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        logger.info(
            f"📝 Match Request CREATE - User: {request.user.email}, Type: {request.user.user_type}"
        )
        ...
        logger.info(
            f"   - priority: {request.data.get('priority')} (type: {type(request.data.get('priority'))})"
        )
        logger.info(
            f"   - tenant_message length: {len(request.data.get('tenant_message', ''))}"
        )
        ...
        try:
            ...
            return Response(
                {
                    "error": str(e),
                    "message": "Ya existe una solicitud para esta propiedad",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.user_type != "tenant":
            raise permissions.PermissionDenied(
                "Solo los arrendatarios pueden enviar solicitudes de match"
            )
        ...
        if existing_request:
            ...
            raise drf_serializers.ValidationError(
                "Ya has enviado una solicitud para esta propiedad"
            )

        # Crear la solicitud
        match_request = serializer.save(
            tenant=self.request.user, landlord=property.landlord
        )
    ...
    def mark_viewed(self, request, pk=None):
        ...
        return Response(
            {"message": "Solicitud marcada como vista", "status": match_request.status}
        )
    ...
    def generate_contract(self, request, pk=None):
        ...
        try:
            ...
            logger.info(
                f"✅ Contrato {contract.contract_number} generado automáticamente desde match {match_request.match_code}"
            )
    ...
    def upload_document(self, request, pk=None):
        ...
        try:
            ...
            match_request.workflow_data["last_document_uploaded_at"] = (
    ...
    def advance_to_contract_stage(self, request, pk=None):
        ...
        logger.info(
            f"🔍 Verificando permisos: User={request.user.email}, Landlord={match_request.landlord.email}"
        )
        if request.user != match_request.landlord:
        ...
        logger.info(
            f"🔍 Verificando etapa: Actual={match_request.workflow_stage}, Esperado=2"
        )
        if match_request.workflow_stage != 2:
        ...
        logger.info(
            f"🔍 Documentos pendientes: {pending_docs}, rechazados: {rejected_docs}"
        )
        ...
        match_request.workflow_data["documents_approved_at"] = (
            timezone.now().isoformat()
        )
...
class MatchCriteriaViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if self.request.user.user_type != "tenant":
            raise permissions.PermissionDenied(
                "Solo los arrendatarios pueden crear criterios de búsqueda"
            )
    ...
    def find_matches(self, request, pk=None):
        ...
        return Response(
            {"matches": matches_with_scores, "total_found": len(matches_with_scores)}
        )
...
class PotentialMatchesAPIView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"potential_matches": matches_data, "total_found": len(matches_data)}
        )
        ...
        if request.user.user_type == "tenant":
            ...
            for property in request.user.properties.filter(is_active=True):
                pending_count = MatchRequest.objects.filter(
                    property=property, status__in=["pending", "viewed"]
                ).count()
                if pending_count > 0:
...
class MatchDashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        if user.user_type == "tenant":
            ...
            recent_requests_queryset = MatchRequest.objects.filter(
                tenant=user
            ).order_by("-created_at")
            ...
            pending_count = recent_requests_queryset.filter(
            ...
            # Propiedades activas
            active_properties = user.properties.filter(
                is_active=True, status="available"
            ).count()
...
class MatchPreferencesAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response({"has_preferences": True, "preferences": preferences})
    ...
    def post(self, request):
        ...
        try:
            ...
            criteria = MatchCriteria.objects.create(tenant=request.user, **request.data)
            message = "Preferencias creadas exitosamente"

        return Response(
            {"message": message, "preferences_id": str(criteria.id)},
            status=status.HTTP_201_CREATED,
        )
    ...
    def get(self, request):
        ...
        if user.user_type == "tenant":
            ...
            requests = MatchRequest.objects.filter(
                tenant=user, created_at__gte=start_date
            )
            ...
            requests = MatchRequest.objects.filter(
                landlord=user, created_at__gte=start_date
            )
            ...
            if responded_requests.exists():
                ...
                analytics["avg_response_time_hours"] = (
                    total_response_time / responded_requests.count()
                )
            ...
            analytics["most_requested_properties"] = sorted(
                property_requests.items(), key=lambda x: x[1], reverse=True
            )[:5]
...
class SmartMatchingAPIView(APIView):
    ...
    def _advanced_matching_algorithm(self, criteria, limit, min_score):
        ...
        for match in matches:
            ...
            preferred_types = [
                req.property.property_type
                for req in user_history.filter(status="accepted")
            ]
    ...
    def _ml_matching_algorithm(self, criteria, limit, min_score):
        ...
        for match in matches:
            ...
            avg_response = MatchRequest.objects.filter(
                landlord=landlord, responded_at__isnull=False
            ).count()
            ...
            match["confidence"] = min(
                match["match_score"] / 90, 1.0
            )  # ML tiene más confianza
    ...
    def _get_match_reasons(self, property, criteria):
        ...
        if criteria.max_price and property.rent_price <= criteria.max_price * Decimal(
            "0.8"
        ):
...
class FindMatchRequestAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Propiedad no encontrada"}, status=status.HTTP_404_NOT_FOUND
            )
...
class CheckExistingMatchRequestAPIView(APIView):
    ...
    def get(self, request):
        ...
        if existing_request:
            return Response(
                {
                    ...
                    "request": {
                        ...
                        "tenant_message": existing_request.tenant_message[:100] + "..."
                        if len(existing_request.tenant_message) > 100
                        else existing_request.tenant_message,
            ...
            return Response({"has_existing_request": False, "can_create_new": True})
        ...
        try:
            ...
            match_request = (
                MatchRequest.objects.filter(
                    tenant=request.user, property_id=property_id
                )
                .order_by("-created_at")
                .first()
            )
