...
class ContractWorkflowHistorySerializer(serializers.ModelSerializer):
    ...
    related_objection_details = ContractObjectionSerializer(
        source="related_objection", read_only=True
    )
...
class LandlordControlledContractListSerializer(serializers.ModelSerializer):
    ...
    property_address = serializers.CharField(
        source="property.full_address", read_only=True
    )
    ...
    def get_progress_percentage(self, obj):
        state_progress = {
            ...
            "READY_TO_SIGN": 90,
            "FULLY_SIGNED": 95,
            "pending_tenant_biometric": 60,
            "pending_guarantor_biometric": 70,
            "pending_landlord_biometric": 80,
            "completed_biometric": 90,
            "active": 100,
            "PUBLISHED": 100,
        }
...
class LandlordControlledContractDetailSerializer(serializers.ModelSerializer):
            ...
            if field_name in validated_data:
                ...
                if isinstance(incoming_data, dict) and isinstance(existing_data, dict):
                    ...
                    if field_name == "contract_terms":
                        ...
                        if "codeudor_data" in incoming_data and isinstance(
                            incoming_data["codeudor_data"], dict
                        ):
                            existing_codeudor = existing_data.get("codeudor_data", {})
                            merged_data["codeudor_data"] = {
                                **existing_codeudor,
                                **incoming_data["codeudor_data"],
                            }

                        logger.info("[CONTRACT UPDATE] Merging contract_terms:")
                        logger.info(
                            f"  - guarantee_type: {merged_data.get('guarantee_type')}"
                        )
                        logger.info(
                            f"  - guarantor_required: {merged_data.get('guarantor_required')}"
                        )
                        logger.info(
                            f"  - guests_policy: {merged_data.get('guests_policy')}"
                        )
                        logger.info(
                            f"  - codeudor_data present: {bool(merged_data.get('codeudor_data'))}"
                        )
    ...
    def get_recent_history(self, obj):
        ...
        return ContractWorkflowHistorySerializer(
...
class ContractCreateSerializer(serializers.ModelSerializer):
    ...
    tenant_data = serializers.JSONField(
        required=False, default=dict
    )  # CRITICAL: Datos del arrendatario
    ...
    smoking_allowed = serializers.BooleanField(
        required=False, default=False, write_only=True
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            "property_data",
            "landlord_data",
            "tenant_data",  # CRITICAL: tenant_data agregado
            "basic_terms",
            "guarantee_terms",
            "special_clauses",
            "contract_template",
    ...
    def create(self, validated_data):
        ...
        tenant_data = validated_data.pop(
            "tenant_data", {}
        )  # CRITICAL: Extraer datos del arrendatario
        ...
        validated_data["tenant_identifier"] = (
            ""  # Se completará cuando se invite al tenant
        )
        ...
        economic_terms = {
            ...
            "security_deposit": float(
                basic_terms.get("security_deposit", security_deposit or 0)
            ),
            ...
            "rent_increment_percentage": float(
                basic_terms.get("rent_increment_percentage", 0)
            ),
        }
        ...
        contract_terms = {
            # Duración y fechas
            "contract_duration_months": basic_terms.get(
                "duration_months",
                basic_terms.get(
                    "contract_duration_months", contract_duration_months or 12
                ),
            ),
            "start_date": basic_terms.get("start_date", ""),
            "end_date": basic_terms.get("end_date", ""),
            # Pagos
            "payment_due_day": basic_terms.get(
                "payment_day", basic_terms.get("payment_due_day", 5)
            ),
            "grace_period_days": basic_terms.get("grace_period_days", 5),
            "rent_increase_type": basic_terms.get("rent_increase_type", "ipc"),
            # Servicios incluidos
            "utilities_included": basic_terms.get(
            ...
            # Responsabilidades
            "maintenance_responsibility": basic_terms.get(
        ...
        if guarantee_terms and (
            guarantee_terms.get("guarantor_required")
            or guarantee_terms.get("guarantee_type", "none") != "none"
        ):
            contract_terms.update(
                {
                    ...
                    "guarantee_amount": float(
                        guarantee_terms.get("guarantee_amount", 0)
                    ),
                    "requires_biometric_codeudor": guarantee_terms.get(
        ...
        validated_data["tenant_data"] = self._enhance_tenant_data(
            tenant_data
        )  # CRITICAL: Guardar datos del arrendatario
        ...
        if contract_content:
            validated_data["description"] = contract_content[
                :1000
    ...
    def _enhance_property_data(self, property_data):
        ...
        if "property_type" in enhanced:
            ...
            enhanced["property_type_display"] = property_types.get(
                enhanced["property_type"], enhanced["property_type"]
            )
    ...
    def _enhance_landlord_data(self, landlord_data):
        ...
        if "document_type" in enhanced:
            ...
            enhanced["document_type_display"] = doc_types.get(
                enhanced["document_type"], enhanced["document_type"]
            )
    ...
    def _enhance_tenant_data(self, tenant_data):
        ...
        if "employment_type" in enhanced:
            ...
            enhanced["employment_type_display"] = employment_types.get(
                enhanced["employment_type"], enhanced["employment_type"]
            )
    ...
    def validate_basic_terms(self, value):
        ...
        for field in required_fields:
            if field not in value:
                raise serializers.ValidationError(
                    f"Campo requerido en basic_terms: {field}"
                )
        ...
        if "monthly_rent" in value:
            try:
                ...
                if monthly_rent <= 0:
                    raise serializers.ValidationError(
                        "El canon mensual debe ser mayor a 0"
                    )
            except (TypeError, ValueError):
                raise serializers.ValidationError(
                    "El canon mensual debe ser un número válido"
                )
        ...
        if "security_deposit" in value:
            try:
                ...
                raise serializers.ValidationError(
                    "El depósito debe ser un número válido"
                )
        ...
        if value.get("guarantor_required") and value.get("guarantee_type") != "none":
            ...
            for field in required_codeudor_fields:
                if not codeudor_data.get(field):
                    field_display = (
                        field.replace("codeudor_", "").replace("_", " ").title()
                    )
                    raise serializers.ValidationError(
                        f"Se requiere {field_display} del codeudor"
                    )
            ...
            if "codeudor_email" in codeudor_data:
                ...
                if not re.match(email_pattern, codeudor_data["codeudor_email"]):
                    raise serializers.ValidationError(
                        "Email del codeudor debe tener un formato válido"
                    )
...
class LandlordDataSerializer(serializers.Serializer):
    ...
    bank_account_type = serializers.ChoiceField(
        choices=[("savings", "Ahorros"), ("checking", "Corriente")], required=False
    )
    ...
    maintenance_responsibilities = serializers.CharField(
...
class ContractObjectionResponseSerializer(serializers.Serializer):
    ...
    response = serializers.ChoiceField(
        choices=[("ACCEPTED", "Aceptada"), ("REJECTED", "Rechazada")]
    )
...
class ContractSignatureSerializer(serializers.Serializer):
    ...
    def validate_signature_data(self, value):
        ...
        for field in required_fields:
            if field not in value:
                raise serializers.ValidationError(
                    f"Campo requerido en signature_data: {field}"
                )
        return value
...
class ContractModificationRequestSerializer(serializers.ModelSerializer):
    ...
    contract_address = serializers.CharField(
        source="contract.property_address", read_only=True
    )
...
class ContractModificationRequestCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ContractModificationRequest
        fields = ["contract", "requested_changes", "reason"]
    ...
    def validate_requested_changes(self, value):
        ...
        if not isinstance(value, dict):
            raise serializers.ValidationError(
                "requested_changes debe ser un objeto JSON."
            )

        if not value:
            raise serializers.ValidationError(
                "Debe especificar al menos un cambio solicitado."
            )
