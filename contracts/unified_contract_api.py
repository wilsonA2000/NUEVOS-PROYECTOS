...
class UnifiedContractViewSet(viewsets.ModelViewSet):
    ...
    def tenant_approve(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"✅ Contrato {contract.contract_number} aprobado por arrendatario {request.user.email}"
            )
    ...
    def tenant_object(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"⚠️ Contrato {contract.contract_number} tiene {len(objections_data)} objeciones"
            )
    ...
    def send_to_tenant_review(self, request, pk=None):
        ...
        if contract.primary_party != request.user:
            raise PermissionDenied(
                "Solo el arrendador puede enviar el contrato a revisión"
            )
        ...
        with transaction.atomic():
            ...
            logger.info(
                f"📤 Contrato {contract.contract_number} enviado a revisión del arrendatario"
            )
        ...
        with transaction.atomic():
            for response in responses:
                objection = ContractObjection.objects.get(
                    id=response["objection_id"], contract=contract
                )
