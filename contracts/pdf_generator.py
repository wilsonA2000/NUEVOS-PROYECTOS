...
try:
    ...
    raise ImportError(
        "qrcode y Pillow son requeridos. Instalar con: pip install qrcode pillow"
    )
...
class NotarialTemisWatermark(Flowable):
    ...
    def draw(self):
        ...
        self.canv.line(
            arm_right_x, arm_right_y, arm_right_x, arm_right_y - sword_length
        )

        # Empuñadura de la espada (línea horizontal pequeña)
        self.canv.setLineWidth(1.5)
        self.canv.line(
            arm_right_x - 8, arm_right_y - 5, arm_right_x + 8, arm_right_y - 5
        )
        ...
        self.canv.drawCentredString(
            center_x, center_y - 145, "Plataforma Digital Inmobiliaria"
        )
...
class ProfessionalPageTemplate(PageTemplate):
    ...
    def __init__(self, id, contract_number, **kwargs):
        ...
        frames = [
            Frame(
                54,
                54,  # x, y (0.75 inch margins optimized)
                letter[0] - 108,
                letter[1] - 180,  # width, height optimized
                leftPadding=0,
                bottomPadding=0,
                rightPadding=0,
                topPadding=0,
    ...
    def _draw_professional_frame(self, canvas):
        ...
        )  # Color dorado oscuro tipo bronce
    ...
    def _draw_corner_decorations(self, canvas):
        ...
        canvas.line(
            letter[0] - 27,
            letter[1] - 27 - corner_size,
            letter[0] - 27 - corner_size,
            letter[1] - 27,
        )
    ...
    def _draw_laurel_borders(self, canvas):
        ...
        try:
            canvas.drawCentredString(
                -letter[1] / 2, letter[0] - 8, lateral_pattern[:60]
            )
        except Exception:
    ...
    def _draw_header(self, canvas):
        ...
        canvas.drawCentredString(
            letter[0] / 2,
            letter[1] - 100,
            "WILSON ARGUELLO - ABOGADOS CONSULTORES INMOBILIARIOS",
        )
    ...
    def _get_dynamic_header(self):
        ...
        try:
            ...
            if hasattr(self, "pdf_generator") and self.pdf_generator:
                property_data = self.pdf_generator._get_property_data(
                    self.pdf_generator._current_contract
                )
                property_type = property_data.get("type", "").lower()

                # Detectar si es comercial o de vivienda
                if any(
                    word in property_type
                    for word in [
                        "comercial",
                        "oficina",
                        "local",
...
class ContractPDFGenerator:
    ...
    def _register_fonts(self):
        ...
        self.professional_font = (
            "Helvetica-Narrow"
            if self._font_available("Helvetica-Narrow")
            else "Helvetica"
        )
    ...
    def generate_contract_pdf(
        self, contract, include_signatures=True, include_biometric=True
    ):
        """
        ...
        professional_template = ProfessionalPageTemplate(
            "professional", contract.contract_number
        )
        ...
        signatures_elements = self._build_signatures_section_professional(
            contract, include_signatures, include_biometric
        )
    ...
    def _build_cover_page(self, contract):
        ...
        story.append(Paragraph(contract_title, self.styles["ContractTitle"]))
        ...
        story.append(
            Paragraph(
                f"{_signing_city}, {formato_fecha_es(contract.created_at)}",
        ...
        story.append(
            Paragraph("RESUMEN EJECUTIVO DEL CONTRATO", self.styles["ContractSubtitle"])
        )
        ...
        summary_data = [
            ...
            [
                "Arrendatario:",
                tenant_data.get("full_name", "Por definir")
                if tenant_data
                else "Por definir",
            ],
            ...
            [
                "Estado:",
                {
                    "active": "ACTIVO",
                    "ACTIVE": "ACTIVO",
                    "pending_authentication": "EN AUTENTICACIÓN",
                    "pending_landlord_biometric": "EN AUTENTICACIÓN",
                    "pending_tenant_biometric": "EN AUTENTICACIÓN",
                    "all_biometrics_completed": "AUTENTICADO",
                    "draft": "BORRADOR",
                    "signed": "FIRMADO",
                }.get(getattr(contract, "status", ""), "VIGENTE"),
            ],
        ]
        ...
        if landlord_data.get("document_expedition_place"):
            landlord_expedition_parts.append(
                f"expedida en {landlord_data.get('document_expedition_place')}"
            )
        ...
        landlord_expedition_text = (
            ", ".join(landlord_expedition_parts) if landlord_expedition_parts else ""
        )
        ...
        if tenant_data and tenant_data.get("document_expedition_place"):
            tenant_expedition_parts.append(
                f"expedida en {tenant_data.get('document_expedition_place')}"
            )
        ...
        tenant_expedition_text = (
            ", ".join(tenant_expedition_parts) if tenant_expedition_parts else ""
        )
        ...
        """.format(
            ...
            landlord_expedition=f" ({landlord_expedition_text})"
            if landlord_expedition_text
            else "",
            tenant_name=tenant_data.get("full_name", "[ARRENDATARIO]")
            if tenant_data
            else "[ARRENDATARIO]",
            tenant_doc_type=tenant_data.get("document_type", "CC")
            if tenant_data
            else "CC",
            tenant_doc_number=tenant_data.get("document_number", "[NÚMERO]")
            if tenant_data
            else "[NÚMERO]",
            tenant_expedition=f" ({tenant_expedition_text})"
            if tenant_expedition_text
            else "",
        )
    ...
    def _build_parties_detailed_info(self, contract):
        ...
        story.append(
            Paragraph(
                "INFORMACIÓN DETALLADA DE LAS PARTES", self.styles["ContractTitle"]
            )
        )
        ...
        doc_info = f"{self._get_document_type_display(landlord_data.get('document_type', 'CC'))} {landlord_data.get('document_number', '')}"
        expedition_info = []
        if landlord_data.get("document_expedition_place"):
            expedition_info.append(
                f"expedido en {landlord_data.get('document_expedition_place')}"
            )
        if landlord_data.get("document_expedition_date"):
            # Formatear fecha si es string ISO (YYYY-MM-DD)
            exp_date = landlord_data.get("document_expedition_date")
        ...
        landlord_table = Table(
            landlord_info_wrapped, colWidths=[2.2 * inch, 3.8 * inch]
        )
        ...
        if tenant_data:
            ...
            if tenant_data.get("document_expedition_place"):
                tenant_expedition_info.append(
                    f"expedido en {tenant_data.get('document_expedition_place')}"
                )
            if tenant_data.get("document_expedition_date"):
                tenant_expedition_info.append(
                    f"el {tenant_data.get('document_expedition_date')}"
                )
            ...
            if tenant_data.get("company_name") or tenant_data.get("position"):
                ...
                tenant_info_data.append(
                    [
                        "Ocupación:",
                        f"{employment_display} - {' '.join(work_info)}"
                        if work_info
                        else employment_display,
                    ]
                )
                ...
                if tenant_data.get("monthly_income"):
                    tenant_info_data.append(
                        [
                            "Ingresos Mensuales:",
                            self._format_currency(tenant_data.get("monthly_income", 0)),
                        ]
                    )

            # Agregar contacto de emergencia si está disponible
            if tenant_data.get("emergency_contact_name") or tenant_data.get(
                "emergency_contact_phone"
            ):
                emergency_info = tenant_data.get("emergency_contact_name", "")
                if tenant_data.get("emergency_contact_relationship"):
                    emergency_info += (
                        f" ({tenant_data.get('emergency_contact_relationship')})"
                    )
                if tenant_data.get("emergency_contact_phone"):
                    emergency_info += (
                        f" - Tel: {tenant_data.get('emergency_contact_phone')}"
                    )
        ...
        story.append(
            Paragraph("INFORMACIÓN DEL INMUEBLE", self.styles["ContractSubtitle"])
        )
        ...
        guests_policy_display = {
            ...
            raw_guests_policy,
            f'{raw_guests_policy.upper() if raw_guests_policy else "LIMITADO"}',
        )
        ...
        property_info_data = [
            ...
            [
                "Área Total:",
                f"{int(float(property_data.get('area', 0)))} m²"
                if property_data.get("area")
                else "No especificada",
            ],
            [
                "Estrato Socioeconómico:",
                property_data.get("stratum") or "No especificado",
            ],
            ...
            [
                ...
                "AMOBLADO" if property_data.get("furnished") else "SIN AMOBLAR",
            ],
            ...
            [
                "Mascotas Permitidas:",
                "SÍ" if contract_terms.get("pets_allowed") else "NO",
            ],
            [
                "Fumar Permitido:",
                "SÍ" if contract_terms.get("smoking_allowed") else "NO",
            ],
        ...
        property_table = Table(
            property_info_wrapped, colWidths=[2.2 * inch, 3.8 * inch]
        )
    ...
    def _build_contract_terms_detailed(self, contract):
        ...
        story.append(Spacer(1, 0.15 * inch))
        story.append(
            Paragraph("TÉRMINOS ECONÓMICOS Y DE DURACIÓN", self.styles["ContractTitle"])
        )
        ...
        economic_data = [
            ...
            ["Incremento Anual", "Según IPC certificado por DANE", "Ley 820 de 2003"],
        ]

        economic_table = Table(
            economic_data, colWidths=[2.2 * inch, 2.2 * inch, 2.1 * inch]
        )
    ...
    def _build_legal_clauses_part1(self, contract):
        ...
        clauses_part1 = [
            ...
            },
            {
                ...
                a {self._to_num(contract_terms.get('security_deposit', 0)) / max(self._to_num(contract_terms.get('monthly_rent', 1)), 1):.1f}
    ...
    def _build_dynamic_guarantee_clause_34(self, contract):
        ...
        clause_content = self._generate_guarantee_clause_content(
            guarantee_type, guarantee_data
        )
        ...
        if guarantee_type in [
            "codeudor_salario",
            "codeudor_finca_raiz",
        ] and guarantee_data.get("codeudor_full_name"):
    ...
    def _get_guarantee_data(self, contract):
        ...
        if hasattr(contract, "contract_terms") and isinstance(
            contract.contract_terms, dict
        ):
            # Verificar si hay datos de garantías en contract_terms
            # Soporta tanto 'guarantor_required' como 'guarantee_type' directamente
            has_guarantee = contract.contract_terms.get(
                "guarantor_required"
            ) or contract.contract_terms.get("guarantee_type") not in [None, "", "none"]
            if has_guarantee:
                guarantee_data.update(
                    {
                        "guarantee_type": contract.contract_terms.get(
                            "guarantee_type", "codeudor"
                        ),
                        "guarantee_amount": contract.contract_terms.get(
                            "guarantee_amount", 0
                        ),
                    ...
                    guarantee_data.update(
                        {
                            ...
                            "codeudor_full_name": codeudor_data.get(
                                "codeudor_full_name", ""
                            ),
                            "codeudor_document_type": codeudor_data.get(
                                "codeudor_document_type", ""
                            ),
                            "codeudor_document_number": codeudor_data.get(
                                "codeudor_document_number", ""
                            ),
                            "codeudor_phone": codeudor_data.get("codeudor_phone", ""),
                            "codeudor_email": codeudor_data.get("codeudor_email", ""),
                            "codeudor_address": codeudor_data.get(
                                "codeudor_address", ""
                            ),
                            "codeudor_city": codeudor_data.get("codeudor_city", ""),
                            "codeudor_department": codeudor_data.get(
                                "codeudor_department", ""
                            ),
                            "codeudor_occupation": codeudor_data.get(
                                "codeudor_occupation", ""
                            ),
                            "codeudor_monthly_income": codeudor_data.get(
                                "codeudor_monthly_income", ""
                            ),
                            "codeudor_company_name": codeudor_data.get(
                                "codeudor_company_name", ""
                            ),
                            "codeudor_company_phone": codeudor_data.get(
                                "codeudor_company_phone", ""
                            ),
                            "codeudor_relationship": codeudor_data.get(
                                "codeudor_relationship", ""
                            ),
                            "codeudor_marital_status": codeudor_data.get(
                                "codeudor_marital_status", ""
                            ),
                            "codeudor_spouse_name": codeudor_data.get(
                                "codeudor_spouse_name", ""
                            ),
                            "codeudor_spouse_document": codeudor_data.get(
                                "codeudor_spouse_document", ""
                            ),
                            "codeudor_property_address": codeudor_data.get(
                                "codeudor_property_address", ""
                            ),
                            "codeudor_property_value": codeudor_data.get(
                                "codeudor_property_value", ""
                            ),
                            "codeudor_references": codeudor_data.get(
                                "codeudor_references", []
                            ),
                            # Datos del codeudor_salario (empleador)
                            "codeudor_employer_name": codeudor_data.get(
                                "codeudor_employer", ""
                            ),
                            "codeudor_position": codeudor_data.get(
                                "codeudor_position", ""
                            ),
                            "codeudor_work_phone": codeudor_data.get(
                                "codeudor_work_phone", ""
                            ),
                            # Datos de garantía inmobiliaria (codeudor_finca_raiz)
                            "guarantee_property_matricula": codeudor_data.get(
                                "property_matricula", ""
                            ),
                            "guarantee_property_area": codeudor_data.get(
                                "property_area_guarantee", ""
                            ),
                            "guarantee_property_address": codeudor_data.get(
                                "property_address_guarantee", ""
                            ),
                            "guarantee_property_city": codeudor_data.get(
                                "property_city_guarantee", ""
                            ),
                            "guarantee_property_department": codeudor_data.get(
                                "property_department_guarantee", ""
                            ),
                            "guarantee_property_predial": codeudor_data.get(
                                "property_predial_number", ""
                            ),
                            "guarantee_property_catastral": codeudor_data.get(
                                "property_catastral_number", ""
                            ),
                            "guarantee_property_linderos": codeudor_data.get(
                                "property_linderos", ""
                            ),
                        }
                    )
        ...
        # Para contratos regulares con guarantees relationship
            ...
            if guarantees:
                guarantee_data = {
                    ...
                    if guarantees.guarantee_type == "CO_SIGNER"
                    else "none",
                    "codeudor_full_name": guarantees.co_signer_data.get("name", "")
                    if guarantees.co_signer_data
                    else "",
    ...
    def _generate_guarantee_clause_content(self, guarantee_type, guarantee_data):
        ...
        if guarantee_type == "none" or not guarantee_type:
            ...
            codeudor_name = guarantee_data.get(
                "codeudor_full_name", "[NOMBRE DEL CODEUDOR]"
            )
            codeudor_doc = f"{guarantee_data.get('codeudor_document_type', 'CC')} {guarantee_data.get('codeudor_document_number', '[NÚMERO]')}"
            codeudor_employer = guarantee_data.get(
                "codeudor_employer_name", "[EMPLEADOR]"
            )
            ...
            matricula = guarantee_data.get(
                "guarantee_property_matricula", "[MATRÍCULA INMOBILIARIA]"
            )
            predial = guarantee_data.get(
                "guarantee_property_predial", "[NÚMERO PREDIAL]"
            )
            catastral = guarantee_data.get(
                "guarantee_property_catastral", "[NÚMERO CATASTRAL]"
            )
            address = guarantee_data.get(
                "guarantee_property_address", "[DIRECCIÓN DEL INMUEBLE]"
            )
            ...
            linderos_text = (
                f" Los linderos del inmueble son: {linderos}." if linderos else ""
            )
    ...
    def _build_cosigner_details(self, guarantee_data):
        ...
        cosigner_details = [
            ...
            [
                "Documento:",
                f"{guarantee_data.get('codeudor_document_type', 'CC')} {guarantee_data.get('codeudor_document_number', '')}",
            ],
        ...
        if guarantee_data.get("codeudor_company_name"):
            cosigner_details.extend(
                [
                    ...
                    [
                        "Teléfono empresa:",
                        guarantee_data.get("codeudor_company_phone", ""),
                    ],
                    [
                        "Ingresos mensuales:",
                        f"${guarantee_data.get('codeudor_monthly_income', '')}",
                    ],
                ]
            )
        ...
        if guarantee_data.get("codeudor_spouse_name"):
            cosigner_details.extend(
                [
                    [
                        "Nombre del cónyuge:",
                        guarantee_data.get("codeudor_spouse_name", ""),
                    ],
                    [
                        "Documento del cónyuge:",
                        guarantee_data.get("codeudor_spouse_document", ""),
                    ],
                ]
            )
        ...
        if guarantee_data.get("codeudor_property_address"):
            cosigner_details.extend(
                [
                    [
                        "Dirección propiedad:",
                        guarantee_data.get("codeudor_property_address", ""),
                    ],
                    [
                        "Valor propiedad:",
                        f"${guarantee_data.get('codeudor_property_value', '')}",
                    ],
                ]
            )
            ...
            cosigner_details.extend(
                [
                    [
                        "Empleador:",
                        guarantee_data.get(
                            "codeudor_employer_name",
                            guarantee_data.get("codeudor_company_name", ""),
                        ),
                    ],
                    [
                        "Cargo:",
                        guarantee_data.get(
                            "codeudor_job_title",
                            guarantee_data.get("codeudor_occupation", ""),
                        ),
                    ],
                    [
                        "Ingresos mensuales:",
                        self._format_currency(
                            float(guarantee_data.get("codeudor_monthly_income", 0))
                        ),
                    ],
                ]
            )
            ...
            cosigner_details.extend(
                [
                    ...
                    [
                        ...
                        guarantee_data.get("guarantee_property_address", ""),
                    ],
                    ["Ciudad:", guarantee_data.get("guarantee_property_city", "")],
                    [
                        "Departamento:",
                        guarantee_data.get("guarantee_property_department", ""),
                    ],
                    [
                        "Área:",
                        f"{guarantee_data.get('guarantee_property_area', '')} m²"
                        if guarantee_data.get("guarantee_property_area")
                        else "",
                    ],
                    [
                        "Matrícula inmobiliaria:",
                        guarantee_data.get("guarantee_property_matricula", ""),
                    ],
                    [
                        "Número predial:",
                        guarantee_data.get("guarantee_property_predial", ""),
                    ],
                    [
                        "Número catastral:",
                        guarantee_data.get("guarantee_property_catastral", ""),
                    ],
                    [
                        "Linderos:",
                        guarantee_data.get("guarantee_property_linderos", ""),
                    ],
                ]
            )
        ...
        cosigner_details = [
            [label, value]
            for label, value in cosigner_details
    ...
    def _build_signatures_section_professional(
        self, contract, include_signatures=True, include_biometric=True
    ):
        ...
        anteriores, las partes suscriben el presente contrato en la ciudad de {_signing_city2},
        ...
        guarantee_data = self._get_guarantee_data(contract)
        has_codeudor = guarantee_data.get("guarantee_type") in [
            "codeudor_salario",
            "codeudor_finca_raiz",
        ] and guarantee_data.get("codeudor_full_name")
        ...
        landlord_auth_date = landlord_data.get(
            "authentication_date", "[Pendiente autenticación]"
        )
        tenant_auth_date = (
            tenant_data.get("authentication_date", "[Pendiente autenticación]")
            if tenant_data
            else "[Pendiente autenticación]"
        )
        ...
        signatures_data = [
            ...
            [
                ...
                tenant_data.get("full_name", "[NOMBRE ARRENDATARIO]")
                if tenant_data
                else "[NOMBRE ARRENDATARIO]",
            ],
            [
                f"{self._get_document_type_display(landlord_data.get('document_type', 'CC'))} {landlord_data.get('document_number', 'No registrado')}",
                f"{self._get_document_type_display(tenant_data.get('document_type', 'CC'))} {tenant_data.get('document_number', 'No registrado')}"
                if tenant_data
                else "Cédula de Ciudadanía No registrado",
            ],
        ...
        if has_codeudor:
            story.append(Spacer(1, 0.2 * inch))
            codeudor_name = guarantee_data.get(
                "codeudor_full_name", "[NOMBRE CODEUDOR]"
            )
            codeudor_doc = f"{guarantee_data.get('codeudor_document_type', 'CC')} {guarantee_data.get('codeudor_document_number', '[NÚMERO]')}"
            codeudor_auth_date = guarantee_data.get(
                "authentication_date", "[Pendiente autenticación]"
            )
        ...
        if include_biometric:
            story.append(Spacer(1, 0.15 * inch))
            story.append(
                Paragraph("VERIFICACIÓN DIGITAL", self.styles["ContractSubtitle"])
            )
    ...
    def _build_verification_section_professional(self, contract):
        ...
        verification_elements.append(
            Paragraph("INFORMACIÓN DE VERIFICACIÓN", self.styles["ContractSubtitle"])
        )
        ...
        if qr_code:
            ...
            verification_table = Table(
                verification_info, colWidths=[1.5 * inch, 3.5 * inch]
            )
        ...
        Abogado - Consejo Superior de la Judicatura - Colombia<br/><br/>
        ...
        verification_elements.append(
            Paragraph(footer_text, self.styles["ContractFooter"])
        )
    ...
    def _get_landlord_data(self, contract):
        ...
        if hasattr(contract, "landlord_data") and contract.landlord_data:
            ...
            if hasattr(contract, "landlord") and contract.landlord:
                ...
                if not data.get("phone") or data.get("phone").strip() == "":
                    data["phone"] = getattr(user, "phone_number", "") or getattr(
                        user, "whatsapp", ""
                    )
            ...
            return {
                ...
                "document_type": data.get(
                    "document_type_display", data.get("document_type", "CC")
                ),
        ...
        if not data.get("document_type") or not data.get("document_number"):
            ...
            if biometric_data:
                data["document_type"] = data.get("document_type") or biometric_data.get(
                    "document_type", "CC"
                )
                data["document_number"] = data.get(
                    "document_number"
                ) or biometric_data.get("document_number", "")
        ...
        if data.get("authentication_date"):
            ...
            if bio.get("document_type") and not data.get("document_type"):
                data["document_type"] = bio["document_type"]
        return {
            "full_name": full_name,
            "email": data.get("email") or "No registrado",
            "phone": data.get("phone") or "No registrado",
            "document_type": data.get("document_type") or "Cédula de Ciudadanía",
            "document_number": data.get("document_number") or "No registrado",
            "address": data.get("address") or "No registrada",
            "authentication_date": auth_date,
        }
    ...
    def _get_tenant_data(self, contract):
        ...
        if (
            (not data or not data.get("full_name"))
            and hasattr(contract, "tenant")
            and contract.tenant
        ):
            ...
            user_data = {
                ...
                else f"{getattr(user, 'first_name', '')} {getattr(user, 'last_name', '')}".strip(),
                ...
                "document_number": getattr(user, "document_number", ""),
                "current_address": getattr(user, "current_address", "")
                or getattr(user, "address", ""),
        ...
        if data.get("authentication_date"):
            ...
            if tenant_bio.get("document_type") and not data.get("document_type"):
                data["document_type"] = tenant_bio["document_type"]
        return {
            # Datos personales básicos
            "full_name": full_tenant_name,
            "email": data.get("email") or "No registrado",
            "phone": data.get("phone") or "No registrado",
            "document_type": data.get("document_type") or "CC",
            "document_type_display": data.get("document_type_display")
            or self._get_document_type_display(data.get("document_type", "CC")),
            ...
            or data.get("emergency_relationship")
            or "",
            # Otros campos
            "marital_status": data.get("marital_status") or "Pendiente",
            "authentication_date": tenant_auth_date,
        }
    ...
    def _get_contract_terms(self, contract):
        ...
        if hasattr(contract, "economic_terms") and contract.economic_terms:
            ...
            contract_terms_data = (
                ...
                if hasattr(contract, "contract_terms") and contract.contract_terms
                else {}
            )
            ...
            return {
                ...
                "payment_day": contract_terms_data.get(
                    "payment_due_day", contract.economic_terms.get("payment_day", 5)
                ),
                ...
                # Incremento de renta
                "rent_increase_type": contract_terms_data.get(
                    "rent_increase_type", "ipc"
                ),
                "grace_period_days": contract_terms_data.get("grace_period_days", 5),
                # Servicios incluidos
                "utilities_included": contract_terms_data.get(
                    "utilities_included", False
                ),
                "internet_included": contract_terms_data.get(
                ...
                # Responsabilidades
                "maintenance_responsibility": contract_terms_data.get(
                    "maintenance_responsibility", "tenant"
                ),
                "guarantor_required": contract_terms_data.get(
    ...
    def _get_clauses_from_db(self, contract) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            clauses = template.get_ordered_clauses().filter(clause_number__lt=34)
    ...
    def _build_clause_context(self, contract) -> Dict[str, str]:
        ...
        context = {
            ...
            # === ECONÓMICOS ===
            "monthly_rent": self._format_currency(
                contract_terms.get("monthly_rent", 0)
            ),
            "monthly_rent_words": self._number_to_words(
                contract_terms.get("monthly_rent", 0)
            ),
            "security_deposit": self._format_currency(
                contract_terms.get("security_deposit", 0)
            ),
            "payment_day": str(contract_terms.get("payment_day", 5)),
            # === DURACIÓN ===
            "contract_duration_months": str(
                contract_terms.get("contract_duration_months", 12)
            ),
            ...
            "landlord_document_number": landlord_data.get(
            ...
            # === CODEUDOR / GARANTÍAS ===
            "codeudor_full_name": guarantee_data.get(
                "codeudor_full_name", "[NOMBRE CODEUDOR]"
            ),
            "codeudor_document_type": guarantee_data.get(
                "codeudor_document_type", "CC"
            ),
            "codeudor_document_number": guarantee_data.get(
                "codeudor_document_number", "[DOCUMENTO]"
            ),
            "codeudor_address": guarantee_data.get(
                "codeudor_address", "[DIRECCIÓN CODEUDOR]"
            ),
            "codeudor_phone": guarantee_data.get("codeudor_phone", "[TELÉFONO]"),
            "codeudor_email": guarantee_data.get("codeudor_email", "[EMAIL]"),
            "codeudor_occupation": guarantee_data.get(
                "codeudor_occupation", "[OCUPACIÓN]"
            ),
            "codeudor_monthly_income": self._format_currency(
                float(guarantee_data.get("codeudor_monthly_income", 0) or 0)
            ),
            "codeudor_employer": guarantee_data.get(
                "codeudor_employer_name", "[EMPLEADOR]"
            ),
    ...
    def _build_parties_info(self, contract):
        ...
        parties_data = [
            ...
            [
                ...
                f"{tenant_data.get('document_type', '')} {tenant_data.get('document_number', '')}"
                if tenant_data
                else "Por definir",
            ],
            [
                "Teléfono:",
                landlord_data.get("phone", ""),
                tenant_data.get("phone", "") if tenant_data else "Por definir",
            ],
            [
                "Email:",
                landlord_data.get("email", ""),
                tenant_data.get("email", "") if tenant_data else "Por definir",
            ],
            [
                "Dirección:",
                landlord_data.get("address", ""),
                tenant_data.get("current_address", "")
                if tenant_data
                else "Por definir",
            ],
        ]
        ...
        parties_table = Table(
            parties_data, colWidths=[1.5 * inch, 2.5 * inch, 2.5 * inch]
        )
    ...
    def _build_contract_terms(self, contract):
        ...
        story.append(
            Paragraph("TÉRMINOS Y CONDICIONES", self.styles["ContractSubtitle"])
        )
        ...
        if property_data.get("parking_spaces"):
            property_details.append(
                ["Parqueaderos:", property_data.get("parking_spaces")]
            )
        property_details.append(
            ["Amoblado:", "Sí" if property_data.get("furnished") else "No"]
        )
        ...
        # Términos económicos
        story.append(
            Paragraph("2. CANON DE ARRENDAMIENTO", self.styles["ContractSubtitle"])
        )
        ...
        if security_deposit > 0:
            story.append(
                Paragraph("3. DEPÓSITO DE GARANTÍA", self.styles["ContractSubtitle"])
            )
            deposit_info = f"""
        ...
        # Duración
        story.append(
            Paragraph("4. DURACIÓN DEL CONTRATO", self.styles["ContractSubtitle"])
        )
        duration_months = contract_terms.get("contract_duration_months", 12)
        start_date = getattr(contract, "start_date", None)
        end_date = getattr(contract, "end_date", None)
    ...
    def _build_legal_clauses(self, contract):
        ...
        if special_clauses:
            story.append(
                Paragraph("<b>CLÁUSULAS ESPECIALES:</b>", self.styles["ContractNormal"])
            )
    ...
    def _build_signatures_section(self, contract, include_biometric=True):
        ...
        if include_biometric and (landlord_signed or tenant_signed):
            story.append(Spacer(1, 0.15 * inch))
            story.append(
                Paragraph("VERIFICACIÓN BIOMÉTRICA", self.styles["ContractSubtitle"])
            )
    ...
    def _add_signature_image(self, signature_data, title, signed_at):
        ...
        if signature_data and "signature_image" in signature_data:
            try:
                ...
                story.append(
                    Paragraph("[Firma Digital Aplicada]", self.styles["Normal"])
                )
    ...
    def _build_verification_section(self, contract):
        ...
        story.append(PageBreak())
        story.append(
            Paragraph("VERIFICACIÓN DEL CONTRATO", self.styles["ContractSubtitle"])
        )
        ...
        if qr_code:
            ...
            verification_table = Table(
                verification_data, colWidths=[2 * inch, 4 * inch]
            )
            verification_table.setStyle(
                TableStyle(
                    [
    ...
    def _to_num(self, value, default=0):
        """Convierte un valor (posiblemente string) a float, devuelve default si falla."""
        if isinstance(value, (int, float)):
            return float(value)
        try:
            s = str(value).strip()
            # Intento directo primero (cubre "1500000.00", "1500000", etc.)
            return float(s)
        except ValueError:
            pass
        try:
    ...
    def _format_currency(self, amount):
        ...
        try:
            numeric = self._to_num(amount)
            return f"${numeric:,.0f} COP"
        except (ValueError, TypeError):
            return f"${amount} COP"
    ...
    def _get_contract_title(self, contract):
        ...
        try:
            ...
            if any(
                word in property_type
                for word in ["comercial", "oficina", "local", "negocio", "commercial"]
            ):
    ...
    def _get_biometric_auth_data(self, contract, party_type):
        ...
        try:
            from contracts.models import BiometricAuthentication, Contract
            ...
            # BiometricAuthentication.contract apunta a Contract (legacy).
            # Si recibimos un LandlordControlledContract (mismo UUID), resolvemos al Contract.
            contract_for_query = contract
            if not isinstance(contract, Contract):
                try:
                    contract_for_query = Contract.objects.get(pk=contract.pk)
                except Contract.DoesNotExist:
                    contract_for_query = contract

            auth = BiometricAuthentication.objects.filter(
                contract=contract_for_query, user=user, status="completed"
            ).first()

            if auth:
                raw_doc_type = auth.document_type or "CC"
                return {
                    "document_type": self._get_document_type_display(raw_doc_type),
                    "document_number": auth.document_number or "",
