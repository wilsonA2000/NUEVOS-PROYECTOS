...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if verbose:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Iniciando procesamiento de notificaciones de contratos: {start_time}"
                )
            )
        ...
        try:
            ...
            self.stdout.write(
                f'Contratos próximos a expirar: {stats["expiring_contracts"]}'
            )
            self.stdout.write(f'Firmas pendientes: {stats["pending_signatures"]}')
            self.stdout.write(
                f'Recordatorios de renovación: {stats["renewal_reminders"]}'
            )
            self.stdout.write(
                f'Total de notificaciones: {stats["total_notifications"]}'
            )
            ...
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(
                        "MODO DE PRUEBA: No se enviaron notificaciones reales"
                    )
                )
    ...
    def _process_pending_signatures(self, dry_run=False, verbose=False):
        ...
        for contract in contracts:
            ...
            signed_users = ContractSignature.objects.filter(
                contract=contract, is_valid=True
            ).values_list("signer_id", flat=True)

            signatories = [contract.primary_party, contract.secondary_party]
            pending_signers = [
                user for user in signatories if user.id not in signed_users
            ]
