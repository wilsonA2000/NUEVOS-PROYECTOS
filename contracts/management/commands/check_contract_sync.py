...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        lcc_ids = set(str(pk) for pk in LandlordControlledContract.objects.values_list('id', flat=True))
        contract_ids = set(str(pk) for pk in Contract.objects.values_list('id', flat=True))
        ...
        if lcc_without_contract:
            self.stdout.write(style.WARNING(
                f'⚠ {len(lcc_without_contract)} LCC sin Contract legacy:'))
        ...
        if contract_without_lcc:
            self.stdout.write(style.WARNING(
                f'⚠ {len(contract_without_lcc)} Contract sin LCC:'))
