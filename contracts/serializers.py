        ...
        try:
            if hasattr(obj, "property") and obj.property:
                ...
                return {
                    ...
                    "rent_price": float(property_obj.rent_price)
                    if getattr(property_obj, "rent_price", None)
                    else 0,
                    "main_image": property_obj.main_image.url
                    if getattr(property_obj, "main_image", None)
                    else None,
                    ...
                    if getattr(property_obj, "area", None)
                    else 0,
                }
...
class ColombianContractSerializer(serializers.ModelSerializer):
    ...
    match_code = serializers.CharField(
        source="match_request.match_code", read_only=True
    )
    property_title = serializers.CharField(
        source="match_request.property.title", read_only=True
    )
    tenant_name = serializers.CharField(
        source="match_request.tenant.get_full_name", read_only=True
    )
    landlord_name = serializers.CharField(
        source="match_request.property.landlord.get_full_name", read_only=True
    )
    ...
    def get_can_be_signed(self, obj):
        ...
        return (
            obj.status in ["PENDING_SIG", "PARTIAL_SIG"]
            and obj.landlord_verified
            and obj.tenant_verified
        )
    ...
    def validate_match_request(self, value):
        ...
        if value.status != "accepted":
            raise serializers.ValidationError(
                "Solo se pueden crear contratos desde matches aceptados"
            )
        if hasattr(value, "contract"):
            raise serializers.ValidationError(
                "Este match ya tiene un contrato asociado"
            )
        return value
...
class ContractValidationSerializer(serializers.Serializer):
    ...
    estimated_monthly_cost = serializers.DecimalField(
...
class DocumentCaptureRequestSerializer(serializers.Serializer):
    ...
    document_type = serializers.ChoiceField(
        choices=BiometricAuthentication.DOCUMENT_TYPES, default="cedula_ciudadania"
    )
...
class CombinedCaptureRequestSerializer(serializers.Serializer):
    ...
    combined_image = serializers.CharField(
        help_text="Imagen del documento junto al rostro en base64"
    )
