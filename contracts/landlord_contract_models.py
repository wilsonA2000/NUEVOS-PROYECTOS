...
class LandlordControlledContract(models.Model):
    ...
    **Ver `docs/CONTRACT_ARCHITECTURE.md`** — VeriHome tiene 3 modelos
    ...
    WORKFLOW_STATES = [
        ...
        ),  # Revisión obligatoria del abogado
        ...
        (
            "TENANT_RETURNED",
            "Devuelto por Arrendatario",
        ),  # Arrendatario devuelve para corrección
        (
            "LANDLORD_CORRECTING",
            "Arrendador Corrigiendo",
        ),  # Arrendador modifica tras devolución
        (
            "RE_PENDING_ADMIN",
            "Re-enviado a Revisión Admin",
        ),  # Re-revisión por abogado (ciclo)
        # === FIN NUEVOS ESTADOS ===
    ...
    has_pending_objections = models.BooleanField(
        "Tiene objeciones pendientes", default=False
    )
    objections_count = models.PositiveIntegerField("Número de objeciones", default=0)
    last_objection_date = models.DateTimeField(
        "Fecha de última objeción", null=True, blank=True
    )
    ...
    landlord_approved_at = models.DateTimeField(
        "Fecha de aprobación del arrendador", null=True, blank=True
    )
    ...
    invitation_sent_at = models.DateTimeField(
        "Invitación enviada", null=True, blank=True
    )
    invitation_expires_at = models.DateTimeField(
        "Invitación expira", null=True, blank=True
    )
    ...
    invitation_accepted_at = models.DateTimeField(
        "Invitación aceptada", null=True, blank=True
    )
    ...
    tenant_signed_at = models.DateTimeField(
        "Fecha de firma del arrendatario", null=True, blank=True
    )
    ...
    landlord_signed_at = models.DateTimeField(
        "Fecha de firma del arrendador", null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            self.current_state in ("PENDING_ADMIN_REVIEW", "RE_PENDING_ADMIN")
            and not self.admin_review_deadline
        ):
            from datetime import timedelta
    ...
    def _generate_contract_number(self) -> str:
        ...
        yearly_count = (
            LandlordControlledContract.objects.filter(created_at__year=year).count() + 1
        )

        return f"VH-{year}-{yearly_count:06d}"

    def add_workflow_entry(
        self, action: str, user: User, details: Dict[str, Any] = None
    ):
        ...
        description = (
            (details or {}).get("description") or action or "Evento de workflow"
        )
    ...
    def can_transition_to(self, new_state: str) -> bool:
        ...
        valid_transitions = {
            ...
            "LANDLORD_CORRECTING": [
                "RE_PENDING_ADMIN",
                "CANCELLED",
            ],  # 🔄 NUEVO: Arrendador re-envía
            "RE_PENDING_ADMIN": [
                "DRAFT",
                "CANCELLED",
            ],  # 🔄 NUEVO: Admin re-aprueba → DRAFT (ciclo)
            # === FLUJO EXISTENTE ===
            "OBJECTIONS_PENDING": [
    ...
    def get_missing_data_summary(self) -> Dict[str, List[str]]:
        ...
        missing = {"landlord": [], "tenant": [], "contract": []}
    ...
    def return_to_landlord(self, tenant_user: User, notes: str) -> bool:
        ...
        if not notes or len(notes.strip()) < 10:
            raise ValidationError(
                "Debe proporcionar notas detalladas (mínimo 10 caracteres)"
            )

        if tenant_user != self.tenant:
            raise ValidationError(
                "Solo el arrendatario asignado puede devolver el contrato"
            )
    ...
    def start_landlord_correction(self, landlord_user: User) -> bool:
        ...
        if landlord_user != self.landlord:
            raise ValidationError(
                "Solo el arrendador propietario puede corregir el contrato"
            )
    ...
    def resubmit_for_admin_review(
        self, landlord_user: User, changes_summary: str = ""
    ) -> bool:
        """
        ...
        if landlord_user != self.landlord:
            raise ValidationError(
                "Solo el arrendador propietario puede re-enviar para revisión"
            )
...
class ContractObjection(models.Model):
    ...
    contract = models.ForeignKey(
        LandlordControlledContract, on_delete=models.CASCADE, related_name="objections"
    )

    # Información de la objeción
    objection_type = models.CharField(
        "Tipo de objeción", max_length=20, choices=OBJECTION_TYPES, default="OTHER"
    )
    ...
    current_value = models.TextField(
        "Valor actual", help_text="Valor actual del campo objetado"
    )
    objection_text = models.TextField(
        "Descripción de la objeción", help_text="Explicación detallada de la objeción"
    )
    proposed_modification = models.TextField(
        "Modificación propuesta", blank=True, help_text="Propuesta específica de cambio"
    )

    # Estado y seguimiento
    status = models.CharField(
        "Estado", max_length=20, choices=OBJECTION_STATUS, default="PENDING"
    )
...
class LandlordContractGuarantee(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    contract = models.ForeignKey(
        LandlordControlledContract, on_delete=models.CASCADE, related_name="guarantees"
    )

    # Información básica de la garantía
    guarantee_type = models.CharField(
        "Tipo de garantía", max_length=25, choices=GUARANTEE_TYPES
    )
    title = models.CharField(
    ...
    description = models.TextField(
        "Descripción detallada", help_text="Descripción completa de la garantía"
    )
    ...
    currency = models.CharField(
        "Moneda", max_length=3, default="COP", help_text="Código ISO de la moneda"
    )
    ...
    # Estado y control
    status = models.CharField(
        "Estado", max_length=15, choices=GUARANTEE_STATUS, default="PROPOSED"
    )
...
class ContractInvitation(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    contract = models.ForeignKey(
        LandlordControlledContract, on_delete=models.CASCADE, related_name="invitations"
    )
    ...
    invitation_method = models.CharField(
        max_length=10, choices=METHOD_CHOICES, default="email"
    )
...
class ContractWorkflowHistory(models.Model):
    ...
    old_state = models.CharField("Estado anterior", max_length=30, blank=True)
    new_state = models.CharField("Nuevo estado", max_length=30, blank=True)
    ...
    user_agent = models.TextField(
        "User Agent", blank=True, help_text="Información del navegador/dispositivo"
    )
    session_id = models.CharField("ID de sesión", max_length=255, blank=True)
...
class CodeudorAuthToken(models.Model):
    ...
    codeudor_phone = models.CharField(
        "Teléfono del Codeudor", max_length=20, blank=True
    )
    ...
    def is_valid(self):
        ...
        return not self.is_expired and self.status not in [
            "completed",
            "expired",
            "failed",
        ]
