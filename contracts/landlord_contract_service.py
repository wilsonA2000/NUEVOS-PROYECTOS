...
class LandlordContractService:
    ...
    def create_contract_draft(
        ...
        with transaction.atomic():
            ...
            self.logger.info(
                f"Contrato {contract.contract_number} creado por {landlord.email}"
            )
            return contract

    def complete_landlord_data(
        self, contract_id: uuid.UUID, landlord: User, landlord_data: Dict
        ...
        with transaction.atomic():
            ...
            if contract.current_state != "DRAFT":
                raise ValidationError(
                    f"No se pueden completar datos en estado {contract.current_state}"
                )
            ...
            self.logger.info(
                f"Datos del arrendador completados para contrato {contract.contract_number}"
            )
        ...
        with transaction.atomic():
            ...
            if success:
                ...
                NotificationService.notify_tenant_invited(
                    tenant=temp_user, contract=contract, landlord=landlord
                )

            return success

    def accept_tenant_invitation(
        self, invitation_token: str, tenant: User
    ) -> LandlordControlledContract:
        """
        ...
        with transaction.atomic():
            ...
            self.logger.info(
                f"Invitación aceptada por {tenant.email} para contrato {contract.contract_number}"
            )
            return contract

    def complete_tenant_data(
        self, contract_id: uuid.UUID, tenant: User, tenant_data: Dict
    ) -> LandlordControlledContract:
        """
    ...
    def submit_objection(
        ...
        with transaction.atomic():
            ...
            if contract.current_state not in [
                ...
                raise ValidationError(
                    "No se pueden presentar objeciones en el estado actual"
                )
            ...
            other_party = (
                contract.tenant if user == contract.landlord else contract.landlord
            )
        ...
        with transaction.atomic():
            ...
            if user not in [contract.landlord, contract.tenant]:
                raise PermissionDenied(
                    "No tienes permisos para responder esta objeción"
                )
    ...
    def approve_contract(
        self, contract_id: uuid.UUID, user: User
    ) -> LandlordControlledContract:
        """
        ...
        with transaction.atomic():
            ...
            if user == contract.landlord:
                ...
                raise PermissionDenied(
                    "Usuario no autorizado para aprobar este contrato"
                )
    ...
    def record_digital_signature(
        self, contract_id: uuid.UUID, user: User, signature_data: Dict
    ) -> LandlordControlledContract:
        """
        ...
        with transaction.atomic():
            ...
            if user == contract.landlord:
                ...
                raise PermissionDenied(
                    "Usuario no autorizado para firmar este contrato"
                )
    ...
    def publish_contract(
        self, contract_id: uuid.UUID, landlord: User
    ) -> LandlordControlledContract:
        """
        ...
        with transaction.atomic():
            ...
            if contract.current_state != "FULLY_SIGNED":
                raise ValidationError(
                    "Solo se pueden publicar contratos completamente firmados"
                )
            ...
            self.logger.info(
                f"Contrato {contract.contract_number} publicado exitosamente"
            )
            return contract
    ...
    def _get_contract_for_landlord(
        self, contract_id: uuid.UUID, landlord: User
    ) -> LandlordControlledContract:
    ...
    def _get_contract_for_tenant(
        self, contract_id: uuid.UUID, tenant: User
    ) -> LandlordControlledContract:
    ...
    def _get_contract_for_user(
        self, contract_id: uuid.UUID, user: User
    ) -> LandlordControlledContract:
        """Obtener contrato verificando que el usuario es parte del mismo."""
        try:
            return (
                LandlordControlledContract.objects.filter(id=contract_id)
                .filter(models.Q(landlord=user) | models.Q(tenant=user))
                .get()
            )
        except LandlordControlledContract.DoesNotExist:
            raise PermissionDenied("Contrato no encontrado o sin permisos")
    ...
    def _apply_objection_change(
        self, contract: LandlordControlledContract, objection: ContractObjection
    ) -> None:
    ...
    def _send_invitation_email(
        ...
        try:
            subject = (
                f"Invitación a contrato de arrendamiento - {contract.contract_number}"
            )
            ...
            html_content = render_to_string(
                "contracts/emails/tenant_invitation.html", context
            )
            text_content = render_to_string(
                "contracts/emails/tenant_invitation.txt", context
            )
    ...
    def _notify_landlord_tenant_completed(
        self, contract: LandlordControlledContract
    ) -> None:
        ...
        NotificationService.notify_contract_accepted_by_tenant(
            landlord=contract.landlord, contract=contract, tenant=contract.tenant
        )

    def _notify_objection_submitted(
        self,
        contract: LandlordControlledContract,
        objection: ContractObjection,
        recipient: User,
    ) -> None:
    ...
    def _notify_fully_signed(self, contract: LandlordControlledContract) -> None:
        ...
        NotificationService.notify_contract_fully_signed(
            user=contract.landlord, contract=contract
        )

        NotificationService.notify_contract_fully_signed(
            user=contract.tenant, contract=contract
        )
    ...
    def _notify_contract_published(self, contract: LandlordControlledContract) -> None:
        ...
        NotificationService.notify_contract_published(
            tenant=contract.tenant, contract=contract, landlord=contract.landlord
        )
