...
class TenantContractListSerializer(serializers.ModelSerializer):
    ...
    landlord_phone = serializers.CharField(
        source="landlord.profile.phone", read_only=True
    )
    ...
    def get_security_deposit(self, obj):
        return (
            obj.economic_terms.get("security_deposit", 0) if obj.economic_terms else 0
        )

    def get_contract_duration_months(self, obj):
        return (
            obj.contract_terms.get("contract_duration_months", 12)
    ...
    def get_utilities_included(self, obj):
        return (
            obj.contract_terms.get("utilities_included", False)
            if obj.contract_terms
            else False
        )
    ...
    def get_pets_allowed(self, obj):
        return (
            obj.contract_terms.get("pets_allowed", False)
            if obj.contract_terms
            else False
        )
    ...
    def get_smoking_allowed(self, obj):
        return (
            obj.contract_terms.get("smoking_allowed", False)
            if obj.contract_terms
            else False
        )
...
class TenantContractDetailSerializer(serializers.ModelSerializer):
    ...
    def get_landlord_details(self, obj):
        ...
        if obj.landlord:
            ...
            return {
                ...
                "emergency_contact_name": landlord_data.get(
                    "emergency_contact_name", ""
                ),
                "emergency_contact_phone": landlord_data.get(
    ...
    def get_property_details(self, obj):
        ...
        if obj.property:
            return {
                ...
                "amenities": [amenity.name for amenity in obj.property.amenities.all()]
                if hasattr(obj.property, "amenities")
                else [],
            }
        return None
    ...
    def get_next_action_required(self, obj):
        ...
        actions = {
            ...
            "BOTH_REVIEWING": {
                ...
                if not obj.tenant_approved
                else "wait_landlord",
                "title": "Aprobar contrato"
                if not obj.tenant_approved
                else "Esperando al arrendador",
                "description": "Revisa y aprueba los términos finales del contrato"
                if not obj.tenant_approved
                else "El arrendador debe aprobar también",
                "priority": "high" if not obj.tenant_approved else "medium",
            },
            "READY_TO_SIGN": {
                "action": "sign_contract"
                if not obj.tenant_signed
                else "wait_landlord_sign",
                "title": "Firmar contrato"
                if not obj.tenant_signed
                else "Esperando firma del arrendador",
                "description": "Firma digitalmente el contrato"
                if not obj.tenant_signed
                else "El arrendador debe firmar también",
    ...
    def get_tenant_capabilities(self, obj):
        ...
        return {
            ...
            "can_create_objections": obj.current_state
            in ["LANDLORD_REVIEWING", "TENANT_REVIEWING", "OBJECTIONS_PENDING"],
            "can_respond_objections": False,  # Deshabilitado - modelo no soporta objected_by
            "can_approve": obj.current_state == "BOTH_REVIEWING"
            and not obj.tenant_approved,
            "can_sign": obj.current_state == "READY_TO_SIGN" and not obj.tenant_signed,
            "can_view_contract": obj.current_state
            in ["BOTH_REVIEWING", "READY_TO_SIGN", "FULLY_SIGNED", "PUBLISHED"],
        }
...
class TenantDataSerializer(serializers.Serializer):
    ...
    time_at_current_address = serializers.IntegerField(
        min_value=0, help_text="Meses en la dirección actual"
    )
    ...
    work_reference_name = serializers.CharField(
        max_length=200, required=False, allow_blank=True
    )
    work_reference_phone = serializers.CharField(
        max_length=20, required=False, allow_blank=True
    )
    work_reference_position = serializers.CharField(
    ...
    pets_description = serializers.CharField(
...
class TenantContractApprovalSerializer(serializers.Serializer):
    ...
    tenant_notes = serializers.CharField(
        max_length=1000, required=False, allow_blank=True
    )
    ...
    def validate_confirm_understanding(self, value):
        if not value:
            raise serializers.ValidationError(
                "Debes confirmar que entiendes los términos del contrato"
            )
        return value
...
class TenantContractSignatureSerializer(serializers.Serializer):
    ...
    def validate_accept_legal_responsibility(self, value):
        if not value:
            raise serializers.ValidationError(
                "Debes aceptar la responsabilidad legal de la firma"
            )
        return value
