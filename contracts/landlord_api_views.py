...
class LandlordContractViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            LandlordControlledContract.objects.filter(landlord=self.request.user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            self._record_contract_creation_history(
                contract, request.user, serializer.validated_data
            )
            ...
            response_data = {
                ...
                "has_guarantee_terms": bool(
                    contract.contract_terms.get("guarantor_required")
                ),
                ...
                "contract_duration_months": contract.contract_terms.get(
    ...
    def complete_landlord_data(self, request, pk=None):
        ...
        try:
            ...
            response_data = LandlordControlledContractDetailSerializer(
                updated_contract
            ).data
    ...
    def publish_contract(self, request, pk=None):
        ...
        try:
            ...
            published_contract = service.publish_contract(
                contract_id=contract.id, landlord=request.user
            )
    ...
    def stats(self, request):
        ...
        contracts_with_objections = (
            queryset.filter(objections__isnull=False).distinct().count()
        )

        pending_signatures = queryset.filter(current_state="READY_TO_SIGN").count()

        # Ingresos mensuales
        monthly_revenue = queryset.filter(published=True).aggregate(
            total=models.Sum("monthly_rent")
        )["total"] or Decimal("0.00")

        # Promedio de días para completar
        completed_contracts_qs = queryset.filter(published_at__isnull=False).annotate(
        ...
        avg_completion_days = avg_completion.days if avg_completion else 0
            ...
            if guarantee_id:
                try:
                    guarantee = LandlordContractGuarantee.objects.get(
                        id=guarantee_id, contract=contract
                    )
    ...
    def _send_codeudor_invitation_email(self, auth_token, auth_url):
        ...
        try:
            ...
            context = {
                ...
                "landlord_name": auth_token.created_by.get_full_name()
                or auth_token.created_by.email,
                "property_address": auth_token.contract.property_data.get(
                    "address", "No disponible"
                ),
                "auth_url": auth_url,
                "expires_at": auth_token.expires_at.strftime("%d/%m/%Y %H:%M"),
                "personal_message": auth_token.personal_message,
                "codeudor_type_display": "Codeudor con Salario"
                if auth_token.codeudor_type == "codeudor_salario"
                else "Codeudor con Finca Raíz",
            }

            # Intentar usar template HTML, si no existe usar texto plano
            try:
                html_message = render_to_string(
                    "emails/codeudor_invitation.html", context
                )
            except Exception:
                html_message = None
...
VeriHome - Plataforma de Gestión Inmobiliaria
            ...
            logger.info(
                f"Email de invitación enviado a codeudor: {auth_token.codeudor_email}"
            )
            return True
    ...
    def list_codeudor_tokens(self, request, pk=None):
        ...
        tokens = CodeudorAuthToken.objects.filter(contract=contract).order_by(
            "-created_at"
        )
                ...
                "tokens": [
                    {
                        ...
                        "status": t.status,
                        "status_display": dict(CodeudorAuthToken.STATUS_CHOICES).get(
                            t.status, t.status
                        ),
                        "is_valid": t.is_valid,
    ...
    @action(
        detail=True,
        methods=["post"],
        url_path="resend-codeudor-invitation/(?P<token_id>[^/.]+)",
    )
    def resend_codeudor_invitation(self, request, pk=None, token_id=None):
        ...
        try:
            ...
            return Response(
                {"error": "Token no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def generate_pdf(self, request, pk=None):
        ...
        try:
            # Parámetros de query
            include_signatures = (
                request.query_params.get("include_signatures", "true").lower() == "true"
            )
            include_biometric = (
                request.query_params.get("include_biometric", "true").lower() == "true"
            )
    ...
    def download_pdf(self, request, pk=None):
        ...
        try:
            ...
            response = HttpResponse(pdf_file.read(), content_type="application/pdf")
            response["Content-Disposition"] = (
                f'attachment; filename="contrato_firmado_{contract.contract_number}.pdf"'
            )
            response["Content-Length"] = (
                pdf_file.size if hasattr(pdf_file, "size") else len(pdf_file.read())
            )
    ...
    def preview_pdf(self, request, pk=None):
        ...
        try:
            ...
            pdf_file = generate_contract_pdf(
                contract, include_signatures=False, include_biometric=False
            )

            # Respuesta para visualización en línea
            response = HttpResponse(pdf_file.read(), content_type="application/pdf")
            response["Content-Disposition"] = (
                f'inline; filename="preview_contrato_{contract.contract_number}.pdf"'
            )
            ...
            logger.error(
                f"Error generando preview PDF para contrato {contract.id}: {str(e)}"
            )
            return Response(
            ...
            return Response(
                {
                    ...
                    "tenant_return_notes": contract.tenant_return_notes,
                    "last_return_date": contract.last_return_date.isoformat()
    ...
    def reopen_negotiation(self, request, pk=None):
        ...
        contract.save(
            update_fields=["current_state", "tenant_approved", "landlord_approved"]
        )
    ...
    def resubmit_for_admin_review(self, request, pk=None):
        ...
        try:
            ...
            success = contract.resubmit_for_admin_review(
                landlord_user=request.user, changes_summary=changes_summary
            )
            ...
            try:
                ...
                if match_request:
                    ...
                    match_request.save()
                    logger.info(
                        f"✅ MatchRequest {match_request.id} updated - contract resubmitted for admin review"
                    )
            except Exception as e:
        ...
        circular_qs = contract.history_entries.filter(
            ...
            | Q(
                metadata__legacy_event_type__in=[
                    "TENANT_RETURN",
                    "LANDLORD_START_CORRECTION",
                    "RESUBMIT_FOR_ADMIN_REVIEW",
                    "SPECIFIC_CONCERNS_ADDED",
                    "DETAILED_CHANGES_ADDED",
                    "tenant_return",
                    "landlord_correction_start",
                    "resubmit_for_review",
                ]
            )
        ).order_by("-timestamp")
        ...
        return Response(
            {
                ...
                "locked_info": {
                    ...
                    "locked_reason": contract.locked_reason,
                    "locked_by": contract.locked_by.email
                    if contract.locked_by
                    else None,
                }
                if contract.is_locked
                else None,
                ...
                if contract.tenant_return_notes
                else None,
...
class ContractObjectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).select_related("contract", "objected_by", "responded_by")
...
class ContractModificationRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .select_related("contract", "requested_by")
            .prefetch_related("contract__landlord", "contract__tenant")
        )
    ...
    def respond(self, request, pk=None):
        ...
        try:
            ...
            try:
                ...
                landlord_response = serializer.validated_data.get(
                    "landlord_response", ""
                )
            ...
            response_serializer = ContractModificationRequestSerializer(
