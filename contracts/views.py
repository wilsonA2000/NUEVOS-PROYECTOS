...
class ContractDetailView(LoginRequiredMixin, DetailView):
        ...
        context.update(
            {
                ...
                "landlord_id": getattr(
                    contract.primary_party, "identification_number", ""
                ),
                ...
                "tenant_id": getattr(
                    contract.secondary_party, "identification_number", ""
                ),
                "tenant_occupation": getattr(contract.secondary_party, "job_title", ""),
                "tenant_income": getattr(
                    contract.secondary_party, "monthly_income", ""
                ),
                ...
                "property_type": contract.property.get_property_type_display()
                if contract.property
                else "N/A",
                "property_bedrooms": contract.property.bedrooms
                if contract.property
                else 0,
                "property_bathrooms": contract.property.bathrooms
                if contract.property
                else 0,
                "property_area": contract.property.total_area
                if contract.property
                else 0,
                "property_furnished": "amueblada"
                if (contract.property and contract.property.furnished)
                else "sin amoblar",
                "pets_allowed": "permitidas"
                if (contract.property and contract.property.pets_allowed)
                else "no permitidas",
                # Datos financieros
                "monthly_rent": contract.monthly_rent,
                "security_deposit": contract.security_deposit,
                "maintenance_fee": getattr(contract.property, "maintenance_fee", 0)
                if contract.property
                else 0,
                "minimum_lease_term": getattr(
                    contract.property, "minimum_lease_term", 12
                )
                if contract.property
                else 12,
                # Fechas
                ...
                "utilities_included": ", ".join(contract.property.utilities_included)
                if (contract.property and contract.property.utilities_included)
                else "Ninguno incluido",
                "property_features": ", ".join(contract.property.property_features)
                if (contract.property and contract.property.property_features)
                else "",
                "nearby_amenities": ", ".join(contract.property.nearby_amenities)
                if (contract.property and contract.property.nearby_amenities)
                else "",
            }
        )
    ...
    def _generate_dynamic_clauses(self, contract, template_context):
        ...
        clause_context = {
            ...
            "pets_permission": "podrá"
            if (contract.property and contract.property.pets_allowed)
            else "no podrá",
            "pet_deposit": "$500,000",
            "parking_spaces": str(getattr(contract.property, "parking_spaces", 0))
            if contract.property
            else "0",
            "parking_numbers": "por asignar",
            "furniture_status": "amueblado"
            if (contract.property and contract.property.furnished)
            else "sin amoblar",
...
class DownloadContractPDFView(LoginRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {
            ...
            "minimum_lease_term": getattr(contract.property, "minimum_lease_term", 12)
            if contract.property
            else 12,
            # Fechas
            "start_date": contract.start_date,
        ...
        html_content = render_to_string(
            "contracts/professional_contract_template.html", context
        )

        # Retornar como HTML para previsualización (para PDF usar weasyprint o similar)
        response = HttpResponse(html_content, content_type="text/html")
        response["Content-Disposition"] = (
            f'inline; filename="contrato_{contract.contract_number}.html"'
        )
