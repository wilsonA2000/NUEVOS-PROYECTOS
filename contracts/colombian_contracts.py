...
class ColombianContractType(models.TextChoices):
    ...
    ARRENDAMIENTO_VIVIENDA_URBANA = (
        "ARR_VIV_URB",
        "Contrato de Arrendamiento de Vivienda Urbana (Ley 820 de 2003)",
    )
    ARRENDAMIENTO_LOCAL_COMERCIAL = (
        "ARR_COM",
        "Contrato de Arrendamiento de Local Comercial",
    )
    PRESTACION_SERVICIOS = (
        "PREST_SERV",
        "Contrato de Prestación de Servicios (Art. 1495 CC)",
    )
...
class LegalClause(models.Model):
    ...
    contract_type = models.CharField(
        max_length=20, choices=ColombianContractType.choices
    )
    ...
    legal_reference = models.CharField(
        max_length=200, help_text="Referencia legal (ej: Art. 1973 CC, Ley 820/2003)"
    )
...
class ColombianContract(models.Model):
    ...
    **Ver `docs/CONTRACT_ARCHITECTURE.md`** — uso aislado: sólo lo
    ...
    landlord_id_type = models.CharField(
        max_length=20,
        choices=[
    ...
    tenant_id_type = models.CharField(
        max_length=20,
        choices=[
            ("CC", "Cédula de Ciudadanía"),
            ("CE", "Cédula de Extranjería"),
    ...
    payment_due_day = models.IntegerField(
        default=5, help_text="Día del mes para pago de canon"
    )
    ...
    property_inventory = models.JSONField(
        default=list, help_text="Lista detallada de elementos del inmueble"
    )
    ...
    created_by = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="contracts_created"
    )

    # Documentos adjuntos
    documents = models.JSONField(
        default=dict, help_text="URLs de documentos: certificados, paz y salvos, etc."
    )
        ...
        if self.contract_type == ColombianContractType.ARRENDAMIENTO_VIVIENDA_URBANA:
            ...
            if self.security_deposit > self.monthly_rent * 2:
                raise ValidationError(
                    "El depósito no puede exceder 2 meses de arriendo"
                )
    ...
    def generate_legal_clauses(self):
        ...
        mandatory_clauses = LegalClause.objects.filter(
            contract_type=self.contract_type, is_mandatory=True
    ...
    def can_be_signed(self):
        ...
        return all(
            [
                self.status
                in [ContractStatus.PENDING_SIGNATURES, ContractStatus.PARTIALLY_SIGNED],
...
class ContractMilestone(models.Model):
    ...
    contract = models.ForeignKey(
        ColombianContract, on_delete=models.CASCADE, related_name="milestones"
    )
...
class ContractGuarantee(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    contract = models.ForeignKey(
        ColombianContract, on_delete=models.CASCADE, related_name="guarantees"
    )
