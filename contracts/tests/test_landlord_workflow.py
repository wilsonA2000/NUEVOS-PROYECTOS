...
class LandlordContractWorkflowTest(TestCase):
    ...
    def test_responsible_party_landlord_states(self):
        ...
        for state in ["DRAFT", "LANDLORD_COMPLETING", "LANDLORD_CORRECTING"]:
            ...
            self.assertEqual(
                contract.get_current_responsible_party(),
                "landlord",
    ...
    def test_add_workflow_entry(self):
        ...
        """
        contract = self._create_contract(state="DRAFT")
        entries_before = ContractWorkflowHistory.objects.filter(
            contract=contract
        ).count()
        ...
        entries_after = ContractWorkflowHistory.objects.filter(
            contract=contract
        ).count()
        self.assertEqual(entries_after - entries_before, 1)
        entry = (
            ContractWorkflowHistory.objects.filter(contract=contract)
            .order_by("-timestamp")
            .first()
        )
