...
class ContractTemplate(models.Model):
    ...
    template_type = models.CharField(
        "Tipo de plantilla", max_length=20, choices=TEMPLATE_TYPES
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="contract_templates"
    )
...
class Contract(models.Model):
    ...
    **Ver `docs/CONTRACT_ARCHITECTURE.md`** — VeriHome tiene 3 modelos
    ...
    contract_type = models.CharField(
        "Tipo de contrato", max_length=20, choices=CONTRACT_TYPES
    )
    ...
    status = models.CharField(
        "Estado", max_length=50, choices=STATUS_CHOICES, default="draft"
    )
    ...
    auto_renewal_notice_days = models.PositiveIntegerField(
        "Días de aviso para renovación automática", default=30
    )
    ...
    tenant_approved_at = models.DateTimeField(
        "Fecha de Aprobación del Arrendatario", null=True, blank=True
    )
    ...
    monthly_rent = models.DecimalField(
        "Renta mensual", max_digits=12, decimal_places=2, null=True, blank=True
    )
    security_deposit = models.DecimalField(
        "Depósito de garantía", max_digits=12, decimal_places=2, null=True, blank=True
    )
    late_fee = models.DecimalField(
        "Recargo por mora", max_digits=8, decimal_places=2, null=True, blank=True
    )
    ...
    tenant_reviewed_at = models.DateTimeField(
        "Fecha de Revisión del Arrendatario", null=True, blank=True
    )

    # Control de autenticación biométrica
    landlord_auth_completed = models.BooleanField(
        "Autenticación del Arrendador Completada", default=False
    )
    landlord_auth_date = models.DateTimeField(
        "Fecha de Autenticación del Arrendador", null=True, blank=True
    )
    tenant_auth_completed = models.BooleanField(
        "Autenticación del Arrendatario Completada", default=False
    )
    tenant_auth_date = models.DateTimeField(
        "Fecha de Autenticación del Arrendatario", null=True, blank=True
    )

    # Control de entrega de llaves
    keys_delivered = models.BooleanField("Llaves Entregadas", default=False)
    keys_delivery_date = models.DateTimeField(
        "Fecha de Entrega de Llaves", null=True, blank=True
    )
    keys_delivery_notes = models.TextField("Notas de Entrega", blank=True)
    execution_started_at = models.DateTimeField(
        "Fecha de Inicio de Ejecución", null=True, blank=True
    )

    # Campos para PDF y descarga
    pdf_file = models.FileField(
        "Archivo PDF del contrato", upload_to="contracts/pdfs/", null=True, blank=True
    )
    pdf_generated_at = models.DateTimeField(
        "Fecha de generación PDF", null=True, blank=True
    )
            ...
            try:
                with transaction.atomic():
                    ...
                    if self.status in ["published", "active"]:
                        self.match_request.status = (
                            "completed" if self.status == "active" else "accepted"
                        )
                        ...
                        self.match_request.workflow_status = (
                            "contract_signed"
        ...
        if self.status in _lcc_state_map:
            try:
                ...
                logging.getLogger(__name__).error(
                    f"Error sincronizando LCC dual workflow: {e}"
                )
    ...
    def get_next_biometric_step(self):
        ...
        if not self.tenant_auth_completed:
            ...
            self.guarantor
            and not BiometricAuthentication.objects.filter(
...
class ContractSignature(models.Model):
    ...
    signer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="contract_signatures"
    )

    # Información de la firma
    signature_type = models.CharField(
        "Tipo de firma", max_length=20, choices=SIGNATURE_TYPES
    )
    authentication_method = models.CharField(
        "Método de autenticación", max_length=30, choices=AUTHENTICATION_METHODS
    )
    signature_data = models.TextField(
        "Datos de la firma", help_text="Firma codificada o hash"
    )
    signature_image = models.ImageField(
        "Imagen de la firma", upload_to="signatures/", null=True, blank=True
    )
    ...
    verification_level = models.CharField(
        ...
        max_length=20,
        choices=[("basic", "Básica"), ("enhanced", "Mejorada"), ("maximum", "Máxima")],
        default="basic",
    )
    ...
    def verify_signature(self):
        ...
        # Verificar hash
        data_to_hash = (
            f"{self.contract.id}{self.signer.id}{self.signed_at}{self.signature_data}"
        )
...
class ContractAmendment(models.Model):
    ...
    # Estado y aprobación
    status = models.CharField(
        "Estado", max_length=20, choices=AMENDMENT_STATUS, default="draft"
    )
    requested_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="requested_amendments"
    )
...
class ContractTermination(models.Model):
    ...
    contract = models.OneToOneField(
        Contract, on_delete=models.CASCADE, related_name="termination"
    )

    # Información de la terminación
    reason = models.CharField(
        "Razón de terminación", max_length=30, choices=TERMINATION_REASONS
    )
    description = models.TextField("Descripción detallada", max_length=1000)
    requested_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="requested_terminations"
    )
    ...
    penalty_amount = models.DecimalField(
        "Monto de penalización", max_digits=12, decimal_places=2, null=True, blank=True
    )
    refund_amount = models.DecimalField(
        "Monto de reembolso", max_digits=12, decimal_places=2, null=True, blank=True
    )
    final_payment_due = models.DecimalField(
        "Pago final adeudado", max_digits=12, decimal_places=2, null=True, blank=True
    )
...
class ContractRenewal(models.Model):
    ...
    original_contract = models.ForeignKey(
        Contract, on_delete=models.CASCADE, related_name="renewals"
    )
    ...
    new_monthly_rent = models.DecimalField(
        "Nueva renta mensual", max_digits=12, decimal_places=2, null=True, blank=True
    )
    ...
    # Estado y seguimiento
    status = models.CharField(
        "Estado", max_length=20, choices=RENEWAL_STATUS, default="pending"
    )
    requested_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="requested_renewals"
    )
...
class BiometricAuthentication(models.Model):
    ...
    # Relaciones
    contract = models.ForeignKey(
        Contract, on_delete=models.CASCADE, related_name="biometric_authentications"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="biometric_authentications"
    )

    # Estado de autenticación
    status = models.CharField(
        "Estado", max_length=20, choices=AUTHENTICATION_STATUS, default="pending"
    )

    # Imágenes biométricas
    face_front_image = models.ImageField(
        "Foto frontal del rostro", upload_to="biometric/faces/", null=True, blank=True
    )
    face_side_image = models.ImageField(
        "Foto lateral del rostro", upload_to="biometric/faces/", null=True, blank=True
    )
    ...
    voice_recording = models.FileField(
        "Grabación de voz", upload_to="biometric/voice/", null=True, blank=True
    )
    ...
    voice_analysis = models.JSONField(
        "Análisis de voz", default=dict, help_text="Análisis de la grabación de voz"
    )

    # Puntuaciones de confianza
    face_confidence_score = models.FloatField(
        "Puntuación de confianza facial", default=0.0
    )
    document_confidence_score = models.FloatField(
        "Puntuación de confianza del documento", default=0.0
    )
    voice_confidence_score = models.FloatField(
        "Puntuación de confianza de voz", default=0.0
    )
    overall_confidence_score = models.FloatField(
        "Puntuación general de confianza", default=0.0
    )
...
class ContractDocument(models.Model):
    ...
    document_type = models.CharField(
        "Tipo de documento", max_length=20, choices=DOCUMENT_TYPES
    )
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="uploaded_contract_documents"
    )
    file_size = models.PositiveIntegerField(
        "Tamaño del archivo (bytes)", null=True, blank=True
    )
...
class ContractAdditionalClause(models.Model):
    ...
    contract = models.ForeignKey(
        Contract, on_delete=models.CASCADE, related_name="additional_clauses"
    )
    ...
    ordinal_text = models.CharField(
        "Texto Ordinal", max_length=50, help_text="Ej: UNDÉCIMA, DUODÉCIMA, etc."
    )

    # Metadatos
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="created_contract_clauses"
    )
    created_at = models.DateTimeField("Fecha de Creación", auto_now_add=True)
    updated_at = models.DateTimeField("Fecha de Actualización", auto_now=True)
    ...
    order = models.PositiveIntegerField(
        "Orden", default=0, help_text="Orden de aparición en el contrato"
    )
