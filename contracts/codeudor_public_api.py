...
class CodeudorTokenValidateView(APIView):
    ...
    def _generate_voice_text(self, auth_token, contract):
        ...
        codeudor_type_text = (
            "personal"
            if auth_token.codeudor_type == "codeudor_salario"
            else "con garantía de finca raíz"
        )
    ...
    def _get_or_create_legacy_contract(self, landlord_contract):
        ...
        try:
            ...
            if not contract and landlord_contract.tenant:
                ...
                logger.info(
                    f"Created legacy contract {contract.id} for codeudor biometric"
                )
    ...
    def post(self, request, token):
        try:
            ...
            missing_steps = [
                ...
                if not biometric_data.get(f"{s}_captured", False)
            ]
    ...
    def _update_contract_workflow(self, auth_token):
        ...
        try:
            ...
            if result:
                ...
                if landlord_contract.current_state in [
...
class CodeudorStatusView(APIView):
    ...
    def get(self, request, token):
        try:
            ...
            completed_steps = [
                s for s in required_steps if biometric_data.get(f"{s}_captured", False)
            ]
            ...
            return Response(
                {
                    ...
                    "status_display": dict(CodeudorAuthToken.STATUS_CHOICES).get(
                        auth_token.status, auth_token.status
                    ),
                    ...
                    "missing_steps": [
                        s for s in required_steps if s not in completed_steps
                    ],
                    "overall_confidence": biometric_data.get("overall_confidence"),
                    "completed_at": auth_token.completed_at.isoformat()
                    if auth_token.completed_at
                    else None,
                    "expires_at": auth_token.expires_at.isoformat()
