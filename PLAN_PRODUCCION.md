...
| D41 | **Doble beat schedule que se pisan**: `settings.CELERY_BEAT_SCHEDULE` y `verihome/celery.py` (`app.conf.beat_schedule = {...}`, línea 61) definen schedules DISTINTOS. `config_from_object` carga el de settings y luego celery.py lo SOBREESCRIBE entero (asignación, no merge) → **gana celery.py**. Consecuencia: tareas que solo están en settings NO se agendan: `check-admin-review-sla`, `check-biometric-expiration`, `check-payment-reminders`, `escalate-overdue-payments`. Reconciliar a UNA sola definición (mover todo a settings y borrar el bloque de celery.py) y verificar qué tareas deben correr realmente | D39 (2026-06-20) | 🟠 Media → antes del go-live: hay tareas de negocio (SLA admin, expiración biométrica, recordatorios de pago) que hoy no corren por beat |
