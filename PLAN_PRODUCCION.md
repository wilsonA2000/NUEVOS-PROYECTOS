...
| D14 | PropertyForm usa validación nativa del browser → tooltips "Please fill out this field" **en inglés** | 1.3 (2026-06-12) | ✅ Resuelta (verificado 2026-06-21): el `<form>` ya usa `noValidate` → sin tooltips nativos en inglés; la validación la maneja react-hook-form en español |
| D15 | PropertyForm exige `lot_area` (área de lote) **incluso para apartamentos** | 1.3 (2026-06-12) | ✅ Resuelta 2026-06-21: `required`/helperText de `lot_area` ahora condicional a `propertyType` (opcional en apartamentos, usando el `watch('property_type')` existente). tsc/build OK |
...
| D19 | Card del tenant muestra el workflow_status **crudo** ("documents_approved") en vez de label legible | 1.6 (2026-06-12) | ✅ Resuelta (verificado 2026-06-21): `workflowStatusLabel` map en TenantContractsDashboard traduce los slugs a español |
...
| D34 | `GuaranteeDocumentUpload.tsx` huérfano (sin imports vivos) con upload SIMULADO (`Math.random()` éxito/fallo) — código muerto a borrar | 2.4 (2026-06-12) | ✅ Resuelta (verificado 2026-06-21): el archivo ya no existe (borrado en limpieza de código muerto) |
