...
class DashboardWidget(models.Model):
    ...
    widget_type = models.CharField(
        "Tipo de Widget", max_length=30, choices=WIDGET_TYPES
    )
    ...
    size = models.CharField(
        "Tamaño", max_length=20, choices=WIDGET_SIZES, default="medium"
    )
    ...
    position_y = models.PositiveIntegerField(
        "Posición Y", default=0, validators=[MinValueValidator(0)]
    )
    width = models.PositiveIntegerField(
        "Ancho", default=2, validators=[MinValueValidator(1), MaxValueValidator(4)]
    )
    height = models.PositiveIntegerField(
        "Alto", default=1, validators=[MinValueValidator(1), MaxValueValidator(3)]
    )
    ...
    refresh_interval = models.IntegerField(
        "Intervalo de actualización (segundos)", choices=REFRESH_INTERVALS, default=300
    )
    data_source = models.CharField("Fuente de datos", max_length=100, blank=True)
    filter_config = models.JSONField(
        "Configuración de filtros", default=dict, blank=True
    )

    # Configuración de estilo
    color_scheme = models.CharField(
        "Esquema de colores", max_length=20, default="default"
    )
    ...
    last_data_update = models.DateTimeField(
        "Última actualización de datos", null=True, blank=True
    )
    ...
    def needs_refresh(self):
        ...
        return (
            timezone.now() - self.last_data_update
...
class UserDashboardLayout(models.Model):
    ...
    layout_type = models.CharField(
        "Tipo de Layout", max_length=20, choices=LAYOUT_TYPES, default="default"
    )
    layout_name = models.CharField(
        "Nombre del Layout", max_length=100, default="Mi Dashboard"
    )
    ...
    grid_row_height = models.PositiveIntegerField(
        "Altura de fila",
        default=120,
        validators=[MinValueValidator(80), MaxValueValidator(200)],
    )
    grid_margin = models.PositiveIntegerField(
        "Margen del grid",
        default=10,
    ...
    show_welcome_widget = models.BooleanField(
        "Mostrar widget de bienvenida", default=True
    )
    auto_refresh_enabled = models.BooleanField(
        "Auto-actualización habilitada", default=True
    )
    ...
    def get_active_widgets(self):
        ...
        return self.widgets.filter(
            is_active=True, userwidgetconfig__is_enabled=True
        ).order_by("order")
...
class UserWidgetConfig(models.Model):
    ...
    user_layout = models.ForeignKey(
        UserDashboardLayout, on_delete=models.CASCADE, verbose_name="Layout del usuario"
    )
    widget = models.ForeignKey(
        DashboardWidget, on_delete=models.CASCADE, verbose_name="Widget"
    )
    ...
    custom_position_x = models.PositiveIntegerField(
        "Posición X personalizada", null=True, blank=True
    )
    custom_position_y = models.PositiveIntegerField(
        "Posición Y personalizada", null=True, blank=True
    )
    custom_width = models.PositiveIntegerField(
        "Ancho personalizado", null=True, blank=True
    )
    custom_height = models.PositiveIntegerField(
        "Alto personalizado", null=True, blank=True
    )

    # Configuración personalizada
    custom_title = models.CharField("Título personalizado", max_length=100, blank=True)
    custom_config = models.JSONField(
        "Configuración personalizada", default=dict, blank=True
    )
    ...
    def get_effective_position(self):
        ...
        return {
            "x": self.custom_position_x
            if self.custom_position_x is not None
            else self.widget.position_x,
            "y": self.custom_position_y
            if self.custom_position_y is not None
            else self.widget.position_y,
        }
...
class WidgetDataCache(models.Model):
    ...
    query_parameters = models.JSONField(
        "Parámetros de consulta", default=dict, blank=True
    )
