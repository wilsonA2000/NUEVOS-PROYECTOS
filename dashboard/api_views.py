...
class DashboardWidgetViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        last_order = (
            DashboardWidget.objects.aggregate(max_order=models.Max("order"))[
                "max_order"
            ]
            or 0
        )
    ...
    def data(self, request, pk=None):
        ...
        try:
            ...
            # Guardar en cache
            cache_timeout = (
                widget.refresh_interval if widget.refresh_interval > 0 else 300
            )
    ...
    def available_types(self, request):
        ...
        return Response(
            {"widget_types": widget_types, "total_types": len(widget_types)}
        )
    ...
    def _get_supported_user_types(self, widget_type):
        ...
        return user_type_support.get(
            widget_type, ["landlord", "tenant", "service_provider"]
        )
...
class UserDashboardLayoutViewSet(viewsets.ModelViewSet):
    ...
    def add_widget(self, request, pk=None):
        ...
        if not widget_id:
            return Response(
                {"error": "widget_id es requerido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"error": "Widget no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def remove_widget(self, request, pk=None):
        ...
        try:
            widget_config = UserWidgetConfig.objects.get(
                user_layout=layout, widget__id=widget_id
            )
            widget_config.delete()

            return Response({"message": "Widget removido exitosamente"})
    ...
    def update_layout(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            for config_data in widget_configs:
                try:
                    widget_config = UserWidgetConfig.objects.get(
                        user_layout=layout, widget__id=config_data["widget_id"]
                    )
...
class DashboardDataAPIView(APIView):
    ...
    def get(self, request):
        ...
        layout, created = UserDashboardLayout.objects.get_or_create(
            user=user, defaults={"layout_name": f"Dashboard de {user.get_full_name()}"}
        )
        ...
        dashboard_data["performance_metrics"] = (
            analytics_service.get_dashboard_performance(user)
        )
    ...
    def get(self, request):
        ...
        try:
            ...
            for widget_config in layout.get_active_widgets():
                widget_data = analytics_service.get_widget_analytics(
                    widget_config.widget, days
                )
...
class WidgetCacheManagementAPIView(APIView):
    ...
    def get(self, request):
        ...
        cache_entries = WidgetDataCache.objects.filter(user=user).order_by(
            "-created_at"
        )

        cache_stats = {
            "total_entries": cache_entries.count(),
            "valid_entries": cache_entries.filter(
                expires_at__gt=timezone.now()
            ).count(),
            "expired_entries": cache_entries.filter(
                expires_at__lte=timezone.now()
            ).count(),
            "total_size_mb": sum(entry.data_size_bytes for entry in cache_entries)
            / (1024 * 1024),
    ...
    def delete(self, request):
        ...
        if clear_all:
            ...
            deleted_count = WidgetDataCache.objects.filter(
                user=user, widget__id=widget_id
            ).count()
            WidgetDataCache.objects.filter(user=user, widget__id=widget_id).delete()
