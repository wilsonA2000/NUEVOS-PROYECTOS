...
class AdvancedDashboardDataService:
    ...
    async def get_comprehensive_dashboard_data(
        ...
        return {
            ...
            "financial_analytics": results[1]
            if not isinstance(results[1], Exception)
            else {},
            "user_behavior": results[2]
            if not isinstance(results[2], Exception)
            else {},
            "property_performance": results[3]
            if not isinstance(results[3], Exception)
            else {},
            "contract_insights": results[4]
            if not isinstance(results[4], Exception)
            else {},
            "matching_intelligence": results[5]
            if not isinstance(results[5], Exception)
            else {},
            "market_trends": results[6]
        ...
        if self.user_type == "landlord":
            ...
            for period_name, start_date in periods.items():
                metrics[period_name] = {
                    "properties": {
                        ...
                        "avg_days_to_rent": self._calculate_avg_days_to_rent(
                            properties, start_date
                        ),
                        "property_types_distribution": self._get_property_types_distribution(
                            properties
                        ),
                    },
                    "revenue": {
                        "total": self._get_revenue_for_period(start_date, end_date),
                        "average_per_property": self._get_avg_revenue_per_property(
                            start_date, end_date
                        ),
                        "growth_rate": self._calculate_revenue_growth_rate(period_name),
                        "top_earning_properties": self._get_top_earning_properties(
                            start_date, end_date, 5
                        ),
                    },
                    "contracts": {
                        "active": Contract.objects.filter(
                            property__landlord=self.user, status="active"
                        ).count(),
            ...
            for period_name, start_date in periods.items():
                metrics[period_name] = {
                    "contracts": {
                        "active": Contract.objects.filter(
                            secondary_party=self.user, status="active"
                        ).count(),
                        "completed": Contract.objects.filter(
                            secondary_party=self.user, status="completed"
                        ).count(),
                        "satisfaction_score": self._calculate_tenant_satisfaction_score(),
                    },
                    "payments": {
                        "total_paid": self._get_payments_for_period(
                            start_date, end_date
                        ),
                        "on_time_rate": self._calculate_on_time_payment_rate(
                            start_date
                        ),
                        "avg_payment_delay": self._calculate_avg_payment_delay(
                            start_date
                        ),
                    },
                    "matching": {
                        "requests_sent": MatchRequest.objects.filter(
                            tenant=self.user, created_at__gte=start_date
                        ).count(),
                        "acceptance_rate": self._calculate_match_acceptance_rate(
                            start_date
                        ),
                        "avg_response_time": self._calculate_avg_match_response_time(
                            start_date
                        ),
                    },
                }
    ...
    def _analyze_revenue_patterns(
        ...
        hourly_patterns = (
            transactions.annotate(hour=Extract("created_at", "hour"))
            .values("hour")
            .annotate(total=Sum("amount"), count=Count("id"))
            .order_by("hour")
        )
        ...
        property_type_revenue = (
            transactions.values("property__property_type")
    ...
    def _analyze_property_profitability(self) -> List[Dict[str, Any]]:
        ...
        properties = Property.objects.filter(landlord=self.user).annotate(
            total_revenue=Coalesce(
                Sum("transactions__amount", filter=Q(transactions__status="completed")),
                0,
            ),
            ...
            profitability_data.append(
                {
                    ...
                    "profitability_grade": self._grade_property_profitability(
                        roi, cash_flow, risk_score
                    ),
                    "recommendations": self._generate_property_recommendations(
                        property_obj, roi, risk_score
                    ),
                }
            )
    ...
    def _get_user_behavior_analytics(self) -> Dict[str, Any]:
        ...
        activities = UserActivityLog.objects.filter(
            user=self.user, timestamp__gte=start_date
        )
        ...
        feature_usage = (
            ...
            .annotate(count=Count("id"), avg_duration=Avg("duration_seconds"))
            .order_by("-count")
        )
    ...
    def _calculate_user_efficiency_metrics(self, activities) -> Dict[str, Any]:
        ...
        avg_session_duration = (
            activities.aggregate(avg_duration=Avg("duration_seconds"))["avg_duration"]
            or 0
        )
    ...
    def _get_matching_intelligence(self) -> Dict[str, Any]:
        ...
        if self.user_type == "landlord":
            matches = MatchRequest.objects.filter(
                landlord=self.user, created_at__gte=start_date
            )
            success_analysis = self._analyze_landlord_match_success(matches)
            optimization_suggestions = (
                self._generate_landlord_optimization_suggestions()
            )
        else:
            matches = MatchRequest.objects.filter(
                tenant=self.user, created_at__gte=start_date
    ...
    def _calculate_avg_days_to_rent(self, properties, start_date: datetime) -> float:
        ...
        recently_rented = properties.filter(status="rented", updated_at__gte=start_date)
                ...
                anomalies.append(
                    {
                        ...
                        "type": "spike"
                        if float(item["total"]) > mean_revenue
                        else "drop",
                    }
                )
    ...
    def _get_cache_info(self) -> Dict[str, Any]:
        ...
        cache_keys = (
            cache.keys(f"{self.cache_prefix}*") if hasattr(cache, "keys") else []
        )
    ...
    async def get_advanced_widget_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cache_timeout = (
                ...
                if self.widget.refresh_interval > 0
                else 300
            )
...
class DashboardAnalyticsEngine:
    ...
    def _analyze_widget_effectiveness(self) -> List[Dict[str, Any]]:
        ...
        for widget_type, display_name in DashboardWidget.WIDGET_TYPES:
            widgets = DashboardWidget.objects.filter(
                widget_type=widget_type, is_active=True
            )

            if widgets.exists():
                # Métricas de uso
                total_users = UserWidgetConfig.objects.filter(
                    widget__widget_type=widget_type, is_enabled=True
                ).count()
                ...
                widget_stats.append(
                    {
                        ...
                        "retention_rate": self._calculate_widget_retention_rate(
                            widget_type
                        ),
                        "recommendations": self._generate_widget_improvement_recommendations(
                            widget_type
                        ),
                    }
                )
        ...
        return sorted(
            widget_stats, key=lambda x: x["effectiveness_score"], reverse=True
        )

    def _calculate_widget_effectiveness_score(
        self, users: int, hits: int, time_spent: float
    ) -> float:
    ...
    def _generate_system_optimizations(self) -> List[Dict[str, Any]]:
        ...
        underutilized_widgets = DashboardWidget.objects.filter(
            is_active=True, userwidgetconfig__isnull=True
        ).count()
    ...
    def _calculate_system_health_score(self) -> Dict[str, Any]:
        ...
        # Score combinado
        health_score = (
            cache_health * 0.3 + user_activity * 0.4 + system_utilization * 0.3
        )
