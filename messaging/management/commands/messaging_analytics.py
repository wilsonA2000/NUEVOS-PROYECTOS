...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--user-id", type=int, help="ID de usuario específico para analizar"
        )
        parser.add_argument(
            "--export-file", type=str, help="Archivo para exportar los resultados"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Error generando analíticas: {str(e)}"))
            raise
    ...
    def _generate_analytics(self, start_date, end_date, user_id=None):
        ...
        analytics["conversations"] = {
            ...
            "by_type": dict(
                threads.values("thread_type")
                .annotate(count=Count("id"))
                .values_list("thread_type", "count")
            ),
            "by_status": dict(
                threads.values("status")
                .annotate(count=Count("id"))
        ...
        analytics["messages"] = {
            ...
            "by_type": dict(
                messages.values("message_type")
                .annotate(count=Count("id"))
                .values_list("message_type", "count")
            ),
            "read_rate": self._calculate_read_rate(messages),
            "avg_length": messages.aggregate(avg=Avg("content"))["avg"] or 0
            if messages.exists()
            else 0,
            "with_attachments": messages.filter(attachments__isnull=False)
            .distinct()
            .count(),
        }
        ...
        analytics["daily_activity"] = self._get_daily_activity(
        ...
        analytics["reactions"] = {
            ...
            "by_type": dict(
                reactions.values("reaction_type")
                .annotate(count=Count("id"))
    ...
    def _get_user_analytics(self, start_date, end_date):
        ...
        top_senders = (
            Message.objects.filter(sent_at__gte=start_date, sent_at__lte=end_date)
            .values("sender__id", "sender__first_name", "sender__last_name")
        ...
        return {
            ...
            "total_active_users": Message.objects.filter(
                sent_at__gte=start_date, sent_at__lte=end_date
            )
            .values("sender")
            .distinct()
            .count(),
    ...
    def _format_text_output(self, analytics, days):
        ...
        # Período
        output.append(
            f'Período: {analytics["period"]["start_date"].strftime("%Y-%m-%d")} - {analytics["period"]["end_date"].strftime("%Y-%m-%d")}'
        )
        ...
        output.append(
            f'  Promedio mensajes por conversación: {conv["avg_messages_per_thread"]:.1f}'
        )
        ...
        if "users" in analytics:
            ...
            if users["top_senders"]:
                ...
                for sender in users["top_senders"][:5]:
                    name = (
                        f"{sender['sender__first_name']} {sender['sender__last_name']}"
                    )
