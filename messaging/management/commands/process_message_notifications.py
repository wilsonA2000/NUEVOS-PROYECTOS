...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--cleanup-old", action="store_true", help="Limpiar notificaciones antiguas"
        )
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    "MODO DE PRUEBA - No se enviarán notificaciones reales"
                )
            )
        ...
        try:
            ...
            if options["process_timeouts"]:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "Modo de prueba: advertencias de timeout no enviadas"
                        )
                    )
            ...
            if options["cleanup_old"]:
                ...
                if not dry_run:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            "Modo de prueba: notificaciones no eliminadas"
                        )
                    )
    ...
    def _show_stats(self):
        ...
        try:
            ...
            self.stdout.write("\n" + "=" * 50)
            self.stdout.write(
                self.style.HTTP_INFO("ESTADÍSTICAS DEL SISTEMA DE MENSAJERÍA")
            )
            ...
            recent_messages = Message.objects.filter(sent_at__gte=week_ago).count()
