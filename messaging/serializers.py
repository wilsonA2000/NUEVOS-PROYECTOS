...
class MessageSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        recipient = thread.participants.exclude(
...
class MessageThreadSerializer(serializers.ModelSerializer):
    ...
    def get_last_message(self, obj):
        last = (
            obj.messages.order_by("-sent_at").first()
    ...
    def create(self, validated_data):
        ...
        participant_ids = list(
            {*map(str, body_participants), *map(str, ctx_participants)}
        )
...
class AdvancedMessageThreadSerializer(MessageThreadSerializer):
    ...
    def get_latest_messages(self, obj):
        ...
        return AdvancedMessageSerializer(
            latest_messages, many=True, context=self.context
        ).data
...
class ConversationAnalyticsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "first_response_time",
            "messages_by_initiator",
            "messages_by_responder",
            "first_message_at",
            "last_activity_at",
            "is_resolved",
            "resolution_time",
...
class MessageSearchSerializer(serializers.Serializer):
    ...
    thread_type = serializers.ChoiceField(
        choices=MessageThread.THREAD_TYPES, required=False, allow_blank=True
    )
...
class MessageCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Message
        fields = ["thread", "content", "message_type", "reply_to", "attachments"]
...
class ConversationCreateSerializer(serializers.Serializer):
    ...
    thread_type = serializers.ChoiceField(
        choices=MessageThread.THREAD_TYPES, default="general"
    )
...
class BulkMessageActionSerializer(serializers.Serializer):
    ...
    def validate_message_ids(self, value):
        ...
        user_message_ids = Message.objects.filter(
            id__in=value, recipient=request.user
