...
class MessageNotificationManager:
    ...
    def send_new_conversation_notification(
        self, thread: MessageThread, initiator: User, recipient: User
    ) -> bool:
        ...
        try:
            ...
            if success:
                self._log_notification(
                    "new_conversation",
                    recipient,
                    {"thread_id": str(thread.id), "initiator_id": initiator.id},
                )
    ...
    def send_unread_messages_digest(self, user: User, frequency: str = "daily") -> bool:
        ...
        try:
            ...
            for message in unread_messages:
                ...
                conversations[thread_id]["sender_names"].add(
    ...
    def send_conversation_timeout_warning(self, thread: MessageThread) -> bool:
        ...
        try:
            ...
            context = {
                ...
                "other_participant": thread.get_other_participant(
                    initiator
                ).get_full_name(),
                "conversation_url": f"{self.base_url}/messages/thread/{thread.id}",
                "platform_name": "VeriHome",
            ...
            if success:
                self._log_notification(
                    ...
                    initiator,
                    {"thread_id": str(thread.id), "days_inactive": days_inactive},
                )
    ...
    def send_bulk_notifications(
        self, notification_type: str, users: List[User], context: Dict[str, Any]
    ) -> Dict[str, int]:
                ...
                try:
                    ...
                    success = self._send_email(
                        ...
                        template_name=context.get(
                            "template", "messaging/email/generic.html"
                        ),
                    ...
                    logger.error(
                        f"Error sending bulk notification to user {user.id}: {str(e)}"
                    )
                    results["failed"] += 1
    ...
    def _send_realtime_notification(
        ...
        try:
            ...
            async_to_sync(channel_layer.group_send)(
                f"user_{target_user.id}", notification_data
            )
    ...
    def _send_push_notification(self, message: Message) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Push notification would be sent to user {message.recipient.id}"
            )
    ...
    def _should_send_email_notification(self, user: User, message: Message) -> bool:
        ...
        participant = ThreadParticipant.objects.filter(
            thread=message.thread, user=user
        ).first()

        if participant and participant.last_read_at:
            # Si leyó mensajes recientemente, probablemente está activo
            if (
                timezone.now() - participant.last_read_at
    ...
    def _log_notification(
        self, notification_type: str, user: User, data: Dict[str, Any]
    ):
    ...
    def process_unread_digests(self, frequency: str = "daily") -> Dict[str, int]:
        ...
        try:
            ...
            users_with_unread = User.objects.filter(
                ...
                received_messages__sent_at__gte=self.manager._get_digest_timeframe(
                    frequency
                ),
            ).distinct()
    ...
    def check_conversation_timeouts(self) -> Dict[str, int]:
        ...
        try:
            ...
            inactive_threads = MessageThread.objects.filter(
                status="active", last_message_at__lt=timeout_threshold
            )
            ...
            for thread in inactive_threads:
                try:
                    ...
                    logger.error(
                        f"Error sending timeout warning for thread {thread.id}: {str(e)}"
                    )
    ...
    def cleanup_old_notifications(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = UserActivityLog.objects.filter(
                activity_type__startswith="notification_", created_at__lt=cutoff_date
            ).delete()[0]
