...
from .models import MessageThread, Message, ThreadParticipant, MessageReaction
...
class AdvancedMessageThreadViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filtros opcionales
        status_filter = getattr(self.request, "query_params", self.request.GET).get(
            "status"
        )
        if status_filter:
            queryset = queryset.filter(status=status_filter)

        thread_type = getattr(self.request, "query_params", self.request.GET).get(
            "type"
        )
        if thread_type:
        ...
        if search:
            queryset = queryset.filter(
                Q(subject__icontains=search) | Q(messages__content__icontains=search)
            ).distinct()
    ...
    def mark_all_read(self, request, pk=None):
        ...
        return Response(
            {"messages_marked": updated_count, "message": "All messages marked as read"}
        )
...
class AdvancedMessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Filtro por hilo
        thread_id = getattr(self.request, "query_params", self.request.GET).get(
            "thread_id"
        )
        if thread_id:
        ...
        return queryset.select_related("sender", "recipient", "thread").order_by(
            "sent_at"
        )
    ...
    def reactions(self, request, pk=None):
        ...
        reactions = MessageReaction.objects.filter(message=message).select_related(
            "user"
        )
        ...
        for reaction in reactions:
            ...
            reaction_data[reaction_type]["users"].append(
                {"id": reaction.user.id, "name": reaction.user.get_full_name()}
            )
    ...
    def get(self, request):
        ...
        try:
            ...
            if getattr(request, "query_params", request.GET).get("is_read"):
                filters["is_read"] = (
                    getattr(request, "query_params", request.GET).get("is_read").lower()
                    == "true"
                )
...
class MessageStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            total_conversations = MessageThread.objects.filter(
                participants=user
            ).count()
            ...
            recent_activity = Message.objects.filter(
                Q(sender=user) | Q(recipient=user), sent_at__gte=week_ago
            ).count()

            # Tiempo promedio de respuesta (últimos 30 días)
            month_ago = timezone.now() - timedelta(days=30)
            user_messages = Message.objects.filter(
                sender=user, sent_at__gte=month_ago, reply_to__isnull=False
            ...
            return Response(
                {
                    ...
                    "response_rate": round(
                        (len(response_times) / max(total_messages_received, 1)) * 100, 2
                    ),
                }
            )
