...
class MessageThread(models.Model):
    ...
    thread_type = models.CharField(
        "Tipo de conversación", max_length=20, choices=THREAD_TYPES
    )

    # Participantes (máximo 2 para conversaciones directas)
    participants = models.ManyToManyField(
        User, related_name="message_threads", through="ThreadParticipant"
    )
    ...
    status = models.CharField(
        "Estado", max_length=20, choices=THREAD_STATUS, default="active"
    )
    is_priority = models.BooleanField("Prioridad alta", default=False)

    # Información de seguimiento
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="initiated_threads"
    )
...
class Message(models.Model):
    ...
    message_type = models.CharField(
        "Tipo de mensaje", max_length=20, choices=MESSAGE_TYPES, default="text"
    )
    ...
    # Estado del mensaje
    status = models.CharField(
        "Estado", max_length=20, choices=MESSAGE_STATUS, default="sent"
    )
    ...
    is_deleted_by_recipient = models.BooleanField(
        "Eliminado por destinatario", default=False
    )
...
class MessageAttachment(models.Model):
    ...
    attachment_type = models.CharField(
        "Tipo de archivo", max_length=20, choices=ATTACHMENT_TYPES
    )
...
class MessageReaction(models.Model):
    ...
    reaction_type = models.CharField(
        "Tipo de reacción", max_length=20, choices=REACTION_TYPES
    )
...
class MessageFolder(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="message_folders"
    )
    ...
    folder_type = models.CharField(
        "Tipo", max_length=20, choices=FOLDER_TYPES, default="custom"
    )
...
class MessageFolderItem(models.Model):
    ...
    folder = models.ForeignKey(
        MessageFolder, on_delete=models.CASCADE, related_name="folder_items"
    )
    message = models.ForeignKey(
        Message, on_delete=models.CASCADE, related_name="folder_items"
    )
...
class MessageTemplate(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="message_templates"
    )
...
class ConversationAnalytics(models.Model):
    ...
    thread = models.OneToOneField(
        MessageThread, on_delete=models.CASCADE, related_name="analytics"
    )

    # Métricas de actividad
    total_messages = models.PositiveIntegerField("Total de mensajes", default=0)
    average_response_time = models.DurationField(
        "Tiempo promedio de respuesta", null=True, blank=True
    )
    first_response_time = models.DurationField(
        "Tiempo de primera respuesta", null=True, blank=True
    )

    # Métricas de participación
    messages_by_initiator = models.PositiveIntegerField(
        "Mensajes del iniciador", default=0
    )
    messages_by_responder = models.PositiveIntegerField(
        "Mensajes del respondedor", default=0
    )
    ...
    resolution_time = models.DurationField(
        "Tiempo de resolución", null=True, blank=True
    )
