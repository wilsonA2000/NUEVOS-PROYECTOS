...
class MessageConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        try:
            ...
            logger.info(
                f"WebSocket disconnected for user {getattr(self.user, 'id', 'unknown')} with code {close_code}"
            )
    ...
    async def handle_mark_as_read(self, data):
        ...
        try:
            ...
            if success:
                await self.send(
                    text_data=json.dumps(
                        {"type": "message_marked_read", "message_id": message_id}
                    )
                )
        ...
        try:
            ...
            if can_access:
                ...
                await self.channel_layer.group_add(
                    conversation_group, self.channel_name
                )

                await self.send(
                    text_data=json.dumps(
                        {"type": "joined_conversation", "thread_id": thread_id}
                    )
                )
            else:
                await self.send(
    ...
    async def handle_leave_conversation(self, data):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "left_conversation", "thread_id": thread_id}
                )
            )
    ...
    def can_access_thread(self, thread_id):
        ...
        try:
            ...
            return MessageThread.objects.filter(
                id=thread_id, participants=self.user
            ).exists()
    ...
    def create_message(self, content):
        ...
        try:
            ...
            message = Message.objects.create(
                thread=thread, sender=self.user, recipient=recipient, content=content
            )
    ...
    def mark_messages_as_read(self, message_ids):
        ...
        try:
            ...
            updated = Message.objects.filter(
                id__in=message_ids, recipient=self.user, is_read=False
