...
class InboxView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            ...
            .select_related("created_by", "property", "contract")
...
class SentView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            MessageThread.objects.filter(created_by=self.request.user, is_deleted=False)
            .select_related("created_by", "property", "contract")
            .prefetch_related("messages", "participants")
            .order_by("-last_message_at")
        )
...
class ArchivedView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            MessageThread.objects.filter(
                participants=self.request.user, is_archived=True, is_deleted=False
            )
            .select_related("created_by", "property", "contract")
            .prefetch_related("messages", "participants")
            .order_by("-last_message_at")
...
class StarredView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            MessageThread.objects.filter(
                participants=self.request.user, is_starred=True, is_deleted=False
            )
            .select_related("created_by", "property", "contract")
            .prefetch_related("messages", "participants")
            .order_by("-last_message_at")
...
class TrashView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return (
            MessageThread.objects.filter(
                participants=self.request.user, is_deleted=True
            )
            .select_related("created_by", "property", "contract")
            .prefetch_related("messages", "participants")
            .order_by("-last_message_at")
...
class ThreadDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        thread.messages.filter(is_read=False).exclude(sender=self.request.user).update(
