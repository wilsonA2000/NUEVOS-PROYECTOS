...
class AdvancedMessagingService:
    ...
    def can_users_communicate(self, user1: User, user2: User) -> Dict[str, Any]:
        ...
        if has_business_relationship:
            return {"can_communicate": True, "reason": "Relación comercial establecida"}
        ...
        if not privacy_check["allowed"]:
            return {"can_communicate": False, "reason": privacy_check["reason"]}

        return {"can_communicate": True, "reason": "Comunicación permitida"}
    ...
    def get_conversation_analytics(self, thread_id: str, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "conversation_duration_hours": conversation_duration.total_seconds()
                / 3600,
                "avg_response_time_minutes": avg_response_time.total_seconds() / 60,
                "unread_count": unread_count,
                "read_rate": round(read_rate, 2),
                "content_analysis": content_analysis,
                "last_activity": thread.last_message_at.isoformat()
    ...
    def search_messages(
        self, user: User, query: str, filters: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if filters:
                if filters.get("thread_type"):
                    messages = messages.filter(
                        thread__thread_type=filters["thread_type"]
                    )
            ...
            return {"success": False, "error": f"Error searching messages: {str(e)}"}
    ...
    def get_conversation_summary(self, thread_id: str, user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            key_points.append(
                {
                    "type": "inicio",
                    "content": first_message.content[:200] + "..."
    ...
    def _find_existing_conversation(
        ...
        threads = MessageThread.objects.filter(
            participants=user1, thread_type=thread_type, status="active"
    ...
    def _validate_message_content(
        ...
        if any(word in content.lower() for word in inappropriate_words):
            return {
                "valid": False,
                "error": "El mensaje contiene contenido inapropiado",
            }
    ...
    def _check_rate_limits(self, user: User, thread: MessageThread) -> Dict[str, Any]:
        ...
        recent_messages = Message.objects.filter(
            sender=user, sent_at__gte=one_minute_ago
        ).count()
        ...
        last_messages = Message.objects.filter(thread=thread, sender=user).order_by(
            "-sent_at"
        )[:3]
    ...
    def _process_attachments(
        self, message: Message, attachments: List[Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "attachments_created": created_attachments}
    ...
    def _check_business_relationship(self, user1: User, user2: User) -> bool:
        ...
        try:
            ...
            properties = Property.objects.filter(landlord=user1)

            # Verificar consultas previas o contratos
            if properties.exists():
                return True

            properties = Property.objects.filter(landlord=user2)
