    ...
    def post(self, request):
        ...
        try:
            original_message = Message.objects.get(
                id=message_id, thread__participants=request.user
            )
            ...
            return Response(
                {"detail": "Mensaje no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
...
class StarMessageAPIView(APIView):
    ...
    def post(self, request, message_pk):
        try:
            ...
            return Response(
                {
                    "detail": f"Mensaje {'destacado' if message.is_starred else 'quita destacado'}"
                }
            )
        except Message.DoesNotExist:
            return Response(
...
class SearchMessagesAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Message.objects.filter(
            thread__participants=self.request.user, content__icontains=query
        )
...
class MessagingStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        active_conversations = MessageThread.objects.filter(
            participants=user, status="active"
        ).count()
    ...
    def get(self, request, user_pk):
        try:
            ...
            if (
                ...
                and target_user.user_type == "tenant"
            ) or (
                current_user.user_type == "tenant"
                and target_user.user_type == "landlord"
            ):
            ...
            # Arrendador con Prestador de Servicios
                ...
                current_user.user_type == "landlord"
                and target_user.user_type == "service_provider"
            ) or (
            ...
            # Arrendatario con Prestador de Servicios
                ...
                if current_user.user_type == "tenant":
                    contracts = Contract.objects.filter(
                        primary_party=current_user, status="active"
                    )
                else:
                    contracts = Contract.objects.filter(
                        primary_party=target_user, status="active"
                    )
