...
results = {"frontend": {}, "backend": {}, "status": "success"}
...
try:
    du_result = subprocess.run(
        ["du", "-sh", "frontend/node_modules"], capture_output=True, text=True
    )
...
try:
    ls_result = subprocess.run(
        ["ls", "frontend/node_modules"], capture_output=True, text=True
    )
...
if results["frontend"].get("reduction_mb"):
    ...
    print(
        f"📊 Bundle size reducción estimada: ~{results['frontend']['reduction_mb'] * 0.5:.1f}MB"
    )

print(
    "\n🎯 Estado global:",
    "✅ ÉXITO" if results["status"] == "success" else "⚠️  REVISAR",
)
