...
try:
    ...
    print("ERROR: requests library no está instalada. Instalar con: pip install requests")
    sys.exit(1)
...
class APITester:
    ...
    def test_endpoint(self, method: str, endpoint: str, expected_status: int = 200,
                     data: dict = None, requires_auth: bool = True,
                     files: dict = None) -> Tuple[bool, str, dict]:
        ...
        try:
            if method == 'GET':
                ...
                if files:
                    response = http_requests.post(url, headers=headers, data=data, files=files, timeout=10)
                else:
                    headers['Content-Type'] = 'application/json'
                    response = http_requests.post(url, headers=headers, json=data, timeout=10)
            elif method == 'PUT':
                headers['Content-Type'] = 'application/json'
                response = http_requests.put(url, headers=headers, json=data, timeout=10)
            elif method == 'PATCH':
                headers['Content-Type'] = 'application/json'
                response = http_requests.patch(url, headers=headers, json=data, timeout=10)
            elif method == 'DELETE':
    ...
    def generate_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if self.results['failed']:
                ...
                connection_errors = [r for r in self.results['failed'] if 'conexión' in r['message'].lower()]
            ...
            if self.results['warnings']:
                f.write("\n### Performance\n\n")
                f.write(f"- **{len(self.results['warnings'])} endpoints con timeouts o respuestas lentas.**\n")
