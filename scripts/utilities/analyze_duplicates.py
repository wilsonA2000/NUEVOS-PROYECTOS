...
def find_components():
    ...
    base_path = Path(
        "/mnt/c/Users/wilso/Desktop/NUEVOS PROYECTOS/frontend/src/components"
    )
    components = []
...
def find_duplicates(components):
    ...
    for comp in components:
        ...
        base_name = re.sub(
            r"(Enhanced|Simple|Professional|Old|Backup|Optimized|Real.*?)",
            "",
            comp["name"],
        )
...
def main():
    ...
    print(
        f"🟡 Prioridad Media: {len(priority_medium)} grupos ({sum(len(g[1]) for g in priority_medium)} componentes)"
    )
    print(
        f"🟢 Prioridad Baja: {len(priority_low)} grupos ({sum(len(g[1]) for g in priority_low)} componentes)"
    )

    total_duplicate_lines = sum(
        total_lines
        for _, _, total_lines in priority_high + priority_medium + priority_low
    )
