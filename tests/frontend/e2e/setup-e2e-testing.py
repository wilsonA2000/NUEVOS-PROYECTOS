...
def create_test_fixtures():
    ...
    for name in amenity_names:
        amenity, created = Amenity.objects.get_or_create(
            name=name, defaults={"description": f"{name} disponible en la propiedad"}
        )
    ...
    for prop_data in test_properties:
        prop, created = Property.objects.get_or_create(
            title=prop_data["title"], landlord=landlord, defaults=prop_data
        )
        if created:
    ...
    conversation, created = Conversation.objects.get_or_create(
        subject="Property Inquiry - Luxury Apartment", defaults={}
    )
    if created:
        ...
        messages = [
            {
                "sender": tenant,
                "body": "Hi, I am interested in your luxury apartment. Is it still available?",
            },
            {
                "sender": landlord,
                "body": "Hello! Yes, the apartment is still available. Would you like to schedule a viewing?",
            },
            {
                "sender": tenant,
                "body": "That would be great! I am available this weekend.",
            },
    ...
    return {"users": test_users, "properties": properties, "amenities": amenities}
...
def install_cypress_dependencies():
    ...
    try:
        ...
        subprocess.run(
            ["npx", "cypress", "install"], check=True, capture_output=True, text=True
        )
...
def verify_test_environment():
    ...
    try:
        ...
        print(
            "❌ Django backend is not running. Please start it with: python manage.py runserver"
        )
        return False
    ...
    try:
        ...
        print(
            "❌ Frontend is not running. Please start it with: cd frontend && npm run dev"
        )
        return False
