...
def test_reject_cleanup():
    ...
    matches = MatchRequest.objects.filter(
        status__in=["pending", "accepted", "contract_ready"]
    )
    ...
    prop_requests_before = PropertyInterestRequest.objects.filter(
        requester=test_match.tenant, property=test_match.property
    ).count()
    ...
    print(
        f"   📋 Solicitudes de propiedad: {prop_requests_after} (antes: {prop_requests_before})"
    )
    ...
    cleanup_successful = (
        contracts_after == 0
        and prop_requests_after == 0
        and docs_after == 0
        and test_match.status == "rejected"
    )
