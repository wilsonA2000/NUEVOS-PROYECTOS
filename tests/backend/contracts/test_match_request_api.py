                ...
                if user_data.get("user_type") == "tenant":
                    ...
                    if props_response.status_code == 200:
                        ...
                        if properties:
                            ...
                            print(
                                f"   Testing with property: {property_title} (ID: {property_id})"
                            )

                            # Create match request
                            print("4. Creating match request...")
                            match_url = (
                                "http://localhost:8000/api/v1/matching/requests/"
                            )
                            ...
                            match_response = requests.post(
                                match_url, json=match_data, headers=headers
                            )
                            print(
                                f"   Match request status: {match_response.status_code}"
                            )
                            ...
                            if match_response.status_code == 201:
                                ...
                                if "Ya has enviado una solicitud" in str(error_data):
                                    print(
                                        "   ℹ️ Match request already exists for this property"
                                    )
                                    print(
                        ...
                        print(
                            f"   ❌ Failed to get properties: {props_response.status_code}"
                        )
                else:
