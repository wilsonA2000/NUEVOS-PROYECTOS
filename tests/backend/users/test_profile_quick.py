...
# Test data
LOGIN_DATA = {"email": "admin@verihome.com", "password": "admin123"}
...
try:
    ...
    print(
        f'   Nombre actual: {current_profile.get("first_name")} {current_profile.get("last_name")}'
    )

    # 3. Actualizar perfil
    print("\n3. Actualizando perfil...")
    update_response = requests.patch(
        f"{BASE_URL}/users/profile/", json=PROFILE_UPDATE_DATA, headers=headers
    )
    ...
    print(
        f'   Nombre actualizado: {updated_profile.get("first_name")} {updated_profile.get("last_name")}'
    )
    ...
    for key, value in PROFILE_UPDATE_DATA.items():
        if verified_profile.get(key) != value:
            print(
                f'   ❌ Campo {key}: esperado "{value}", obtenido "{verified_profile.get(key)}"'
            )
