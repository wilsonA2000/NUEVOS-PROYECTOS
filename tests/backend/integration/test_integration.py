...
class FullUserJourneyIntegrationTest(APITestCase):
    ...
    def test_complete_landlord_tenant_journey(self):
        ...
        login_response = self.client.post(
            self.login_url,
            {"email": landlord_data["email"], "password": landlord_data["password"]},
        )
        ...
        tenant_login_response = self.client.post(
            self.login_url,
            {"email": tenant_data["email"], "password": tenant_data["password"]},
        )
        ...
        search_response = self.client.get(
            f"{self.properties_url}?city=Bogotá&max_price=2000000"
        )
        ...
        property_detail_url = reverse(
            "api:properties-detail", kwargs={"pk": property_id}
        )
        ...
        conversation_response = self.client.post(
            self.conversations_url, conversation_data
        )
        ...
        contract_detail_url = reverse(
            "api:contracts-detail", kwargs={"pk": contract_id}
        )
        ...
        pending_payments = [
            p for p in payments_response.data["results"] if p["status"] == "pending"
        ]
        self.assertGreater(len(pending_payments), 0)

        # Process security deposit payment
        security_payment = next(
            p for p in pending_payments if p["payment_type"] == "security_deposit"
        )
        process_url = reverse(
            "api:payments-process", kwargs={"pk": security_payment["id"]}
        )
...
class PropertyLifecycleIntegrationTest(APITestCase):
    ...
    def test_property_complete_lifecycle(self):
        ...
        self.assertEqual(
            create_response.data["property_type"], property_data["property_type"]
        )
        self.assertEqual(
            str(create_response.data["rent_price"]), property_data["rent_price"]
        )
        ...
        self.assertEqual(
            str(update_response.data["rent_price"]), update_data["rent_price"]
        )
        ...
        price_search_response = self.client.get(
            f"{self.properties_url}?min_price=4000000&max_price=5000000"
        )
        ...
        type_search_response = self.client.get(
            f"{self.properties_url}?property_type=penthouse"
        )
        ...
        active_properties = [
            p for p in inactive_search_response.data["results"] if p["is_active"]
        ]
...
class PaymentFlowIntegrationTest(APITestCase):
    ...
    def test_complete_payment_flow(self):
        ...
        pending_payments = [
            p
            for p in tenant_payments_response.data["results"]
            if p["status"] == "pending"
        ]
        ...
        payment_process_data = {"payment_method": "credit_card", "amount": "2000000"}
        ...
        history_response = self.client.get(
            f"{self.payments_url}?contract={self.contract.id}"
        )
        ...
        incoming_payments = [
            p
            for p in landlord_payments_response.data["results"]
            if p["payee"] == self.landlord.id
        ]
...
class MessagingIntegrationTest(APITestCase):
    ...
    def test_complete_messaging_flow(self):
        ...
        messages_response = self.client.get(
            f"{self.messages_url}?conversation={conversation_id}"
        )
        ...
        mark_read_url = reverse(
            "api:messages-mark-read", kwargs={"pk": initial_message_id}
        )
        ...
        new_messages_response = self.client.get(
            f"{self.messages_url}?conversation={conversation_id}"
        )
        ...
        final_messages_response = self.client.get(
            f"{self.messages_url}?conversation={conversation_id}"
        )
        ...
        conversation_detail_url = reverse(
            "api:conversations-detail", kwargs={"pk": conversation_id}
        )
...
class SearchAndFilterIntegrationTest(APITestCase):
    ...
    def test_comprehensive_search_and_filtering(self):
        ...
        self.assertEqual(
            len(budget_response.data["results"]), 2
        )  # Budget apartment and studio
        ...
        mid_range_response = self.client.get(
            f"{self.properties_url}?min_price=1000000&max_price=3000000"
        )
        ...
        apartment_response = self.client.get(
            f"{self.properties_url}?property_type=apartment"
        )
        ...
        multi_bedroom_response = self.client.get(
            f"{self.properties_url}?min_bedrooms=3"
        )
        self.assertEqual(multi_bedroom_response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(multi_bedroom_response.data["results"]), 2
        )  # House and penthouse

        # 6. FILTER BY AMENITIES
        pets_allowed_response = self.client.get(
            f"{self.properties_url}?pets_allowed=true"
        )
        ...
        self.assertEqual(search_response.status_code, status.HTTP_200_OK)
        family_results = [
            p for p in search_response.data["results"] if "family" in p["title"].lower()
        ]
        self.assertGreaterEqual(len(family_results), 1)

        # 9. ORDERING
        price_asc_response = self.client.get(
            f"{self.properties_url}?ordering=rent_price"
        )
        ...
        price_desc_response = self.client.get(
            f"{self.properties_url}?ordering=-rent_price"
        )
        self.assertEqual(price_desc_response.status_code, status.HTTP_200_OK)
        desc_prices = [
            int(p["rent_price"]) for p in price_desc_response.data["results"]
        ]
