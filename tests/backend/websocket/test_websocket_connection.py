...
class WebSocketTester:
    ...
    async def test_websocket_endpoint(self, endpoint, test_message=None):
        ...
        try:
            ...
            async with websockets.connect(uri, timeout=5) as websocket:
                ...
                if test_message:
                    ...
                    self.log_result(
                        endpoint,
                        True,
                        f"Mensaje enviado: {test_message.get('type', 'unknown')}",
                    )
                    ...
                    try:
                        ...
                        self.log_result(
                            endpoint,
                            True,
                            f"Respuesta: {response_data.get('type', 'unknown')}",
                        )
            ...
            self.log_result(
                endpoint, False, "Conexión rechazada - servidor no disponible"
            )
            return False
        except asyncio.TimeoutError:
    ...
    async def test_all_endpoints(self):
        ...
        endpoints_to_test = [
            ...
            {"endpoint": "/ws/notifications/", "test_message": {"type": "ping"}},
            {
