...
def test_property_creation():
    ...
    try:
        ...
        print(
            json.dumps(
                {k: str(v) for k, v in property_data.items()},
                indent=2,
                ensure_ascii=False,
            )
        )
        ...
        print(
            f"   - Campos requeridos presentes: {all(key in response_data for key in ['id', 'title', 'property_type', 'listing_type'])}"
        )
        ...
        print(
            f"   - ID es UUID válido: {len(property_id) == 36 and property_id.count('-') == 4}"
        )
