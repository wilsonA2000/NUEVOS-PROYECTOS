...
class PaymentStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        include_predictions = (
            request.query_params.get("predictions", "false").lower() == "true"
        )

        # Check cache first
        cache_key = (
            f"payment_stats_{user.id}_{date_range}_{currency}_{include_predictions}"
        )
        ...
        try:
            ...
            stats = {
                ...
                "transaction_summary": self._calculate_transaction_summary(
                    user_transactions
                ),
                "revenue_analytics": self._calculate_revenue_analytics(
                    user, user_transactions, start_date, end_date
                ),
                "payment_methods": self._calculate_payment_method_stats(
                    user, user_transactions
                ),
                "escrow_analytics": self._calculate_escrow_stats(
                    user, start_date, end_date
                ),
                "invoice_analytics": self._calculate_invoice_stats(
                    user, start_date, end_date
                ),
                "payment_plans": self._calculate_payment_plan_stats(
                    user, start_date, end_date
                ),
                "cash_flow": self._calculate_cash_flow_analysis(
                    user, user_transactions, start_date, end_date
                ),
                "trends": self._calculate_payment_trends(
                    user_transactions, start_date, end_date
                ),
                "comparisons": self._calculate_period_comparisons(
                    user, start_date, end_date, date_range
                ),
                "geographic_distribution": self._calculate_geographic_stats(
                    user_transactions
                ),
            ...
            if include_predictions:
                stats["predictions"] = self._generate_payment_predictions(
                    user, user_transactions
                )
            ...
            if hasattr(user, "user_type"):
                if user.user_type == "landlord":
                    stats["landlord_analytics"] = self._calculate_landlord_analytics(
                        user, start_date, end_date
                    )
                elif user.user_type == "tenant":
                    stats["tenant_analytics"] = self._calculate_tenant_analytics(
                        user, start_date, end_date
                    )
                elif user.user_type == "service_provider":
                    stats["service_provider_analytics"] = (
                        self._calculate_service_provider_analytics(
                            user, start_date, end_date
                        )
                    )
            ...
            logger.error(
                f"Error calculating payment stats for user {user.id}: {str(e)}"
            )
            return Response(
    ...
    def _calculate_transaction_summary(self, transactions):
        ...
        # Calculate totals
        total_amount = (
            transactions.aggregate(Sum("total_amount"))["total_amount__sum"] or 0
        )
        avg_transaction = (
    ...
    def _calculate_revenue_analytics(self, user, transactions, start_date, end_date):
        ...
        # Income vs Outgoing
        incoming = (
            transactions.filter(payee=user, status="completed").aggregate(
        ...
        outgoing = (
            transactions.filter(payer=user, status="completed").aggregate(
                total=Sum("total_amount")
            )["total"]
            or 0
        )
        ...
        # Monthly breakdown
        monthly_revenue = (
            transactions.filter(payee=user, status="completed")
        ...
        return {
            ...
            "profit_margin": float(
                (net_income / incoming * 100) if incoming > 0 else 0
            ),
            "monthly_breakdown": [
                {
            ...
            "revenue_by_source": {
                item["transaction_type"]: {
                    ...
                    "count": item["count"],
                    "percentage": float(
                        (item["revenue"] / incoming * 100) if incoming > 0 else 0
                    ),
                }
    ...
    def _calculate_payment_method_stats(self, user, transactions):
        ...
        # Usage statistics
        method_usage = (
            transactions.filter(payment_method__isnull=False)
            .values("payment_method__method_type")
            .annotate(
                count=Count("id"),
                total_amount=Sum("total_amount"),
                success_rate=Avg(Case(When(status="completed", then=1.0), default=0.0)),
            )
        )
        ...
        return {
            ...
            "usage_stats": {
                item["payment_method__method_type"]: {
                    ...
                    "success_rate": float(item["success_rate"] * 100)
    ...
    def _calculate_escrow_stats(self, user, start_date, end_date):
        ...
        total_escrow_amount = (
            escrow_accounts.aggregate(Sum("amount"))["amount__sum"] or 0
        )
    ...
    def _calculate_invoice_stats(self, user, start_date, end_date):
        ...
        return {
            "issued": {
                ...
                "total_amount": float(
                    issued_invoices.aggregate(Sum("amount"))["amount__sum"] or 0
                ),
                "paid_amount": float(
                    issued_invoices.filter(status="paid").aggregate(Sum("amount"))[
                        "amount__sum"
                    ]
                    or 0
                ),
                "by_status": {
                    ...
                    for item in issued_invoices.values("status").annotate(
                        count=Count("id")
                    )
                },
            },
            "received": {
                ...
                "total_amount": float(
                    received_invoices.aggregate(Sum("amount"))["amount__sum"] or 0
                ),
                "paid_amount": float(
                    received_invoices.filter(status="paid").aggregate(Sum("amount"))[
                        "amount__sum"
                    ]
                    or 0
                ),
                "by_status": {
                    item["status"]: item["count"]
                    for item in received_invoices.values("status").annotate(
                        count=Count("id")
                    )
                },
            },
    ...
    def _calculate_payment_plan_stats(self, user, start_date, end_date):
        ...
        total_planned_amount = (
            payment_plans.aggregate(Sum("total_amount"))["total_amount__sum"] or 0
        )

        # Calculate completion rates
        completed_plans = payment_plans.filter(status="completed").count()
        completion_rate = (
            (completed_plans / payment_plans.count() * 100)
    ...
    def _calculate_cash_flow_analysis(self, user, transactions, start_date, end_date):
        ...
        daily_flow = (
            ...
            .values("day")
            .annotate(
                inflow=Sum(Case(When(payee=user, then="total_amount"), default=0)),
                outflow=Sum(Case(When(payer=user, then="total_amount"), default=0)),
            )
            .order_by("day")
        )
    ...
    def _calculate_payment_trends(self, transactions, start_date, end_date):
        ...
        weekly_trends = (
            transactions.filter(status="completed")
            .annotate(week=TruncWeek("processed_at"))
            .values("week")
        ...
        growth_rates = [
            item["growth_rate"] for item in trend_data if item["growth_rate"] != 0
        ]
        ...
        return {
            ...
            "changes": {
                ...
                "total_amount": self._calculate_percentage_change(
                    previous_stats["total_amount"], current_stats["total_amount"]
                ),
                "income": self._calculate_percentage_change(
                    previous_stats["income"], current_stats["income"]
                ),
                "expenses": self._calculate_percentage_change(
                    previous_stats["expenses"], current_stats["expenses"]
                ),
            },
        }
    ...
    def _calculate_basic_stats(self, transactions, user):
        ...
        income = (
            transactions.filter(payee=user).aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            or 0
        )
        expenses = (
            transactions.filter(payer=user).aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            or 0
        )
    ...
    def _calculate_geographic_stats(self, transactions):
        ...
        geographic_data = (
            property_transactions.values("property__city")
            .annotate(count=Count("id"), amount=Sum("total_amount"))
            .order_by("-amount")
        )
    ...
    def _generate_payment_predictions(self, user, transactions):
        ...
        avg_monthly_volume = (
            recent_30d.aggregate(Sum("total_amount"))["total_amount__sum"] or 0
        )
        ...
        return {
            "next_30_days": {
                ...
                "predicted_transactions": int(
                    avg_transaction_count * (1 + growth_rate)
                ),
                "confidence": 0.75,
            },
            "next_90_days": {
                ...
                "predicted_transactions": int(
                    avg_transaction_count * 3 * (1 + growth_rate)
                ),
                "confidence": 0.60,
            },
        }
    ...
    def _calculate_landlord_analytics(self, user, start_date, end_date):
        ...
        # Collection rate
        collection_rate = (
            (collected_rent / total_expected_rent * 100)
        ...
        return {
            ...
            "average_rent_per_property": float(
                total_expected_rent / rent_schedules.count()
            )
            if rent_schedules.count() > 0
            else 0,
        }
    ...
    def _calculate_tenant_analytics(self, user, start_date, end_date):
        ...
        total_rent_paid = (
            rent_payments.filter(status="completed").aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            or 0
        )
        ...
        on_time_payments = rent_payments.filter(
            status="completed"
        ).count()  # Simplified
        on_time_rate = (
            (on_time_payments / total_rent_transactions * 100)
            if total_rent_transactions > 0
            else 0
        )
        ...
        return {
            ...
            "average_monthly_rent": float(total_rent_paid / 12)
            if total_rent_paid > 0
            else 0,  # Simplified
        }
    ...
    def _calculate_service_provider_analytics(self, user, start_date, end_date):
        ...
        total_service_income = (
            service_payments.aggregate(Sum("total_amount"))["total_amount__sum"] or 0
        )
        ...
        return {
            ...
            "average_service_fee": float(total_service_income / service_count)
            if service_count > 0
            else 0,
        }
...
class SystemPaymentStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            stats = {
                ...
                "payment_method_performance": self._calculate_payment_method_performance(
                    all_transactions
                ),
                "geographic_analytics": self._calculate_system_geographic_stats(
                    all_transactions
                ),
    ...
    def _calculate_platform_summary(self, transactions):
        ...
        total_volume = (
            transactions.filter(status="completed").aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            or 0
        )
        ...
        return {
            ...
            "average_transaction_size": float(total_volume / transactions.count())
    ...
    def _calculate_payment_method_performance(self, transactions):
        ...
        return {
            method["payment_method__method_type"]: {
                ...
                "success_rate": float(method["success_rate"] * 100)
    ...
    def _calculate_system_geographic_stats(self, transactions):
        ...
        geographic_data = (
            property_transactions.values("property__city")
            .annotate(count=Count("id"), volume=Sum("total_amount"))
            .order_by("-volume")
        )
