...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ) | Invoice.objects.filter(recipient=self.request.user)
        ...
        try:
            ...
            if contract_id:
                ...
                try:
                    ...
                    if contract.workflow_status not in [
                        "active",
                        "completed_biometric",
                    ]:
                        return Response(
            ...
            with db_transaction.atomic():
                ...
                payment_method_data = {
                    ...
                    "stripe_customer_id": getattr(
                        payment.payer, "stripe_customer_id", None
                    ),
                }
    ...
    def refund(self, request, pk=None):
        ...
        try:
            ...
            result = gateway.refund_payment(
                transaction_id=payment.transaction_id, amount=amount, reason=reason
            )
...
class PaymentPlanViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return PaymentPlan.objects.filter(user=self.request.user).select_related(
            "user", "contract"
        )
    ...
    def pay_installment(self, request, pk=None):
        ...
        try:
            installment = plan.installments.get(
                installment_number=installment_number, status="pending"
            )
...
class SetDefaultPaymentMethodAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            ...
            return Response(
                {"detail": "Método de pago establecido como predeterminado"}
            )
...
class FundEscrowAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            escrow = EscrowAccount.objects.get(id=pk, buyer=request.user)
...
class ReleaseEscrowAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            escrow = EscrowAccount.objects.get(id=pk, seller=request.user)
...
class PayInvoiceAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            invoice = Invoice.objects.get(id=pk, recipient=request.user)
...
class SendInvoiceAPIView(APIView):
    ...
    def post(self, request, pk):
        try:
            invoice = Invoice.objects.get(id=pk, issuer=request.user)
...
class DIANInvoiceXMLAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{invoice.invoice_number}.xml"'
        )
        return response
...
class PaymentDashboardStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        user_transactions = Transaction.objects.filter(
            payer=user
        ) | Transaction.objects.filter(payee=user)

        # Calcular estadísticas
        total_transactions = user_transactions.count()
        total_amount = (
            user_transactions.aggregate(total=models.Sum("total_amount"))["total"] or 0
        )
...
class TransactionReportAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        ) | Transaction.objects.filter(payee=self.request.user)
    ...
    def post(self, request):
        ...
        try:
            ...
            result = gateway.handle_webhook(
                data=payload.decode("utf-8"), headers={"stripe-signature": sig_header}
            )
            ...
            if result.success:
                ...
                return Response(
                    {"error": result.error_message}, status=status.HTTP_400_BAD_REQUEST
                )
...
class RentPaymentScheduleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return RentPaymentSchedule.objects.filter(Q(tenant=user) | Q(landlord=user))
    ...
    def perform_create(self, serializer):
        ...
        if contract.landlord != user:
            raise permissions.PermissionDenied(
                "Solo el arrendador puede crear cronogramas de pago"
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            schedule = RentPaymentSchedule.objects.get(
                id=schedule_id, tenant=request.user, is_active=True
            )
            ...
            if payment_method_id:
                payment_method = PaymentMethod.objects.get(
                    id=payment_method_id, user=request.user, is_active=True
                )
            ...
            dashboard_data.append(
                {
                    ...
                    "last_payment": {
                        "date": last_payment.processed_at.date()
                        if last_payment
                        else None,
                ...
                overdue_tenants.append(
                    {
                        ...
                        "days_overdue": (
                            date.today() - schedule.get_next_due_date()
                        ).days,
                    }
                )
        ...
        return Response(
            {
                "financial_summary": {
                    ...
                    "overdue_amount": overdue_amount,
                    "collection_rate": (
                        collected_this_month / total_monthly_income * 100
                    )
        ...
        try:
            ...
            try:
                ...
                if result.status == "completed":
                    ...
                    transaction.failure_reason = result.metadata.get(
                ...
                logger = logging.getLogger(__name__)
                logger.info(
                    f"Wompi webhook processed: {result.transaction_id} - {result.status}"
                )
                ...
                return Response(
                    {"status": "accepted", "message": "Transaction not found"},
                    status=status.HTTP_200_OK,
                )
...
class BoldWebhookAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not result.success and result.error_code == "INVALID_SIGNATURE":
                return Response(
                    {"error": "Firma inválida"}, status=status.HTTP_403_FORBIDDEN
                )
            ...
            if result.transaction_id:
                try:
                    ...
                    logger.info(
                        f"Bold webhook reconciliado: {result.transaction_id} → {result.status}"
                    )

                except Transaction.DoesNotExist:
                    logger.warning(
                        f"Bold webhook: transacción {result.transaction_id} no encontrada"
                    )
...
class PSEBanksListAPIView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            return Response({"banks": banks, "count": len(banks)})
...
class WompiPaymentStatusAPIView(APIView):
    ...
    def get(self, request, transaction_id):
        ...
        try:
            # Buscar transacción
            transaction = Transaction.objects.get(id=transaction_id, payer=request.user)
...
class PaymentOrderReceiptAPIView(APIView):
    ...
    def get(self, request, pk):
        ...
        user = request.user
        if not (
            user.is_staff or order.payer_id == user.id or order.payee_id == user.id
        ):
            return Response(
                {"error": "Sin permiso para ver esta orden."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class PaymentOrderViewSet(viewsets.ModelViewSet):
    ...
    def cancel(self, request, pk=None):
        ...
        order.add_audit_event(
            "cancelled", f"Cancelada por {user.email}", actor=user, save=False
        )
        order.save()
        ...
        return Response(
            {
                ...
                "overdue": qs.filter(date_due__lt=today)
