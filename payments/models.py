...
class Transaction(models.Model):
    ...
    transaction_number = models.CharField(
        "Número de transacción", max_length=50, unique=True
    )
    ...
    transaction_type = models.CharField(
        "Tipo de transacción", max_length=20, choices=TRANSACTION_TYPES
    )
    ...
    status = models.CharField(
        "Estado", max_length=20, choices=TRANSACTION_STATUS, default="pending"
    )

    # Método de pago
    payment_method = models.ForeignKey(
        PaymentMethod, on_delete=models.SET_NULL, null=True, related_name="transactions"
    )
    ...
    gateway_provider = models.CharField(
        "Proveedor de pasarela", max_length=50, blank=True
    )
    gateway_transaction_id = models.CharField(
        "ID de transacción en pasarela", max_length=100, blank=True
    )
    gateway_response = models.JSONField(
        "Respuesta de pasarela", default=dict, blank=True
    )
...
class EscrowAccount(models.Model):
    ...
    buyer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="escrow_accounts_as_buyer"
    )
    seller = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="escrow_accounts_as_seller"
    )

    # Información del escrow
    escrow_type = models.CharField(
        "Tipo de escrow", max_length=20, choices=ESCROW_TYPES
    )
    amount = models.DecimalField("Monto", max_digits=12, decimal_places=2)
    currency = models.CharField("Moneda", max_length=3, default="COP")
    ...
    status = models.CharField(
        "Estado", max_length=20, choices=ESCROW_STATUS, default="pending"
    )
    ...
    release_date = models.DateField(
        "Fecha de liberación programada", null=True, blank=True
    )
    released_at = models.DateTimeField(
        "Fecha de liberación real", null=True, blank=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.escrow_number:
            year = timezone.now().year
            count = EscrowAccount.objects.filter(created_at__year=year).count() + 1
...
class Invoice(models.Model):
    ...
    issuer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="issued_invoices"
    )
    recipient = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="received_invoices"
    )

    # Información de la factura
    invoice_type = models.CharField(
        "Tipo de factura", max_length=20, choices=INVOICE_TYPES
    )
    ...
    tax_amount = models.DecimalField(
        "Impuestos", max_digits=8, decimal_places=2, default=Decimal("0.00")
    )
    discount_amount = models.DecimalField(
        "Descuentos", max_digits=8, decimal_places=2, default=Decimal("0.00")
    )
    ...
    # Estado y fechas
    status = models.CharField(
        "Estado", max_length=20, choices=INVOICE_STATUS, default="draft"
    )
...
class InvoiceItem(models.Model):
    ...
    quantity = models.DecimalField(
        "Cantidad", max_digits=8, decimal_places=2, default=Decimal("1.00")
    )
    ...
    tax_rate = models.DecimalField(
        "Tasa de impuesto (%)", max_digits=5, decimal_places=2, default=Decimal("0.00")
    )
    discount_rate = models.DecimalField(
        "Tasa de descuento (%)", max_digits=5, decimal_places=2, default=Decimal("0.00")
    )
...
class PaymentPlan(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="payment_plans"
    )
    ...
    installment_amount = models.DecimalField(
        "Monto de cuota", max_digits=10, decimal_places=2
    )
    ...
    # Estado y fechas
    status = models.CharField(
        "Estado", max_length=20, choices=PLAN_STATUS, default="active"
    )
    ...
    late_fee_amount = models.DecimalField(
        "Recargo por mora", max_digits=8, decimal_places=2, default=Decimal("0.00")
    )
    ...
    def get_next_payment_date(self):
        ...
        last_payment = (
            self.installments.filter(status="paid").order_by("-due_date").first()
        )
        if not last_payment:
...
class PaymentInstallment(models.Model):
    ...
    # Estado y fechas
    status = models.CharField(
        "Estado", max_length=20, choices=INSTALLMENT_STATUS, default="pending"
    )
    due_date = models.DateField("Fecha de vencimiento")
    paid_date = models.DateField("Fecha de pago", null=True, blank=True)
...
class RentPaymentSchedule(models.Model):
    ...
    late_fee_amount = models.DecimalField(
        "Recargo por mora (legacy/fallback)", max_digits=10, decimal_places=2, default=0
    )
    grace_period_days = models.PositiveIntegerField("Días de gracia", default=5)
    ...
    auto_charge_enabled = models.BooleanField(
        "Cobro automático habilitado", default=False
    )
    auto_late_fee_enabled = models.BooleanField(
        "Recargo automático por mora", default=True
    )

    # Notificaciones
    reminder_days_before = models.PositiveIntegerField(
        "Días de recordatorio antes", default=3
    )
    send_payment_confirmations = models.BooleanField(
        "Enviar confirmaciones de pago", default=True
    )
...
class RentPaymentReminder(models.Model):
    ...
    reminder_type = models.CharField(
        "Tipo de recordatorio", max_length=20, choices=REMINDER_TYPES
    )
    due_date = models.DateField("Fecha de vencimiento")
...
class LegalInterestRate(models.Model):
    ...
    month = models.PositiveSmallIntegerField(
        "Mes", choices=[(i, i) for i in range(1, 13)]
    )
...
class PaymentOrder(models.Model):
    ...
    status = models.CharField(
        "Estado", max_length=20, choices=ORDER_STATUS, default="pending"
    )
...
try:
    ...
    __all__ = [
        ...
        "RentPaymentReminder",
        "LegalInterestRate",
        "MAX_USURY_MONTHLY_RATE",
        "PaymentOrder",
        "ContractEscrowAccount",
        "ContractEscrowTransaction",
        "ContractEscrowReleaseRule",
