...
def generate_payment_receipt(payment):
    ...
    elements.append(
        Paragraph("Plataforma Inmobiliaria - Recibo de Pago", styles["ReceiptSubtitle"])
    )
    ...
    if payment.platform_fee > 0 or payment.processing_fee > 0:
        ...
        if payment.platform_fee > 0:
            fee_details.append(f"Comisión plataforma: ${payment.platform_fee:,.2f}")
        if payment.processing_fee > 0:
            fee_details.append(f"Tarifa procesamiento: ${payment.processing_fee:,.2f}")
        fee_details.append(f"Total: ${payment.total_amount:,.2f} {payment.currency}")
        elements.append(Paragraph(" | ".join(fee_details), styles["Footer"]))
    ...
    if payment.gateway_transaction_id:
        detail_rows.append(
            ["ID de Transacción Gateway", payment.gateway_transaction_id]
        )
    ...
    if payment.contract or payment.property:
        elements.append(
            Paragraph("Referencia de Contrato y Propiedad", styles["SectionHeader"])
        )

        ref_rows = []
        if payment.contract:
            contract = payment.contract
            ref_rows.append(["Contrato", contract.title or f"#{contract.id}"])
    ...
    elements.append(
        Paragraph(
            "VeriHome - Plataforma Inmobiliaria | www.verihome.com", styles["Footer"]
        )
    )
...
def generate_payment_order_receipt(order):
    ...
    if order.interest_amount and order.interest_amount > 0:
        money_rows.append(
            ["Intereses moratorios:", _format_money(order.interest_amount)]
        )
