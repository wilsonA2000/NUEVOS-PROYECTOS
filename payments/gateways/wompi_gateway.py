...
class WompiGateway(BasePaymentGateway):
    ...
    def validate_config(self):
        ...
        if self.sandbox_mode:
            if not public_key.startswith("pub_test_"):
                logger.warning(
                    "Public key should start with 'pub_test_' in sandbox mode"
                )
        else:
            if not public_key.startswith("pub_prod_"):
                logger.warning(
                    "Public key should start with 'pub_prod_' in production mode"
                )
        ...
        try:
            ...
            if payment_method == "PSE":
                ...
                if kwargs.get("payment_source_id"):
                    transaction_data["payment_method"]["payment_source_id"] = kwargs[
                        "payment_source_id"
                    ]
                if kwargs.get("installments"):
                    transaction_data["payment_method"]["installments"] = kwargs[
                        "installments"
                    ]
            ...
            self.log_info(
                f"Creating Wompi {payment_method} payment for {reference}",
            ...
            if payment_method == "PSE":
                metadata.update(
                    {
                        "async_payment_url": transaction.get("payment_method", {})
                        .get("extra", {})
                        .get("async_payment_url"),
                        "external_identifier": transaction.get("payment_method", {})
                        .get("extra", {})
            ...
            self.log_error(
                f"Wompi API request failed: {error_message}", {"error": error_data}
            )
    ...
    def confirm_payment(self, transaction_id: str) -> PaymentResult:
        ...
        try:
            ...
            self.log_error(
                f"Wompi confirmation request failed: {str(e)}", {"error": error_data}
            )
    ...
    def refund_payment(
        self, transaction_id: str, amount: Optional[Decimal] = None, reason: str = ""
    ) -> PaymentResult:
        """
        ...
        try:
            ...
            self.log_error(
                f"Wompi void request failed: {str(e)}", {"error": error_data}
            )
    ...
    def handle_webhook(
        ...
        try:
            ...
            self.log_info(
                f"Wompi webhook received: {event_type} for {transaction_id} - {status}"
            )
    ...
    def create_payment_source(
        self, token: str, customer_email: str, acceptance_token: str
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_integrity_signature(
        self, reference: str, amount_in_cents: int, currency: str
    ) -> str:
        """
