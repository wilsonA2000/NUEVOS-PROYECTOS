...
class ContractEscrowAccount(models.Model):
    ...
    contract = models.OneToOneField(
        ColombianContract, on_delete=models.PROTECT, related_name="escrow_account"
    )
    ...
    hold_period_hours = models.IntegerField(
        default=24, help_text="Horas antes de liberación automática"
    )
...
class EscrowTransaction(models.Model):
    ...
    escrow_account = models.ForeignKey(
        ContractEscrowAccount, on_delete=models.CASCADE, related_name="transactions"
    )
    ...
    related_milestone = models.ForeignKey(
        ContractMilestone, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class EscrowReleaseRule(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    escrow_account = models.ForeignKey(
        ContractEscrowAccount, on_delete=models.CASCADE, related_name="release_rules"
    )
...
class EscrowService:
    ...
    @staticmethod
    @transaction.atomic
    def create_escrow_for_contract(
        contract: ColombianContract,
        ...
        escrow_account = ContractEscrowAccount.objects.create(
            contract=contract, auto_release_enabled=True, hold_period_hours=24
        )
    ...
    def deposit_funds(
        ...
        payment_result = EscrowService._process_payment(
            amount=amount, payment_method=payment_method, depositor=depositor
        )
    ...
    def hold_funds_for_milestone(
        milestone: ContractMilestone, amount: Decimal, hold_until: datetime
    ) -> EscrowTransaction:
    ...
    def release_funds_for_milestone(
        ...
        if transfer_result["success"]:
            ...
            raise ValidationError(
                f"Error transfiriendo fondos: {transfer_result['error']}"
            )
    ...
    def _process_payment(
        amount: Decimal, payment_method: str, depositor: "User"
    ) -> Dict:
    ...
    @staticmethod
    def _notify_deposit(
        escrow_account: ContractEscrowAccount, amount: Decimal, depositor: "User"
    ):
    ...
    @staticmethod
    def _notify_release(
        escrow_account: ContractEscrowAccount, amount: Decimal, recipient: "User"
    ):
...
class EscrowDisputeResolution:
    ...
    @staticmethod
    def resolve_dispute(dispute_id: str, resolution: Dict, resolved_by: "User"):
