...
class PaymentPlan(models.Model):
    ...
    debtor = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="payment_plans_as_debtor"
    )
    creditor = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="payment_plans_as_creditor"
    )
    ...
    installment_amount = models.DecimalField(
        "Monto por cuota", max_digits=10, decimal_places=2
    )

    # Intereses y cargos
    interest_rate = models.DecimalField(
        "Tasa de interés (%)", max_digits=5, decimal_places=2, default=Decimal("0.00")
    )
    ...
    late_fee = models.DecimalField(
        "Cargo por mora", max_digits=8, decimal_places=2, default=Decimal("0.00")
    )

    # Estado
    status = models.CharField(
        "Estado", max_length=20, choices=PLAN_STATUS, default="draft"
    )
    ...
    total_paid = models.DecimalField(
        "Total pagado", max_digits=12, decimal_places=2, default=Decimal("0.00")
    )
    total_pending = models.DecimalField(
        "Total pendiente", max_digits=12, decimal_places=2, default=Decimal("0.00")
    )
...
class PaymentPlanInstallment(models.Model):
    ...
    amount_paid = models.DecimalField(
        "Monto pagado", max_digits=10, decimal_places=2, default=Decimal("0.00")
    )
