...
def _user(email, user_type='tenant'):
    return User.objects.create_user(
        ...
        first_name='X', last_name='Y', user_type=user_type,
    )
...
class RentReconciliationTests(TestCase):
    ...
    def _setup(self):
        ...
        contract = Contract.objects.create(
            ...
            property=prop, contract_type='rental_urban',
            title='Test', monthly_rent=Decimal('1500000'),
            start_date=date(2026, 1, 1), end_date=date(2026, 12, 31),
        )
        schedule = RentPaymentSchedule.objects.create(
            contract=contract, tenant=tenant, landlord=landlord,
            rent_amount=Decimal('1500000'),
            due_date=1, grace_period_days=5, legal_grace_days_max=30,
            start_date=date(2026, 1, 1),
        )
        order = PaymentOrder.objects.create(
            order_type='rent',
            payer=tenant, payee=landlord, created_by=landlord,
...
class ServiceReconciliationTests(TestCase):
    ...
    def _setup(self):
        ...
        po = PaymentOrder.objects.create(
            order_type='service',
            payer=client_user, payee=provider, created_by=provider,
        ...
        so = ServiceOrder.objects.create(
            ...
            title='Limpieza', amount=Decimal('300000'),
            status='accepted', payment_order=po,
        )
        tx = Transaction.objects.create(
            payer=client_user, payee=provider,
