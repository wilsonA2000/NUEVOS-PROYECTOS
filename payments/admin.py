...
class PaymentOrderAdmin(admin.ModelAdmin):
    list_display = ('order_number', 'order_type', 'status', 'payer', 'payee',
                    'amount', 'interest_amount', 'paid_amount', 'date_due')
    ...
    readonly_fields = ('id', 'order_number', 'created_at', 'updated_at', 'paid_at',
                       'audit_log', 'total_amount_display', 'balance_display')
    date_hierarchy = 'date_due'
    fieldsets = (
        ('Identificación', {'fields': ('id', 'order_number', 'order_type', 'status')}),
        ('Partes', {'fields': ('payer', 'payee', 'created_by')}),
        ('Montos', {'fields': ('amount', 'interest_amount', 'paid_amount',
                                'total_amount_display', 'balance_display')}),
        ('Fechas', {'fields': ('date_due', 'date_grace_end', 'date_max_overdue', 'paid_at')}),
        ('Origen', {'fields': ('rent_schedule', 'installment', 'invoice', 'transaction')}),
