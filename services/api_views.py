...
class ServiceViewSet(viewsets.ModelViewSet):
        ...
        if not user.is_staff:
            ...
            if subscription is None:
                raise PermissionDenied(
                    "Se requiere una suscripción activa para publicar servicios."
                )
    ...
    def perform_destroy(self, instance):
        ...
        if provider and not provider.is_staff:
            ...
            if subscription and subscription.services_published:
                subscription.services_published = max(
                    0, subscription.services_published - 1
                )
                subscription.save(update_fields=["services_published"])
    ...
    def mine(self, request):
        ...
        my_services = Service.objects.filter(provider=request.user).select_related(
            "category"
        )
...
class PopularServicesListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Service.objects.filter(is_active=True).order_by("-popularity_score")[:20]
...
class FeaturedServicesListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Service.objects.filter(is_active=True, is_featured=True).order_by(
            "-popularity_score"
        )
...
class MostRequestedServicesListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Service.objects.filter(is_active=True, is_most_requested=True).order_by(
            "-requests_count"
        )
...
class ServicesByCategoryListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Service.objects.filter(
            is_active=True, category__slug=category_slug, category__is_active=True
...
class ServiceSubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.is_staff:
            return ServiceSubscription.objects.select_related(
    ...
    def current(self, request):
        ...
        try:
            ...
            return Response(
                {"detail": "No tiene suscripción activa"},
    ...
    def subscribe(self, request):
        ...
        if not plan_id:
            return Response(
                {"error": "plan_id es obligatorio"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if existing_active:
            return Response(
                {
                    "error": "Ya tiene una suscripción activa. Cancele primero para cambiar de plan."
                },
    ...
    def stats(self, request):
        ...
        active = ServiceSubscription.objects.filter(
            status__in=["trial", "active"]
        ).count()
        by_plan = dict(
...
class ServiceOrderViewSet(viewsets.ModelViewSet):
    ...
    def send(self, request, pk=None):
        ...
        if order.provider_id != request.user.id:
            return Response(
                {"error": "Solo el emisor puede enviar."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if order.status != "draft":
            return Response(
                {
                    "error": f"Solo órdenes en draft pueden enviarse (actual: {order.status})."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        order.status = "sent"
    ...
    def accept(self, request, pk=None):
        ...
        if order.client_id != request.user.id:
            return Response(
                {"error": "Solo el cliente puede aceptar."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if order.status not in ("sent", "draft"):
            return Response(
                {"error": f"No se puede aceptar una orden en estado {order.status}."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def reject(self, request, pk=None):
        ...
        if order.client_id != request.user.id:
            return Response(
                {"error": "Solo el cliente puede rechazar."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if order.status not in ("sent", "draft"):
            return Response(
                {"error": f"No se puede rechazar una orden en estado {order.status}."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def cancel(self, request, pk=None):
        ...
        if not (user.is_staff or order.provider_id == user.id):
            return Response(
                {"error": "Solo el prestador o admin pueden cancelar."},
                status=status.HTTP_403_FORBIDDEN,
            )
        if order.status in ("paid", "cancelled"):
            return Response(
                {"error": f"Orden ya está {order.status}."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if order.payment_order and order.payment_order.status not in (
            "paid",
            "cancelled",
        ):
