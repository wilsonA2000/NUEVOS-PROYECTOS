...
class PropertyInterestRequestSerializer(BaseRequestSerializer):
    ...
    property_rent_price = serializers.DecimalField(
        source="property.rent_price", max_digits=10, decimal_places=2, read_only=True
    )
...
class TenantDocumentUploadSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            document_type == "otros" or document_type.startswith("otros_")
        ) and not data.get("other_description"):
            raise serializers.ValidationError(
                {
    ...
    def create(self, validated_data):
        ...
        try:
            match_request = MatchRequest.objects.filter(
                tenant=property_request.requester, property=property_request.property
            ).first()

            if match_request:
                validated_data["match_request"] = match_request
                print(
                    f"✅ SERIALIZER: Asignando match_request {match_request.id} al documento"
                )
            else:
                print(
                    f"⚠️ SERIALIZER: No se encontró MatchRequest para {property_request.id}"
                )
        ...
        if not document_type.startswith("otros_"):
            existing_document = TenantDocument.objects.filter(
                property_request=property_request, document_type=document_type
            ).first()
            ...
            if existing_document:
                ...
                print(
                    f"🔄 Documento existente eliminado para reemplazo: {document_type}"
                )
        else:
        ...
        print(
            f"✅ Nuevo documento creado: {document_type} - Status: {new_document.status}"
        )
