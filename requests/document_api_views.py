...
class TenantDocumentUploadAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            property_request = self._get_or_create_property_request(
    ...
    def _get_or_create_property_request(self, process_id, user):
        ...
        try:
            ...
            if user != match_request.tenant:
                print(
                    f"❌ CONSOLIDACIÓN: Usuario {user.id} no es el tenant {match_request.tenant.id}"
                )
                return None

            # Buscar PropertyInterestRequest existente
            existing_property_request = PropertyInterestRequest.objects.filter(
                requester=match_request.tenant, property=match_request.property
            ).first()

            if existing_property_request:
                print(
                    f"✅ CONSOLIDACIÓN: Using existing PropertyInterestRequest {existing_property_request.id}"
                )
            ...
            new_property_request = PropertyInterestRequest.objects.create(
                ...
                lease_duration_months=getattr(
                    match_request, "lease_duration_months", 12
                ),
            ...
            print(
                f"🆕 CONSOLIDACIÓN: Created new PropertyInterestRequest {new_property_request.id} from MatchRequest {process_id}"
            )
            return new_property_request

        except Exception as e:
            print(
                f"⚠️ CONSOLIDACIÓN: MatchRequest {process_id} not found, trying PropertyInterestRequest: {e}"
            )
            pass
        ...
        try:
            ...
            if user != property_request.requester:
                print(
                    f"❌ FALLBACK: Usuario {user.id} no es el requester {property_request.requester.id}"
                )
                return None

            print(
                f"✅ FALLBACK: Using direct PropertyInterestRequest {property_request.id}"
            )
            return property_request

        except Exception as e:
            print(
                f"❌ ERROR: Process {process_id} not found in either MatchRequest or PropertyInterestRequest: {e}"
            )
            return None
...
class TenantDocumentListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user not in [
            property_request.requester,
            property_request.assignee,
        ]:
        ...
        if serializer.is_valid():
            ...
            if document.match_request:
                ...
                if pending == 0 and rejected == 0 and all_docs.exists():
                    ...
                    match_request.save()
                    print(
                        f"✅ Todos los documentos aprobados para match {match_request.match_code}"
                    )
                else:
...
class DocumentChecklistAPIView(generics.GenericAPIView):
    ...
    def get(self, request, process_id):
        ...
        try:
            ...
            class MatchRequestWrapper:
                def __init__(self, match_request):
                    ...
                    self.requester = (
                        match_request.tenant
                    )  # tenant en MatchRequest = requester en PropertyInterestRequest
                    self.assignee = (
                        match_request.landlord
                    )  # landlord en MatchRequest = assignee en PropertyInterestRequest
            ...
            try:
                property_request = get_object_or_404(
                    PropertyInterestRequest, id=process_id
                )
                print(
                    f"🔄 FALLBACK: Using PropertyInterestRequest {process_id} for documents"
                )
            except Exception:
                return Response(
        ...
        print(
            f"  - Requester: {property_request.requester.email if property_request.requester else 'None'} (ID: {property_request.requester.id if property_request.requester else 'None'})"
        )
        print(
            f"  - Assignee: {property_request.assignee.email if property_request.assignee else 'None'} (ID: {property_request.assignee.id if property_request.assignee else 'None'})"
        )
        ...
        if request.user not in allowed_users:
            print(
                f"❌ CHECKLIST: Permission denied - User {request.user.email} not in allowed list"
            )
            error_details = {
                ...
                "debug_info": {
                    ...
                    "requester_email": property_request.requester.email
                    if property_request.requester
                    else None,
                    "assignee_email": property_request.assignee.email
                    if property_request.assignee
                    else None,
                },
            }
        ...
        if hasattr(property_request, "_match_request"):
            ...
            try:
                ...
                if related_property_request:
                    ...
                    print(
                        f"🔄 CONSOLIDACIÓN: Found {uploaded_documents.count()} documents via PropertyInterestRequest"
                    )
                else:
                    uploaded_documents = TenantDocument.objects.none()
                    print(
                        f"🔄 CONSOLIDACIÓN: No PropertyInterestRequest found for MatchRequest {process_id}"
                    )
            except Exception as e:
                uploaded_documents = TenantDocument.objects.none()
        ...
        checklist_data = self._build_checklist(
            documents_map, property_request, otros_documents_list
        )
        ...
        response_data = {
            ...
            **stats,
            ...
            if request.user == property_request.requester
            else "landlord",
        }
    ...
    def _build_checklist(
        self, documents_map, property_request, otros_documents_list=None
    ):
        ...
        codeudor_types = [
            ...
            (
                ...
                "Codeudor: Certificado de Libertad y Tradición",
                False,
            ),
        ]
        ...
        def build_category_documents(types_list, category_name):
            ...
            for doc_type, display_name, is_required in types_list:
                doc_info = {
                    "id": str(documents_map[doc_type].id)
                    if doc_type in documents_map
                    else None,
                    "type": doc_type,
                    "display_name": display_name,
                    "required": is_required,
                    "uploaded": doc_type in documents_map,
                    "status": documents_map[doc_type].status
                    if doc_type in documents_map
                    else None,
                    "status_display": documents_map[doc_type].get_status_display()
                    if doc_type in documents_map
                    else "No subido",
                    "status_color": documents_map[doc_type].get_status_color()
                    if doc_type in documents_map
                    else "default",
                    "uploaded_at": documents_map[doc_type].uploaded_at.isoformat()
                    if doc_type in documents_map
                    else None,
                    "reviewed_at": documents_map[doc_type].reviewed_at.isoformat()
                    if doc_type in documents_map and documents_map[doc_type].reviewed_at
                    else None,
                    "file_url": documents_map[doc_type].get_file_url()
                    if doc_type in documents_map
                    else None,
                    "review_notes": documents_map[doc_type].review_notes
                    if doc_type in documents_map
                    else None,
                    "other_description": documents_map[doc_type].other_description
                    if doc_type in documents_map
                    else None,
                }
        ...
        def build_otros_documents(otros_docs):
            ...
            for doc in otros_docs:
                ...
                doc_info = {
                    ...
                    "original_filename": doc.document_file.name.split("/")[-1]
        ...
        return {
            ...
            "otros_documents": build_otros_documents(
                otros_documents_list
            ),  # ✅ Lista dinámica
        }
    ...
    def _calculate_stats(self, uploaded_documents, checklist_data):
        ...
        for category in ["tomador_documents", "codeudor_documents", "otros_documents"]:
            total_required += sum(
                1 for doc in checklist_data[category] if doc["required"]
            )
        ...
        all_approved = (
            all_required_uploaded and total_pending == 0 and total_rejected == 0
        )
...
class SecureDocumentDownloadView(APIView):
    ...
    def get(self, request, document_id):
        ...
        if not self._can_access(request.user, document):
            ...
            print(
                f"⚠️ SECURE_DOWNLOAD: Acceso denegado - Usuario {request.user.email} intentó acceder a documento {document_id}"
            )
            return Response(
                {
        ...
        try:
            DocumentAccessLog.log_access(
                document=document, user=request.user, action="download", request=request
            )
            print(
                f"📋 SECURE_DOWNLOAD: Acceso registrado - {request.user.email} descargó {document.document_type}"
            )
        ...
        if not document.document_file:
            return Response(
                {
                    "error": "El archivo del documento no existe.",
                    "code": "FILE_NOT_FOUND",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        try:
            ...
            filename = (
                ...
                or f"{document.document_type}_{document.id}.pdf"
            )
...
class SecureDocumentPreviewView(APIView):
    ...
    def get(self, request, document_id):
        ...
        try:
            DocumentAccessLog.log_access(
                document=document, user=request.user, action="preview", request=request
            )
        except Exception as log_error:
        ...
        try:
            ...
            response["Content-Disposition"] = (
                f'inline; filename="{document.original_filename or "documento.pdf"}"'
            )
            response["X-Content-Type-Options"] = "nosniff"
            response["X-VeriHome-Access-Logged"] = "true"
            ...
            return Response(
                {
                    "error": f"Error al mostrar el archivo: {str(e)}",
                    "code": "PREVIEW_ERROR",
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
def document_access_history(request, document_id):
    ...
    can_view_history = (
        ...
        or (document.match_request and document.match_request.landlord == request.user)
        or (
            document.property_request
            and document.property_request.assignee == request.user
        )
    )
    ...
    access_logs = (
        DocumentAccessLog.objects.filter(document=document)
        .select_related("user")
    ...
    history = [
        {
            ...
            "user_agent_short": (log.user_agent[:100] + "...")
