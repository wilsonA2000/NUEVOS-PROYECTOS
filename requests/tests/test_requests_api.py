...
class RequestsAPITestBase(TestCase):
    ...
    def setUp(self):
        ...
        self.landlord = User.objects.create_user(
            ...
            first_name='Land', last_name='Lord',
            user_type='landlord',
        )
...
class DashboardStatsTests(RequestsAPITestBase):
    ...
    def test_dashboard_stats(self):
        BaseRequest.objects.create(
            ...
            requester=self.tenant, assignee=self.landlord,
        )
        self.client.force_authenticate(user=self.tenant)
    ...
    def test_mark_all_as_read(self):
        ...
        for i in range(3):
            RequestNotification.objects.create(
                request=req, recipient=self.tenant,
                notification_type='status_change',
                title=f'Notif {i}', message=f'msg {i}',
            )
        self.client.force_authenticate(user=self.tenant)
        response = self.client.post('/api/v1/requests/api/notifications/mark_all_as_read/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            RequestNotification.objects.filter(recipient=self.tenant, is_read=False).count(),
            0,
        )
...
class CommentTests(RequestsAPITestBase):
    ...
    def test_list_comments(self):
        RequestComment.objects.create(
            request=self.req, author=self.tenant, content='Hola',
        )
        self.client.force_authenticate(user=self.tenant)
...
class RequestModelTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(
            email='u2@test.com', password='test1234',
            first_name='C', last_name='D', user_type='landlord',
        )
    ...
    def test_notification_mark_as_read(self):
        req = BaseRequest.objects.create(
            request_type='property_interest',
            title='t', description='d',
            requester=self.user1, assignee=self.user2,
        )
        notif = RequestNotification.objects.create(
            request=req, recipient=self.user1,
            notification_type='new_request',
            title='Nueva', message='msg',
        )
