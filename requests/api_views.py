...
from .serializers import (
    ...
    CreateBaseRequestSerializer,
    PropertyInterestRequestSerializer,
    CreatePropertyInterestRequestSerializer,
...
class BaseRequestViewSet(viewsets.ModelViewSet):
    ...
    def perform_action(self, request, pk=None):
        ...
        # Verificar permisos
        if (
            action_type in ["accept", "reject", "complete"]
            and request.user != request_obj.assignee
        ):
            return Response(
        ...
        if message:
            RequestComment.objects.create(
                request=request_obj, author=request.user, content=message
            )
    ...
    def dashboard_stats(self, request):
        ...
        stats["recent_activity"] = BaseRequestSerializer(
...
class RequestCommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return RequestComment.objects.filter(request_id=request_id).select_related(
            "author"
        )

    def perform_create(self, serializer):
        request_id = self.kwargs.get("request_pk")
        serializer.save(author=self.request.user, request_id=request_id)
