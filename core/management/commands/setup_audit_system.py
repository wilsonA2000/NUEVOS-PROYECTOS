...
class Command(BaseCommand):
    ...
    def _setup_directories(self) -> int:
        ...
        try:
            ...
            if os.access(logs_dir, os.W_OK):
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"⚠ Directorio creado pero sin permisos de escritura: {logs_dir}"
                    )
                )
                return 0
    ...
    def _create_initial_metrics(self) -> int:
        ...
        initial_metrics = [
            ...
            {"metric_type": "audit_system_setup", "count": 1},
            {
                ...
                "count": User.objects.filter(last_login__date=today).count(),
            },
        ...
        for metric_data in initial_metrics:
            try:
                ...
                if created:
                    metrics_created += 1
                    self.stdout.write(
                        f"  ✓ Métrica creada: {metric.metric_type} = {metric.count}"
                    )
                else:
                    self.stdout.write(
                        f"  ↻ Métrica actualizada: {metric.metric_type} = {metric.count}"
                    )
            except Exception as e:
                self.stdout.write(
                    f'  ✗ Error creando métrica {metric_data["metric_type"]}: {str(e)}'
                )
    ...
    def _test_logging_system(self) -> int:
        ...
        try:
            ...
            if isinstance(analysis, dict) and "risk_score" in analysis:
                ...
                self.stdout.write(
                    "  ✗ Test 3: Análisis de seguridad - FALLÓ: Formato incorrecto"
                )
        except Exception as e:
        ...
        try:
            ...
            if isinstance(report, dict) and "report_id" in report:
                ...
                self.stdout.write(
                    "  ✗ Test 4: Generar reporte - FALLÓ: Formato incorrecto"
                )
        except Exception as e:
        ...
        if tests_passed == total_tests:
            ...
            self.stdout.write(
                self.style.WARNING(f"⚠ {total_tests - tests_passed} tests fallaron")
            )
    ...
    def _show_system_info(self):
        ...
        self.stdout.write(
            "python manage.py audit_maintenance --task=cleanup --retention-days=90"
        )
