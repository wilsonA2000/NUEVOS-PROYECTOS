...
class Command(BaseCommand):
    help = 'Optimiza la base de datos con índices, vacuum y otras mejoras de rendimiento'
    ...
    def _optimize_connection_settings(self, optimizer):
        ...
        if self.dry_run:
            self.stdout.write("  [DRY-RUN] Se aplicarían las siguientes configuraciones:")
    ...
    def _show_database_stats(self, optimizer):
        ...
        try:
            ...
            if 'size_mb' in stats:
                self.stdout.write(f"💾 Tamaño: {stats['size_mb']} MB ({stats['size_bytes']} bytes)")
            elif 'size' in stats:
    ...
    def _analyze_specific_query(self, optimizer, query):
        ...
        try:
            ...
            self.stdout.write(f"⏱️ Tiempo de ejecución: {analysis['execution_time_ms']:.2f} ms")
            ...
            if analysis['execution_time_ms'] > 100:
                ...
                self.stdout.write(
                    self.style.SUCCESS("✅ Query rápida (<50ms)")
                )
