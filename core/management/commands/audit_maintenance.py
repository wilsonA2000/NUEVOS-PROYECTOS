...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if verbose:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Iniciando mantenimiento de auditoría: {start_time}"
                )
            )
            if dry_run:
                self.stdout.write(
                    self.style.WARNING(
                        "MODO DE PRUEBA: No se realizarán cambios permanentes"
                    )
                )
    ...
    def _run_cleanup(self, retention_days: int, dry_run: bool, verbose: bool) -> dict:
        ...
        if verbose:
            self.stdout.write(f'Logs procesados para limpieza: {stats["total"]}')
            self.stdout.write(f'  - Activity logs: {stats["activity_logs"]}')
            self.stdout.write(f'  - User activity logs: {stats["user_activity_logs"]}')
            self.stdout.write(f'  - Admin action logs: {stats["admin_action_logs"]}')
    ...
    def _run_security_check(self, verbose: bool) -> dict:
        ...
        if verbose:
            ...
            self.stdout.write(
                f'Active impersonations: {analysis["active_impersonations"]}'
            )
            ...
            if analysis["suspicious_ips"]:
                ...
                for ip_data in analysis["suspicious_ips"]:
                    self.stdout.write(
                        f'  - {ip_data["ip_address"]}: {ip_data["failed_count"]} fallos'
                    )
    ...
    def _generate_system_alerts(self, dry_run: bool, verbose: bool) -> dict:
        ...
        if verbose:
            self.stdout.write(
                f'Alertas {"simuladas" if dry_run else "creadas"}: {alerts_created}'
            )
    ...
    def _run_optimization(self, dry_run: bool, verbose: bool) -> dict:
        ...
        if not dry_run:
            auto_resolved = SystemAlert.objects.filter(
                auto_resolve_at__lte=timezone.now(), is_resolved=False
            ).update(
                is_resolved=True,
        ...
        if not dry_run:
            ...
            SystemMetrics.objects.update_or_create(
                metric_type="maintenance_runs", date=today, defaults={"count": 1}
            )
    ...
    def _show_summary(self, results: dict, duration: float, verbose: bool):
        ...
        if "optimization" in results:
            ...
            self.stdout.write(
                f'  Alertas auto-resueltas: {opt["auto_resolved_alerts"]}'
            )
        ...
        if "security" in results:
            total_issues += (
                results["security"]["risk_score"] // 20
            )  # Convertir score a número de issues
        if "alerts" in results:
        ...
        if total_issues == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "✓ Sistema en estado óptimo - No se detectaron problemas"
                )
            )
        elif total_issues <= 3:
            self.stdout.write(
            ...
            self.stdout.write(
                self.style.ERROR(
                    f"✗ Se detectaron {total_issues} problemas que requieren atención"
                )
            )
