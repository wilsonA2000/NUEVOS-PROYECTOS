...
class ContactMessageAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if any(
                w in subject_lower for w in ["contrato", "arriendo", "legal", "juridic"]
            ):
                dept = "legal"
            elif any(
                w in subject_lower for w in ["pago", "factura", "cobro", "dinero"]
            ):
                dept = "billing"
            elif any(w in subject_lower for w in ["verificaci", "visita", "agente"]):
                dept = "verification_agents"
            elif any(
                w in subject_lower for w in ["servicio", "mantenimiento", "reparaci"]
            ):
                dept = "technical"
            else:
                dept = "general"
...
class SupportTicketViewSet(viewsets.ModelViewSet):
    ...
    def assign(self, request, pk=None):
        ...
        if not request.user.is_staff:
            return Response(
                {"error": "Solo staff puede asignar tickets"},
        ...
        if staff_id:
            try:
                ...
                return Response(
                    {"error": "Usuario staff no encontrado"},
    ...
    def respond(self, request, pk=None):
        ...
        ticket = self.get_object()
        message = (
            request.data.get("message") or request.data.get("response") or ""
        ).strip()
        if not message:
            return Response(
                {"error": "El mensaje es obligatorio (campo `message` o `response`)"},
        ...
        return Response(
            TicketResponseSerializer(response_obj).data, status=status.HTTP_201_CREATED
        )
    ...
    def resolve(self, request, pk=None):
        ...
        if not request.user.is_staff:
            return Response(
                {"error": "Solo staff puede resolver tickets"},
                status=status.HTTP_403_FORBIDDEN,
            )
    ...
    def stats(self, request):
        ...
        by_dept = dict(
            qs.values_list("department")
            .annotate(c=Count("id"))
            .values_list("department", "c")
        )
        by_priority = dict(
    ...
    def post(self, request):
        ...
        try:
            ...
            report = audit_service.generate_audit_report(
                start_date=start_date, end_date=end_date, include_sections=sections
            )
        ...
        try:
            ...
            if not request.user.is_staff:
                summary = audit_service.get_user_activity_summary(
                    user=request.user, days=30
                )
                return Response({"user_stats": summary, "is_admin": False})
            ...
            unique_users = (
                ActivityLog.objects.filter(
                    timestamp__range=[start_date, end_date], user__isnull=False
                )
                .values("user")
                .distinct()
                .count()
            ...
            failed_activities = ActivityLog.objects.filter(
                timestamp__range=[start_date, end_date], success=False
            ).count()
            ...
            stats = {
                ...
                "recent_security_events": {
                    "failed_logins": security_analysis.get("failed_logins", {}).get(
                        "total", 0
                    ),
                    "suspicious_ips": len(security_analysis.get("suspicious_ips", [])),
                    "active_impersonations": security_analysis.get(
    ...
    def post(self, request):
        ...
        try:
            ...
            if "activity" in log_types:
                ...
                export_data.extend(
                    [{**activity, "log_type": "activity"} for activity in activities]
                )
            ...
            if format_type == "json":
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="audit_logs_{start_date.date()}_{end_date.date()}.json"'
                )
            else:
                # Formato CSV
                import csv
                ...
                response["Content-Disposition"] = (
                    f'attachment; filename="audit_logs_{start_date.date()}_{end_date.date()}.csv"'
                )
