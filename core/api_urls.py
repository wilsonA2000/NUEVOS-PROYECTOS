...
urlpatterns = [
    ...
    path(
        "contact/",
        api_views.ContactMessageAPIView.as_view(),
        name="api_contact_message",
    ),
    # FAQ público
    ...
    path(
        "audit-logs/", api_views.GlobalAuditLogAPIView.as_view(), name="api_audit_logs"
    ),
    # SLA dashboard (admin). ADM-02.
    path(
        "admin/sla-dashboard/",
        api_views.SLADashboardAPIView.as_view(),
        name="api_sla_dashboard",
    ),
    # Estadísticas generales
    path(
        "stats/dashboard/",
        api_views.DashboardStatsAPIView.as_view(),
        name="api_dashboard_stats",
    ),
    path(
        "stats/overview/",
        api_views.SystemOverviewAPIView.as_view(),
        name="api_system_overview",
    ),
    path(
        "security/analysis/",
        api_views.SecurityAnalysisAPIView.as_view(),
        name="api_security_analysis",
    ),
    # Mantenimiento del sistema (admin only)
    path(
        "maintenance/health/",
        api_views.maintenance_health_check,
        name="maintenance_health",
    ),
    path(
        "maintenance/clear-logs/",
        api_views.maintenance_clear_logs,
        name="maintenance_clear_logs",
    ),
    path(
        "maintenance/clear-cache/",
        api_views.maintenance_clear_cache,
        name="maintenance_clear_cache",
    ),
    path(
        "maintenance/clear-sessions/",
        api_views.maintenance_clear_sessions,
        name="maintenance_clear_sessions",
    ),
    path(
        "maintenance/optimize-db/",
        api_views.maintenance_optimize_db,
        name="maintenance_optimize_db",
    ),
]
