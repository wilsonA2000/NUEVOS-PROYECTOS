...
class PerformanceMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        self.metrics["network_bytes_sent"].append(
            {"timestamp": datetime.now(), "value": net_io.bytes_sent}
        )
        self.metrics["network_bytes_recv"].append(
            {"timestamp": datetime.now(), "value": net_io.bytes_recv}
        )

        # Procesos
        self.metrics["process_count"].append(
            {"timestamp": datetime.now(), "value": len(psutil.pids())}
        )
    ...
    def _collect_django_metrics(self):
        ...
        if request_metrics:
            ...
            self.metrics["response_time_avg"].append(
                {"timestamp": datetime.now(), "value": avg_response_time}
            )
            ...
            self.metrics["error_rate"].append(
                {"timestamp": datetime.now(), "value": error_rate}
            )
    ...
    def _collect_cache_metrics(self):
        ...
        try:
            ...
            self.metrics["redis_clients"].append(
                {"timestamp": datetime.now(), "value": info.get("connected_clients", 0)}
            )
            ...
            self.metrics["cache_hit_rate"].append(
                {"timestamp": datetime.now(), "value": hit_rate}
            )
            ...
            with connection.cursor() as cursor:
                if "postgresql" in settings.DATABASES["default"]["ENGINE"]:
                    ...
                    self.metrics["active_connections"].append(
                        {"timestamp": datetime.now(), "value": active_connections}
                    )
                    ...
                    if result:
                        ...
                        self.metrics["avg_query_time"].append(
                            {"timestamp": datetime.now(), "value": avg_query_time or 0}
                        )
                        self.metrics["total_db_calls"].append(
                            {"timestamp": datetime.now(), "value": total_calls or 0}
                        )
...
def get_performance_report():
    ...
    if metrics.get("cache_hit_rate", {}).get("current", 100) < 80:
        report["recommendations"].append("Low cache hit rate. Review caching strategy.")
