...
class AuditService:
        ...
        try:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "action_type": action_type,
                    "description": description,
                    "target_object": f"{content_type.model}:{object_id}"
                    if content_type
                    else None,
                    "success": success,
                    "ip_address": ip_address,
    ...
    def log_bulk_activities(
        self, activities: List[Dict[str, Any]], session_id: Optional[str] = None
    ) -> List[ActivityLog]:
        """
    ...
    def create_system_alert(
        ...
        alert = SystemAlert.objects.create(
            ...
            auto_resolve_at=timezone.now() + auto_resolve_after
    ...
    def analyze_security_events(
        ...
        failed_logins = (
            ActivityLog.objects.filter(
                created_at__gte=cutoff_time, action_type="login", success=False
            )
            .values("ip_address")
        ...
        suspicious_ips = (
            ActivityLog.objects.filter(created_at__gte=cutoff_time, success=False)
            .values("ip_address")
        ...
        user_locations = (
            ActivityLog.objects.filter(created_at__gte=cutoff_time, user__isnull=False)
        ...
        security_alerts = SystemAlert.objects.filter(
            created_at__gte=cutoff_time, category="security", is_resolved=False
        ).count()
        ...
        logger.info(
            ...
            extra={"analysis": analysis, "period_hours": analysis["period_hours"]},
        )
    ...
    def get_user_activity_summary(self, user: User, days: int = 30) -> Dict[str, Any]:
        ...
        activities_by_type = (
            ActivityLog.objects.filter(user=user, created_at__gte=cutoff_date)
            .values("action_type")
            .annotate(count=Count("id"))
            .order_by("-count")
        )
        ...
        failed_activities = ActivityLog.objects.filter(
            user=user, created_at__gte=cutoff_date, success=False
        ).count()
        ...
        admin_activities = UserActivityLog.objects.filter(
            user=user, created_at__gte=cutoff_date, metadata__performed_by_admin=True
        ).count()
        ...
        summary = {
            ...
            "most_active_day": max(activities_by_day, key=lambda x: x["count"])["day"]
    ...
    def generate_audit_report(
        ...
        if "general_stats" in include_sections:
            report["sections"]["general_stats"] = self._get_general_stats(
                start_date, end_date
            )

        # Actividades de usuarios
        if "user_activities" in include_sections:
            report["sections"]["user_activities"] = self._get_user_activities_stats(
                start_date, end_date
            )

        # Acciones de administrador
        if "admin_actions" in include_sections:
            report["sections"]["admin_actions"] = self._get_admin_actions_stats(
                start_date, end_date
            )

        # Eventos de seguridad
        if "security_events" in include_sections:
            report["sections"]["security_events"] = self._get_security_events_stats(
                start_date, end_date
            )

        # Alertas del sistema
        if "system_alerts" in include_sections:
            report["sections"]["system_alerts"] = self._get_system_alerts_stats(
                start_date, end_date
            )

        # Métricas de rendimiento
        if "performance_metrics" in include_sections:
            report["sections"]["performance_metrics"] = self._get_performance_stats(
                start_date, end_date
            )
    ...
    def cleanup_old_logs(
        ...
        stats = {
            ...
            "total": activity_logs_count
            + user_activity_logs_count
            + admin_action_logs_count,
        }
    ...
    def _get_general_stats(
        ...
        unique_users = (
            ActivityLog.objects.filter(
                created_at__range=[start_date, end_date], user__isnull=False
            )
            .values("user")
            .distinct()
            .count()
        ...
        failed_activities = ActivityLog.objects.filter(
            created_at__range=[start_date, end_date], success=False
        ).count()
        ...
        return {
            ...
            "failed_activities": failed_activities,
            "success_rate": (
                (total_activities - failed_activities) / total_activities * 100
            )
            if total_activities > 0
            else 100,
        }

    def _get_user_activities_stats(
        self, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        """Obtiene estadísticas de actividades de usuarios."""
        return {
            "activities_by_type": list(
                ActivityLog.objects.filter(created_at__range=[start_date, end_date])
                .values("action_type")
                .annotate(count=Count("id"))
                .order_by("-count")[:10]
            ),
    ...
    def _get_admin_actions_stats(
        ...
        return {
            ...
            "actions_by_type": list(
                AdminActionLog.objects.filter(created_at__range=[start_date, end_date])
                .values("action_type")
                .annotate(count=Count("id"))
                .order_by("-count")[:10]
            ),
    ...
    def _get_system_alerts_stats(
        ...
        return {
            ...
            "alerts_by_level": list(
                SystemAlert.objects.filter(created_at__range=[start_date, end_date])
                .values("level")
                .annotate(count=Count("id"))
                .order_by("-count")
            ),
            "unresolved_alerts": SystemAlert.objects.filter(
                created_at__range=[start_date, end_date], is_resolved=False
            ).count(),
        }
