...
from .models import SiteConfiguration, Notification, ActivityLog, FAQ, SupportTicket
...
class HomeView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["stats"] = {
            ...
            "total_landlords": User.objects.filter(
                user_type="landlord", is_active=True
            ).count(),
            "total_service_providers": User.objects.filter(
                user_type="service_provider", is_active=True
            ).count(),
        ...
        if self.request.user.is_authenticated:
            ...
            context["recent_ratings"] = Rating.objects.filter(
                overall_rating__gte=8, is_public=True, moderation_status="approved"
...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def _get_landlord_context(self):
        ...
        # Contratos activos
        active_contracts = Contract.objects.filter(primary_party=user, status="active")

        # Pagos pendientes
        pending_payments = Transaction.objects.filter(payee=user, status="pending")

        # Consultas recientes
        from properties.models import PropertyInquiry

        recent_inquiries = PropertyInquiry.objects.filter(
            property__landlord=user, status="new"
        )[:5]
        ...
        return {
            ...
            ).aggregate(total=django.db.models.Sum("amount"))["total"]
            or 0,
        }
    ...
    def _get_tenant_context(self):
        ...
        # Mis contratos
        my_contracts = Contract.objects.filter(secondary_party=user)

        # Pagos realizados
        my_payments = Transaction.objects.filter(payer=user)
        ...
        return {
            ...
            "monthly_rent": my_payments.filter(
                transaction_type="rent_payment", created_at__month=timezone.now().month
    ...
    def _get_service_provider_context(self):
        ...
        service_contracts = Contract.objects.filter(
            Q(primary_party=user) | Q(secondary_party=user), contract_type="service"
        )

        # Calificaciones recibidas
        from ratings.models import Rating

        my_ratings = Rating.objects.filter(reviewee=user, is_active=True)

        # Ingresos del mes
        monthly_income = (
            Transaction.objects.filter(
                payee=user, status="completed", created_at__month=timezone.now().month
            ).aggregate(total=django.db.models.Sum("amount"))["total"]
            or 0
        )
        ...
        return {
            ...
            "average_rating": my_ratings.aggregate(avg=Avg("overall_rating"))["avg"]
            or 0,
...
class SupportView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return SupportTicket.objects.filter(created_by=self.request.user).order_by(
            "-created_at"
        )
...
class CreateTicketView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, "Tu ticket de soporte ha sido creado exitosamente."
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["analytics"] = {
            "profile_views": ActivityLog.objects.filter(
                content_type__model="user", object_id=str(user.id), action_type="view"
            ).count(),
            "properties_viewed": Property.objects.filter(property_views__user=user)
            .distinct()
            .count(),
            "messages_sent": MessageThread.objects.filter(created_by=user).count(),
        }
...
def index(request):
    ...
    if settings.DEBUG:
        ...
        try:
            ...
            return HttpResponse(
                "Frontend no encontrado. Ejecuta 'npm run build' en el directorio frontend.",
                status=404,
            )
