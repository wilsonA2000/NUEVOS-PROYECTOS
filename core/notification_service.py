...
class NotificationService:
    ...
    def create_notification(
        ...
        try:
            ...
            logger.info(
                f"Notificación creada: {notification.id} para usuario {user.email}"
            )
            return notification
        ...
        try:
            ...
            if channel_layer:
                ...
                async_to_sync(channel_layer.group_send)(
                    ...
                    {"type": "notification.new", "notification": notification_data},
                )
    ...
    def _send_email_notification(user: User, notification: Notification):
        ...
        try:
            ...
            template_name = template_map.get(
                notification.notification_type, "core/email/generic_notification.html"
            )
...
VeriHome - {context['current_year']}"""
            ...
            logger.info(
                f"Email notification sent to {user.email} for {notification.notification_type}"
            )
            return True
    ...
    def _send_push_notification(user: User, notification: Notification):
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Push notification skipped (PushSubscription model not available): {notification.title} -> {user.email}"
                )
                return
            ...
            for sub in subscriptions:
                try:
                    ...
                    logger.warning(
                        f"Push notification failed for subscription {sub.id}: {e}"
                    )
        except ImportError:
            logger.info(
                f"Push notification skipped (pywebpush not installed): {notification.title} -> {user.email}"
            )
    ...
    @staticmethod
    def notify_contract_objection_submitted(
        landlord: User, contract: Any, tenant: User, objection_count: int
    ):
    ...
    def notify_signature_request(
        user: User, contract: Any, requester: User, signature_type: str = "digital"
    ):
    ...
    def notify_contract_published(tenant: User, contract: Any, landlord: User):
        ...
        return NotificationService.create_notification(
            ...
            message=(
                f"Tu contrato con {landlord.get_full_name()} está ahora activo. "
    ...
    def notify_contract_terminated(
        user: User, contract: Any, terminator: User, reason: str = ""
    ):
        """Notifica sobre terminación de contrato."""
        terminator_name = (
            "El sistema" if terminator == user else terminator.get_full_name()
        )
