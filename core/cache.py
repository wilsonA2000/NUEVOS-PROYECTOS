            ...
            if hasattr(cache, "_cache") and hasattr(cache._cache, "get_client"):
                ...
                try:
                    ...
                    if keys_to_delete:
                        ...
                        logger.info(
                            f"Deleted {len(keys_to_delete)} cache keys matching pattern: {pattern}"
                        )
                    else:
...
def _invalidate_known_keys(pattern):
    ...
    if pattern.endswith("*"):
            ...
            if hasattr(cache, "_cache"):
                ...
                try:
                    ...
                    for key in all_cache_keys:
                        ...
                        if stripped.startswith(base_pattern) or key.startswith(
                            base_pattern
                        ):
                    ...
                    logger.debug(
                        f"Found {len(matching_keys)} dynamic keys matching pattern: {pattern}"
                    )
        ...
        if not keys_to_delete or len(keys_to_delete) == len(
            pattern_mappings.get(pattern, [])
        ):
            common_suffixes = [
                "list",
                "detail",
    ...
    if deleted_count > 0:
        logger.info(
            f"Deleted {deleted_count} specific cache keys for pattern: {pattern}"
        )
    else:
