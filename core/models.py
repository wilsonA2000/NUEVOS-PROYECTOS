...
class ContactMessage(models.Model):
    ...
    status = models.CharField(
        "Estado", max_length=10, choices=STATUS_CHOICES, default="new"
    )
    ...
    def __str__(self):
        return (
            f"[{self.get_status_display()}] {self.subject} - {self.name} ({self.email})"
        )
...
class SiteConfiguration(models.Model):
    ...
    minimum_contract_duration = models.PositiveIntegerField(
        "Duración mínima de contrato (días)", default=30
    )
    maximum_contract_duration = models.PositiveIntegerField(
        "Duración máxima de contrato (días)", default=3650
    )

    # Configuraciones de verificación
    require_identity_verification = models.BooleanField(
        "Requerir verificación de identidad", default=True
    )
    require_income_verification = models.BooleanField(
        "Requerir verificación de ingresos", default=True
    )
    auto_approve_verified_users = models.BooleanField(
        "Aprobar automáticamente usuarios verificados", default=False
    )

    # Configuraciones de mensajería
    max_messages_per_day = models.PositiveIntegerField(
        "Máximo de mensajes por día", default=100
    )
    message_retention_days = models.PositiveIntegerField(
        "Días de retención de mensajes", default=365
    )

    # Configuraciones de calificaciones
    rating_scale_max = models.PositiveIntegerField(
        "Escala máxima de calificación", default=10
    )
    minimum_ratings_for_badge = models.PositiveIntegerField(
        "Mínimo de calificaciones para badge", default=5
    )
    ...
    google_analytics_id = models.CharField(
        "ID de Google Analytics", max_length=20, blank=True
    )
    facebook_pixel_id = models.CharField(
        "ID de Facebook Pixel", max_length=20, blank=True
    )
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(
        "Tipo", max_length=20, choices=NOTIFICATION_TYPES
    )
    ...
    priority = models.CharField(
        "Prioridad", max_length=10, choices=NOTIFICATION_PRIORITY, default="normal"
    )
...
class SystemAlert(models.Model):
    ...
    acknowledged_at = models.DateTimeField(
        "Fecha de reconocimiento", null=True, blank=True
    )
...
class FAQ(models.Model):
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_faqs"
    )
...
class SupportTicket(models.Model):
    ...
    department = models.CharField(
        "Departamento destino", max_length=25, choices=DEPARTMENTS, default="general"
    )
    priority = models.CharField(
        "Prioridad", max_length=10, choices=PRIORITY_LEVELS, default="normal"
    )
    status = models.CharField(
        "Estado", max_length=20, choices=TICKET_STATUS, default="open"
    )
...
class TicketResponse(models.Model):
    ...
    attachment = models.FileField(
        "Archivo adjunto", upload_to="support_attachments/", null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            f"Respuesta de {self.author.get_full_name()} - {self.ticket.ticket_number}"
        )
...
class SystemMetrics(models.Model):
    ...
    metric_type = models.CharField(
        "Tipo de métrica", max_length=30, choices=METRIC_TYPES
    )
