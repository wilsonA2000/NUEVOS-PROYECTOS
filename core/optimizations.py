...
class OptimizedPagination(PageNumberPagination):
    ...
    def paginate_queryset(self, queryset, request, view=None):
        ...
        if execution_time > 0.5:  # 500ms threshold
            logger.warning(
                f"Slow pagination detected: {execution_time:.3f}s for {view.__class__.__name__}"
            )
...
class QueryOptimizationMixin:
    ...
    OPTIMIZATION_PATTERNS = {
        ...
        "Transaction": {
            ...
            "annotations": {
                "days_since_created": F("created_at")
                - models.Value(datetime.now().date())
            },
        },
        "Message": {
            "select_related": ["sender", "conversation"],
            "prefetch_related": ["attachments", "conversation__participants"],
            "annotations": {"attachment_count": Count("attachments")},
        },
        "User": {
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            response["X-Query-Count"] = (
                len(connection.queries) if settings.DEBUG else "N/A"
            )
...
class OptimizedContractViewSet(QueryOptimizationMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Contract.objects.filter(Q(landlord=user) | Q(tenant=user))
...
class OptimizedTransactionViewSet(QueryOptimizationMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Transaction.objects.filter(Q(payer=user) | Q(payee=user)).order_by(
            "-created_at"
        )
    ...
    def recent(self, request):
        ...
        queryset = self.get_optimized_queryset().filter(created_at__gte=since_date)[
            :50
...
class PerformanceTrackingMixin:
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if execution_time > 1.0:  # 1 second threshold
            logger.warning(
                f"Slow request: {request.method} {request.path} took {execution_time:.3f}s with {query_count} queries"
            )
...
class CachedStatsViewSet(viewsets.ViewSet):
    ...
    def _calculate_dashboard_stats(self):
        ...
        raise NotImplementedError(
            "Subclasses must implement _calculate_dashboard_stats"
        )


# Optimized serializer base classes
class OptimizedPropertySerializer(
    OptimizedSerializerMixin, serializers.ModelSerializer
):
...
class OptimizedContractSerializer(
    OptimizedSerializerMixin, serializers.ModelSerializer
):
...
class OptimizedTransactionSerializer(
    OptimizedSerializerMixin, serializers.ModelSerializer
):
...
def monitor_query_performance(view_class):
    ...
    def monitored_dispatch(self, request, *args, **kwargs):
        ...
        logger.info(
            f"{view_class.__name__}.{request.method} - {execution_time:.3f}s - {query_count} queries"
        )
