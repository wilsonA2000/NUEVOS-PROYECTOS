...
class VeriHomeUser(HttpUser):
    ...
    def login(self):
        ...
        login_data = {"email": "tenant.test@verihome.com", "password": "TenantPass123!"}
            ...
            if properties:
                ...
                if detail_response.status_code == 200:
                    ...
                    if random.random() < 0.2:
                        self.client.post(
                            f"/api/v1/properties/{property_id}/toggle-favorite/"
                        )
        ...
        if response.status_code == 200:
            ...
            if properties:
                ...
                if random.random() < 0.3:
                    ...
                    self.client.patch(
                        f"/api/v1/properties/{property_id}/", json=update_data
                    )
...
class StressTestUser(HttpUser):
    ...
    def on_start(self):
        """Quick login"""
        login_data = {"email": "stress.test@verihome.com", "password": "StressTest123!"}
...
class PerformanceTestUser(HttpUser):
    ...
    def on_start(self):
        ...
        login_data = {"email": "perf.test@verihome.com", "password": "PerfTest123!"}
    ...
    def test_property_list_performance(self):
        ...
        with self.client.get("/api/v1/properties/", catch_response=True) as response:
            if response.elapsed.total_seconds() > 2.0:
                response.failure(
                    f"Request took {response.elapsed.total_seconds()} seconds"
                )
    ...
    def test_property_search_performance(self):
        ...
        with self.client.get(
            "/api/v1/properties/", params=params, catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 3.0:
                response.failure(
                    f"Search took {response.elapsed.total_seconds()} seconds"
                )
    ...
    def test_property_detail_performance(self):
        ...
        with self.client.get(
            f"/api/v1/properties/{property_id}/", catch_response=True
        ) as response:
            if response.elapsed.total_seconds() > 1.5:
                response.failure(
                    f"Property detail took {response.elapsed.total_seconds()} seconds"
                )
    ...
    def test_auth_performance(self):
        ...
        with self.client.get("/api/v1/auth/me/", catch_response=True) as response:
            if response.elapsed.total_seconds() > 1.0:
                response.failure(
                    f"Auth check took {response.elapsed.total_seconds()} seconds"
                )
    ...
    def test_complex_filter_performance(self):
        ...
        with self.client.get(
            ...
            if response.elapsed.total_seconds() > 4.0:
                response.failure(
                    f"Complex filter took {response.elapsed.total_seconds()} seconds"
                )
