...
class PropertyAmenitySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "category", "icon", "description", "is_active"]
...
class PropertyAmenityRelationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "amenity", "amenity_id", "available", "notes"]
...
class PropertyFavoriteSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "user", "property", "property_title", "created_at"]
...
class CreatePropertySerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
            ...
            if field in validated_data and isinstance(validated_data[field], str):
                try:
                    validated_data[field] = (
                        eval(validated_data[field]) if validated_data[field] else []
                    )
                except Exception:
                    validated_data[field] = []
        ...
        for i, image in enumerate(images):
            is_main = (main_image and image == main_image) or (
                i == 0 and not main_image
            )
            PropertyImage.objects.create(
                property=property_instance, image=image, is_main=is_main, order=i
            )
        ...
        for i, video in enumerate(video_files):
            ...
            if request and request.data:
                title = request.data.get(
                    title_key, f"Video {i+1} de {property_instance.title}"
                )
        ...
        for i, youtube_url in enumerate(youtube_urls):
            ...
            if request and request.data:
                title = request.data.get(
                    title_key, f"Video YouTube {i+1} de {property_instance.title}"
                )
                description = request.data.get(description_key, "")
    ...
    def update(self, instance, validated_data):
        ...
        for field in [
            ...
            if field in validated_data and isinstance(validated_data[field], str):
                try:
                    ...
                    validated_data[field] = (
                        json.loads(validated_data[field])
            ...
            for i, image in enumerate(images):
                ...
                if is_main:
                    PropertyImage.objects.filter(
                        property=instance, is_main=True
        ...
        if main_image_id:
            try:
                ...
                PropertyImage.objects.filter(property=instance, is_main=True).update(
                    is_main=False
                )
                # Marcar la imagen especificada como principal
                PropertyImage.objects.filter(
                    property=instance, id=main_image_id
                ).update(is_main=True)
                print(
                    f"✅ Imagen {main_image_id} marcada como principal para propiedad {instance.id}"
                )
        ...
        if video_file:
            PropertyVideo.objects.create(
                property=instance, video=video_file, title=f"Video de {instance.title}"
            )
        ...
        for i, video in enumerate(video_files):
            ...
            PropertyVideo.objects.create(
                property=instance, video=video, title=title, description=description
            )
        ...
        if deleted_count > 0:
            ...
            print(
                f"🔥 CLEANUP: Deleted {deleted_count} existing YouTube videos to prevent duplication"
            )

        # 2. Create fresh YouTube videos from the current request data ONLY
        unique_youtube_urls = list(
            dict.fromkeys([url.strip() for url in youtube_urls if url and url.strip()])
        )
        print(
            f"📹 Processing {len(unique_youtube_urls)} unique YouTube URLs: {unique_youtube_urls}"
        )
        ...
        for i, youtube_url in enumerate(unique_youtube_urls):
            try:
                ...
                print(
                    f"✅ NEW YouTube video created: {youtube_url} (ID: {new_video.id}, Order: {i+1})"
                )
            except Exception as e:
...
class PropertySearchSerializer(serializers.Serializer):
    ...
    min_bathrooms = serializers.DecimalField(
        max_digits=3, decimal_places=1, required=False
    )
    max_bathrooms = serializers.DecimalField(
        max_digits=3, decimal_places=1, required=False
    )
