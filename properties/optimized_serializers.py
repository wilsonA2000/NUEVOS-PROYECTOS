...
class OptimizedPropertySerializer(serializers.ModelSerializer):
    ...
    def get_videos(self, obj):
        ...
        if (
            ...
            and "videos" in obj._prefetched_objects_cache
        ):
    ...
    def get_amenities(self, obj):
        ...
        if hasattr(obj, "featured_amenities_only"):
            return OptimizedPropertyAmenityRelationSerializer(
                obj.featured_amenities_only, many=True
            ).data

        # Check if we have prefetched all amenities for detail view
        if hasattr(obj, "all_amenities"):
            return OptimizedPropertyAmenityRelationSerializer(
                obj.all_amenities, many=True
            ).data
    ...
    def get_recent_inquiries(self, obj):
        ...
        if hasattr(obj, "recent_inquiries"):
            ...
            return PropertyInquirySerializer(
                obj.recent_inquiries, many=True, context=self.context
            ).data
        return []
    ...
    def get_view_statistics(self, obj):
        ...
        if hasattr(obj, "recent_views"):
            return {
                ...
                "unique_viewers": len(
                    set(view.user_id for view in obj.recent_views if view.user_id)
                ),
            }
        return {
            "total_views": obj.views_count,
...
class OptimizedCreatePropertySerializer(serializers.ModelSerializer):
    ...
    def validate_utilities_included(self, value):
        ...
        if isinstance(value, str):
            ...
            utilities = [
                utility.strip() for utility in value.split(",") if utility.strip()
            ]
            return utilities
    ...
    def validate_nearby_amenities(self, value):
        ...
        if isinstance(value, str):
            ...
            # Si es string, convertir CSV a lista
            amenities = [
                amenity.strip() for amenity in value.split(",") if amenity.strip()
            ]
            return amenities
        elif isinstance(value, list):
            # Si ya es lista, validar y limpiar
    ...
    def validate_transportation(self, value):
        ...
        if isinstance(value, str):
            ...
            # Si es string, convertir CSV a lista
            transport = [
                transport.strip() for transport in value.split(",") if transport.strip()
            ]
            return transport
        elif isinstance(value, list):
            # Si ya es lista, validar y limpiar
    ...
    def validate_minimum_lease_term(self, value):
        ...
        if value is not None and value <= 0:
            raise serializers.ValidationError(
                "Minimum lease term must be greater than 0."
            )
        return value
    ...
    def validate(self, data):
        ...
        if listing_type == "rent":
            if (
                not rent_price
                or (isinstance(rent_price, str) and rent_price.strip() == "")
                or rent_price == 0
            ):
                raise serializers.ValidationError(
                    {"rent_price": "Rent price is required for rental properties."}
                )
        ...
        if listing_type == "sale":
            if (
                not sale_price
                or (isinstance(sale_price, str) and sale_price.strip() == "")
                or sale_price == 0
            ):
                raise serializers.ValidationError(
                    {"sale_price": "Sale price is required for sale properties."}
                )
    ...
    def create(self, validated_data):
        ...
        for i, youtube_url in enumerate(youtube_urls):
            ...
            title = (
                request.data.get(title_key, f"YouTube Video {i+1}")
                if request
                else "YouTube Video"
            )
            description = request.data.get(description_key, "") if request else ""
...
class OptimizedUpdatePropertySerializer(OptimizedCreatePropertySerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if video_files:
            for i, video in enumerate(video_files):
                if video:  # Solo archivos válidos
                    ...
                    title = (
                        request.data.get(title_key, f"Video {i+1}")
                        if request
                        else "Video Actualizado"
                    )
                    description = (
                        request.data.get(description_key, "") if request else ""
                    )
        ...
        has_youtube_data = any(
            key.startswith("youtube")
            for key in (request.data.keys() if request else [])
        )
        ...
        if has_youtube_data or youtube_urls:
            ...
            PropertyVideo.objects.filter(
                property=instance, youtube_url__isnull=False
            ).delete()
            ...
            for i, youtube_url in enumerate(youtube_urls):
                if youtube_url and youtube_url.strip():  # Solo URLs no vacías
                    ...
                    title = (
                        request.data.get(title_key, f"YouTube Video {i+1}")
                        if request
                        else "YouTube Video Actualizado"
                    )
                    description = (
                        request.data.get(description_key, "") if request else ""
                    )
