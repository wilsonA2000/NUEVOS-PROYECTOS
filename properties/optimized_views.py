...
from users.permissions import PropertyAccessMixin, RoleBasedPermissionMixin
...
class OptimizedPropertyViewSet(
    ...
    def get_queryset(self):
            ...
            queryset = queryset.annotate(
                ...
                is_favorited_by_user=Exists(
                    PropertyFavorite.objects.filter(
                        property=OuterRef("pk"), user=self.request.user
                    )
                ),
                ...
                user_inquiries_count=Count(
                    "inquiries", filter=Q(inquiries__inquirer=self.request.user)
                ),
            )
        ...
        if self.action == "list":
            ...
            queryset = queryset.prefetch_related(
                ...
                Prefetch(
                    "images",
                    queryset=PropertyImage.objects.filter(is_main=True).order_by(
                        "order"
                    )[:1],
            ...
            queryset = queryset.prefetch_related(
                ...
                Prefetch(
                    "property_views",
                    queryset=PropertyView.objects.select_related("user").order_by(
                        "-viewed_at"
                    )[:20],
    ...
    def list(self, request, *args, **kwargs):
        ...
        if response.status_code == 200:
            cache.set(
                cache_key, response.data, timeout=CACHE_TIMEOUTS.get("properties", 300)
            )
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        # Cache the response
        cache.set(
            cache_key, response_data, timeout=CACHE_TIMEOUTS.get("properties", 600)
        )
    ...
    def _track_property_view(self, property_obj, request):
        ...
        if request.user.is_authenticated:
            ...
            if created:
                ...
                logger.debug(
                    f"Tracked new view for property {property_obj.id} by user {request.user.id}"
                )
    ...
    def perform_create(self, serializer):
        ...
        logger.info(
            f"Created property {property_obj.id} by user {self.request.user.id}"
        )
    ...
    def search_suggestions(self, request):
        ...
        suggestions = (
            ...
            .values("id", "title", "city", "address", "property_type")
            .distinct()[:10]
        )
...
class OptimizedPropertyInquiryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if hasattr(self.request.user, "landlord_profile"):
            ...
            return (
                PropertyInquiry.objects.filter(property__landlord=self.request.user)
            ...
            return (
                PropertyInquiry.objects.filter(inquirer=self.request.user)
                .select_related("property", "property__landlord")
                .order_by("-created_at")
            )
