...
def handle_youtube_videos(property_instance, youtube_videos, delete_missing=False):
    ...
    print(
        f"🔄 Procesando {len(youtube_videos)} videos YouTube para propiedad {property_instance.id}..."
    )
    ...
    for youtube_video in youtube_videos:
        ...
        if url in existing_youtube_urls:
            ...
            try:
                ...
                created_count += 1
                print(
                    f"🎥 Nuevo video YouTube creado: {url} - {youtube_video['title']}"
                )
            except Exception as e:
    ...
    print(
        f"✅ Procesamiento completado: {created_count} creados, {updated_count} actualizados, {deleted_count} eliminados"
    )
    return created_count, updated_count, deleted_count


def handle_video_files(
    property_instance, video_files, video_metadata, delete_missing=False
):
    """
    ...
    if not video_files:
        ...
        if delete_missing:
            existing_file_videos = PropertyVideo.objects.filter(
                property=property_instance, video__isnull=False
            )
            deleted_count = existing_file_videos.count()
            if deleted_count > 0:
                print(
                    f"🗑️ Eliminando {deleted_count} videos de archivo que ya no están en la lista"
                )
                existing_file_videos.delete()
        return created_count, updated_count, deleted_count

    print(
        f"🎬 Procesando {len(video_files)} archivos de video para propiedad {property_instance.id}..."
    )
...
def sync_property_videos(property_instance, request_data):
    ...
    for key, value in request_data.items():
                ...
                youtube_videos.append(
                    {
                        ...
                        "title": request_data.get(
                            f"youtube_{index}_title", f"Video {index}"
                        ),
                        "description": request_data.get(
                            f"youtube_{index}_description", ""
                        ),
        ...
        elif key.startswith("video_") and (
            "_title" in key or "_description" in key or "_order" in key
        ):
    ...
    if youtube_videos or request_data.get("sync_youtube", False):
        created, updated, deleted = handle_youtube_videos(
            property_instance, youtube_videos, delete_missing=True
        )
    ...
    stats["total_videos"] = PropertyVideo.objects.filter(
        property=property_instance
    ).count()
