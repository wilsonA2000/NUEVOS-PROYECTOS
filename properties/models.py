...
class Property(models.Model):
    ...
    property_type = models.CharField(
        "Tipo de propiedad", max_length=20, choices=PROPERTY_TYPES
    )
    listing_type = models.CharField(
        "Tipo de listado", max_length=10, choices=LISTING_TYPE, default="rent"
    )
    status = models.CharField(
        "Estado", max_length=20, choices=PROPERTY_STATUS, default="available"
    )
    ...
    built_area = models.DecimalField(
        "Área construida (m²)", max_digits=8, decimal_places=2, null=True, blank=True
    )
    lot_area = models.DecimalField(
        "Área del terreno (m²)", max_digits=8, decimal_places=2, null=True, blank=True
    )
    parking_spaces = models.PositiveIntegerField(
        "Espacios de estacionamiento", default=0
    )
    ...
    year_built = models.PositiveIntegerField(
        "Año de construcción", null=True, blank=True
    )
    ...
    sale_price = models.DecimalField(
        "Precio de venta", max_digits=15, decimal_places=2, null=True, blank=True
    )
    security_deposit = models.DecimalField(
        "Depósito de garantía", max_digits=12, decimal_places=2, null=True, blank=True
    )
    maintenance_fee = models.DecimalField(
        "Cuota de mantenimiento", max_digits=8, decimal_places=2, null=True, blank=True
    )

    # Condiciones de renta
    minimum_lease_term = models.PositiveIntegerField(
        "Plazo mínimo de renta (meses)", default=12
    )
    maximum_lease_term = models.PositiveIntegerField(
        "Plazo máximo de renta (meses)", null=True, blank=True
    )
    ...
    property_features = models.JSONField(
        "Características de la propiedad", default=list
    )
            ...
            if not main_image:
                with transaction.atomic():
                    ...
                    if first_image:
                        ...
                        PropertyImage.objects.filter(id=first_image.id).update(
                            is_main=True
                        )
                        ...
                        print(
                            f"🔧 AUTO-REPAIR: Set image {first_image.id} as main for property {self.id}"
                        )
...
class PropertyVideo(models.Model):
    ...
    video = models.FileField(
        "Video", upload_to="properties/videos/", null=True, blank=True
    )
    youtube_url = models.URLField(
        "URL de YouTube", max_length=500, null=True, blank=True
    )
    title = models.CharField("Título", max_length=200)
    ...
    thumbnail = models.ImageField(
        "Miniatura", upload_to="properties/video_thumbnails/", null=True, blank=True
    )
    order = models.PositiveIntegerField("Orden", default=0)
    created_at = models.DateTimeField("Fecha de subida", auto_now_add=True)
...
class PropertyAmenity(models.Model):
    ...
    icon = models.CharField(
        "Icono", max_length=50, blank=True, help_text="Nombre del icono CSS"
    )
...
class PropertyAmenityRelation(models.Model):
    ...
    property = models.ForeignKey(
        Property, on_delete=models.CASCADE, related_name="amenity_relations"
    )
    amenity = models.ForeignKey(
        PropertyAmenity, on_delete=models.CASCADE, related_name="property_relations"
    )
...
class PropertyInquiry(models.Model):
    ...
    inquirer = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="property_inquiries"
    )
    ...
    lease_duration = models.PositiveIntegerField(
        "Duración de arrendamiento (meses)", null=True, blank=True
    )
    budget_min = models.DecimalField(
        "Presupuesto mínimo", max_digits=12, decimal_places=2, null=True, blank=True
    )
    budget_max = models.DecimalField(
        "Presupuesto máximo", max_digits=12, decimal_places=2, null=True, blank=True
    )

    # Estado y seguimiento
    status = models.CharField(
        "Estado", max_length=20, choices=INQUIRY_STATUS, default="new"
    )
