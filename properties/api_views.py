...
class PropertyViewSet(
    PropertyAccessMixin, RoleBasedPermissionMixin, viewsets.ModelViewSet
):
    ...
    def perform_destroy(self, instance):
        ...
        try:
            ...
            logger.info(
                f"Propiedad eliminada exitosamente: {property_title} (ID: {property_id})"
            )
            ...
            try:
                if hasattr(instance, "is_active"):
                    ...
                    SmartCache.invalidate_pattern("verihome:properties:*")
                    logger.warning(
                        f"Fallback a soft delete para propiedad {property_id}"
                    )
                else:
                ...
                logger.error(
                    f"Error en fallback para propiedad {property_id}: {str(fallback_error)}"
                )
                raise
...
class PropertyImageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        property_id = self.kwargs.get("property_pk") or self.request.query_params.get(
            "property"
        )
        if property_id:
...
class PropertyVideoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if hasattr(request, "impersonation_session"):
            ...
            UserActivityLog.objects.create(
                ...
                description=f"Agregar video a propiedad {video.property.title}",
                metadata={
                    "property_id": str(video.property.id),
    ...
    def reorder(self, request, pk=None, **kwargs):
        ...
        serializer = self.get_serializer(video)
        return Response(
            {"message": "Orden del video actualizado", "video": serializer.data}
        )
...
class PropertyInquiryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if hasattr(user, "landlord_profile"):
            return (
                PropertyInquiry.objects.filter(property__landlord=user)
                .select_related("property", "inquirer", "inquirer__tenant_profile")
                .order_by("-created_at")
            )
        ...
        # Arrendatarios y prestadores de servicios ven sus propias consultas
            ...
            return (
                PropertyInquiry.objects.filter(inquirer=user)
    ...
    def perform_create(self, serializer):
        ...
        if hasattr(request, "impersonation_session"):
            ...
            UserActivityLog.objects.create(
                ...
                description=f"Consulta sobre propiedad {inquiry.property.title}",
                metadata={
                    "property_id": str(inquiry.property.id),
...
class PropertyFavoriteViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            PropertyFavorite.objects.filter(user=self.request.user)
            .select_related("property", "property__landlord")
            .prefetch_related(
                Prefetch(
                    "property__images",
...
class PropertySearchAPIView(PropertyAccessMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = (
            Property.objects.filter(is_active=True, status="available")
            .select_related("landlord")
            .prefetch_related("images", "amenity_relations__amenity")
        )
...
class FeaturedPropertiesAPIView(PropertyAccessMixin, generics.ListAPIView):
    ...
    queryset = Property.objects.filter(
        is_active=True, status="available", is_featured=True
    )
    serializer_class = PropertySerializer
    permission_classes = [permissions.IsAuthenticated]
...
class TrendingPropertiesAPIView(PropertyAccessMixin, generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        trending_properties = (
            PropertyView.objects.filter(viewed_at__gte=thirty_days_ago)
            .values("property")
        ...
        return Property.objects.filter(
            is_active=True, status="available", id__in=property_ids
        )
    ...
    def get(self, request):
        ...
        if hasattr(user, "user_type") and user.user_type == "landlord":
            ...
            active_properties = Property.objects.filter(
                landlord=user, status="available"
            ).count()
            rented_properties = Property.objects.filter(
                landlord=user, status="rented"
            ).count()
...
class ToggleFavoriteAPIView(APIView):
    ...
    def post(self, request, property_id):
        ...
        try:
            ...
            return Response(
                {"message": message, "action": action, "property_id": property_id}
            )
...
class PropertyVideoDetailAPIView(APIView):
    ...
    def delete(self, request, video_id):
        ...
        if not video:
            return Response(
                {"error": "Video no encontrado"}, status=status.HTTP_404_NOT_FOUND
            )
